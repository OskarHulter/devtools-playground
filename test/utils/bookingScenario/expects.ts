...
import type { AppsStatus } from "@sln/types/Calendar";
import type { CalendarEvent } from "@sln/types/Calendar";
import type { Fixtures } from "@sln/web/test/fixtures/fixtures";

import { DEFAULT_TIMEZONE_BOOKER } from "./getMockRequestDataForCancelBooking";
...
expect.extend({
	toHaveEmail(
		...
		const icsObject = ics?.content ? isln.sync.parseICS(ics?.content) : null;
		...
		if (!isEmailContentMatched) {
			...
			return {
				pass: false,
				message: () =>
					`Email content ${isNot ? "is" : "is not"} matching. ${JSON.stringify(emailsToLog)}`,
