...
import { handleStripePaymentSuccess } from "@sln/features/ee/payments/api/webhook";
import { weekdayToWeekIndex, type WeekDays } from "@sln/lib/date-fns";
import type { HttpError } from "@sln/lib/http-error";
...
} from "@sln/prisma/client";
import type { SchedulingType, SMSLockState } from "@sln/prisma/enums";
import type { BookingStatus } from "@sln/prisma/enums";
import type { teamMetadataSchema } from "@sln/prisma/zod-utils";
import type { userMetadataType } from "@sln/prisma/zod-utils";
import type { AppMeta } from "@sln/types/App";
import type { NewCalendarEventType } from "@sln/types/Calendar";
import type { EventBusyDate, IntervalLimit } from "@sln/types/Calendar";

import { getMockPaymentService } from "./MockPaymentService";
import type { getMockRequestDataForBooking } from "./getMockRequestDataForCancelBooking";
...
export type InputEventType = {
	...
	Omit<
		...
		"users" | "schedule" | "bookingLimits" | "durationLimits"
	>
>;

type AttendeeBookingSeatInput = Pick<
	Prisma.BookingSeatCreateInput,
	"referenceUid" | "data"
>;
...
type WhiteListedBookingProps = {
	...
	references?: (Omit<
		ReturnType<typeof getMockBookingReference>,
		"credentialId"
	> & {
...
type InputBooking = Partial<Omit<Booking, keyof WhiteListedBookingProps>> &
	WhiteListedBookingProps;
...
export async function addEventTypesToDb(
	...
	for (let i = 0; i < eventTypes.length; i++) {
		...
		if (eventType.schedule) {
			log.silly(
				"TestData: Creating Schedule for EventType",
...
export async function addEventTypes(
	...
	const eventTypesWithUsers = eventTypes.map((eventType) => {
		if (!eventType.slotInterval && !eventType.length) {
			throw new Error(
				"eventTypes[number]: slotInterval or length must be defined",
			);
		}
	...
	log.silly(
		"TestData: Creating EventType",
		JSON.stringify(eventTypesWithUsers),
	);
	return await addEventTypesToDb(eventTypesWithUsers);
}

function addBookingReferencesToDB(
	bookingReferences: Prisma.BookingReferenceCreateManyInput[],
) {
		...
		workflows.map((workflow) => {
			return prismock.workflow.create({
				data: {
					...
					activeOn: {
						create: workflow.activeEventTypeId
							? { eventTypeId: workflow.activeEventTypeId }
							: undefined,
					},
				},
				include: {
...
export async function addTeamsToDb(
	teams: NonNullable<InputUser["teams"]>[number]["team"][],
) {
...
async function addUsers(users: InputUser[]) {
	...
	for (let i = 0; i < users.length; i++) {
		...
		if (user.teams) {
			const addedTeams = await addTeamsToDb(
				user.teams.map((team) => team.team),
			);
			newUser.teams = {
				// eslint-disable-next-line @typescript-eslint/ban-ts-comment
				//@ts-ignore
...
export function getMockedCredential({
	...
	const app =
		appStoreMetadata[metadataLookupKey as keyof typeof appStoreMetadata];
	return {
		type: app.type,
		appId: app.slug,
		app: app,
...
export const TestData = {
	...
	apps: {
		"google-calendar": {
			...
			//@ts-ignore
			keys: {
				expiry_date: Number.POSITIVE_INFINITY,
				client_id: "client_id",
	...
	eventTypes.forEach((eventType) => {
		if (
			eventType.users?.filter((eventTypeUser) => {
				return !users.find(
					(userToCreate) => userToCreate.id === eventTypeUser.id,
				);
			}).length
		) {
			throw new Error(
				`EventType ${eventType.id} has users that are not present in ScenarioData["users"]`,
			);
		}
	});
	return {
...
export function mockCalendar(
	...
	const appMock =
		appStoreMock.default[
			appStoreLookupKey as keyof typeof appStoreMock.default
		];
					...
					return {
						...
						createEvent: async (
							...
							return Promise.resolve({
								...
								id:
									normalizedCalendarData.create?.id ||
									"FALLBACK_MOCK_CALENDAR_EVENT_ID",
						...
						updateEvent: async (
							...
							const isGoogleMeetLocation =
								event.location === BookingLocations.GoogleMeet;
							return Promise.resolve({
								type: app.type,
								additionalInfo: {},
								uid: "PROBABLY_UNUSED_UID",
								...
								password: "MOCK_PASSWORD",
								url: "https://UNUSED_URL",
								location: isGoogleMeetLocation
									? "https://UNUSED_URL"
									: undefined,
								hangoutLink: isGoogleMeetLocation
									? "https://UNUSED_URL"
									: undefined,
								conferenceData: isGoogleMeetLocation
						...
						getAvailability: async (): Promise<EventBusyDate[]> => {
							if (calendarData?.getAvailabilityCrash) {
								throw new Error(
									"MockCalendarService.getAvailability fake error",
								);
							}
							return new Promise((resolve) => {
...
export function mockCalendarToCrashOnCreateEvent(
	metadataLookupKey: keyof typeof appStoreMetadata,
) {
	return mockCalendar(metadataLookupKey, { creationCrash: true });
}

export function mockCalendarToCrashOnUpdateEvent(
	metadataLookupKey: keyof typeof appStoreMetadata,
) {
					...
					VideoApiAdapter: (credential) => {
						return {
							...
							createMeeting: (...rest: any[]) => {
								if (creationCrash) {
									throw new Error(
										"MockVideoApiAdapter.createMeeting fake error",
									);
								}
								...
								return Promise.resolve({
									type: appStoreMetadata[
										metadataLookupKey as keyof typeof appStoreMetadata
									].type,
									...videoMeetingData,
								});
							},
							...
							updateMeeting: async (...rest: any[]) => {
								if (updationCrash) {
									throw new Error(
										"MockVideoApiAdapter.updateMeeting fake error",
									);
								}
								...
								log.silly(
									"MockVideoApiAdapter.updateMeeting",
							...
							deleteMeeting: async (...rest: any[]) => {
								log.silly(
									"MockVideoApiAdapter.deleteMeeting",
									JSON.stringify(rest),
								);
...
export function mockPaymentApp({
	...
	appStoreLookupKey = appStoreLookupKey || metadataLookupKey;
	const { paymentUid, externalId, MockPaymentService } =
		getMockPaymentService();
	// eslint-disable-next-line @typescript-eslint/ban-ts-comment
	//@ts-ignore
	appStoreMock.default[
		appStoreLookupKey as keyof typeof appStoreMock.default
...
export function mockCrmApp(
	...
	const appMock =
		appStoreMock.default[
			metadataLookupKey as keyof typeof appStoreMock.default
		];
	appMock &&
		`mockResolvedValue` in appMock &&
		appMock.mockResolvedValue({
				...
				CrmService: class {
					...
					getContacts(email: string) {
						if (crmData?.getContacts) {
							...
							const contactsOfEmail = contactsQueried.filter(
...
export function getMockedStripePaymentEvent({
	paymentIntentId,
...
export async function mockPaymentSuccessWebhookFromStripe({
	externalId,
}: { externalId: string }) {
...
export function getMockBookingReference(
	bookingReference: Partial<BookingReference> &
		Pick<BookingReference, "type" | "uid" | "credentialId">,
) {
...
export const getMockPassingAppStatus = ({
	slug,
	overrideName,
}: { slug: string; overrideName?: string }) => {
	return getMockAppStatus({ slug, overrideName, failures: 0, success: 1 });
};

export const replaceDates = (
	dates: string[],
