...
const IS_EMBED_TEST = process.argv.some((a) =>
	a.startsWith("--project=@sln/embed-core"),
);
const IS_EMBED_REACT_TEST = process.argv.some((a) =>
	a.startsWith("--project=@sln/embed-react"),
);

const webServer: PlaywrightTestConfig["webServer"] = [
	{
		command:
			"NEXT_PUBLIC_IS_E2E=1 NODE_OPTIONS='--dns-result-order=ipv4first' yarn workspace @sln/web start -p 3000",
		port: 3000,
		timeout: 60_000,
...
if (IS_EMBED_TEST) {
	...
	webServer.push({
		command: "yarn workspace @sln/embed-core dev",
		port: 3100,
		timeout: 60_000,
		reuseExistingServer: !process.env.CI,
	});
...
if (IS_EMBED_REACT_TEST) {
	...
	webServer.push({
		command: "yarn workspace @sln/embed-react dev",
		port: 3101,
		timeout: 60_000,
		reuseExistingServer: !process.env.CI,
	});
	...
	async toBeEmbedCalLink(
		...
		if (expectedPathname && expectedPathname !== pathname) {
			return {
				pass: false,
				message: () =>
					`Expected pathname to be ${expectedPathname} but got ${pathname}`,
			};
		}
		...
		if (expectedOrigin && expectedOrigin !== origin) {
			return {
				pass: false,
				message: () =>
					`Expected origin to be ${expectedOrigin} but got ${origin}`,
			};
		}
		...
		for (const [expectedKey, expectedValue] of Object.entries(
			...
			if (value !== expectedValue) {
				return {
					message: () =>
						`${expectedKey} should have value ${expectedValue} but got value ${value}`,
					pass: false,
				};
			}
		}
		...
		const { visibility: visibilityAfter, background: backgroundAfter } =
			await iframe.evaluate(() => {
				return {
...
function ensureAppServerIsReadyToServeEmbed(webServer: {
	port?: number;
	url?: string;
}) {
