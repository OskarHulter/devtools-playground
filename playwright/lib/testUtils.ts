...
export async function selectFirstAvailableTimeSlotNextMonth(
	page: Page | Frame,
) {
...
export const bookTimeSlot = async (
	page: Page,
	opts?: { name?: string; email?: string; title?: string },
) => {
	// --- fill form
...
export const createNewSeatedEventType = async (
	page: Page,
	args: { eventTitle: string },
) => {
	const eventTitle = args.eventTitle;
	...
	await page
		.locator('[data-testid="vertical-tab-event_advanced_tab_title"]')
		.click();
...
export async function gotoRoutingLink({
	...
	if (!formId) {
		...
		const href = await page
			.locator('[data-testid="form-action-preview"]')
			.getAttribute("href");
		if (!href) {
...
export async function getInviteLink(page: Page) {
	const response = await page.waitForResponse(
		"**/api/trpc/teams/createInvite?batch=1",
	);
...
export async function expectEmailsToHaveSubject({
	...
	if (!emails) return null;
	const emailsOrganizerReceived = await getEmailsReceivedByUser({
		emails,
		userEmail: organizer.email,
	});
	const emailsBookerReceived = await getEmailsReceivedByUser({
		emails,
...
export const createUserWithLimits = ({
	...
	if (!bookingLimits && !durationLimits) {
		throw new Error(
			"Need to supply at least one of bookingLimits or durationLimits",
		);
	}
...
export async function createUserWithSeatedEventAndAttendees(
	...
	const booking = await fixtures.bookings.create(
		user.id,
		user.username,
		eventType.id,
		{
