...
import { SchedulingType } from "@sln/prisma/client";
import type { Schedule, TimeRange } from "@sln/types/schedule";
...
testBothFutureAndLegacyRoutes.describe("free user", () => {
	...
	test("cannot book same slot multiple times", async ({
		...
		await page
			.locator("[data-testid=booking-fail]")
...
testBothFutureAndLegacyRoutes.describe("pro user", () => {
	...
	test("cannot book an unconfirmed event multiple times with the same email", async ({
		page,
		users,
	}) => {
		await page
