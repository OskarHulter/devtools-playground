...
test.describe("Signup Flow Test", async () => {
	...
	test("Premium Username Flow - creates stripe checkout", async ({
		...
		test.skip(!IS_PREMIUM_USERNAME_ENABLED, "Only run on sln.com");
	...
	test("Signup with valid (non premium) username", async ({
		page,
		users,
		features,
	}) => {
	...
	test("If signup is disabled allow team invites", async ({
		...
		await test.step("Invite User to team", async () => {
			...
			const inviteLink = await expectInvitationEmailToBeReceived(
				...
				emails,
				invitedUserEmail,
				`${team.name}'s admin invited you to join the team ${team.name} on sln.com`,
				"signup?token",
			);
