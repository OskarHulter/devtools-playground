...
import { intervalLimitKeyToUnit } from "@sln/lib/intervalLimit";
...
const getLastEventUrlWithMonth = (
	user: Awaited<ReturnType<typeof createUserWithLimits>>,
	date: Dayjs,
) => {
...
test.skip("Booking limits", () => {
	entries(BOOKING_LIMITS_SINGLE).forEach(([limitKey, bookingLimit]) => {
		...
		test(limitUnit, async ({ page, users }) => {
			...
			const monthUrl = getLastEventUrlWithMonth(
				user,
				firstMondayInBookingMonth,
			);
			...
			const bookingDay = availableDays.getByText(
				firstMondayInBookingMonth.date().toString(),
				{
					exact: true,
				},
			);
			...
			await test.step("but not over", async () => {
				...
				expect(
					availableDaysAfter && availableDaysAfter - availableDaysBefore,
				).toBe(expectedAvailableDays[limitUnit]);

				// try to book directly via form page
				await page.goto(slotUrl);
				await bookTimeSlot(page);

				await expect(page.getByTestId("booking-fail")).toBeVisible({
					timeout: 1000,
				});
			});
			...
			await test.step("but can reschedule", async () => {
				...
				const rescheduledBooking = await prisma.booking.findFirstOrThrow({
					where: { uid: bookingId },
				});

				const year = rescheduledBooking.startTime.getFullYear();
				const month = String(
					rescheduledBooking.startTime.getMonth() + 1,
				).padStart(2, "0");
				const day = String(rescheduledBooking.startTime.getDate()).padStart(
					2,
					"0",
				);
				...
				const formerDay = availableDays.getByText(
					rescheduledBooking.startTime.getDate().toString(),
					{
						exact: true,
					},
				);
				...
				const newBooking = await prisma.booking.findFirstOrThrow({
					where: { fromReschedule: bookingId },
				});
			...
			await test.step(`month after booking`, async () => {
				await page.goto(
					getLastEventUrlWithMonth(
						user,
						firstMondayInBookingMonth.add(1, "month"),
					),
				);
				...
				await expect((await availableDays.count()) === 0).toBe(
					limitUnit === "year",
				);
			});
		});
	...
	test("multiple", async ({ page, users }) => {
		...
		for (const [limitKey, limitValue] of entries(BOOKING_LIMITS_MULTIPLE)) {
			...
			await test.step(`month after booking`, async () => {
				await page.goto(
					getLastEventUrlWithMonth(user, bookingDate.add(1, "month")),
				);
