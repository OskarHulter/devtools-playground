...
test.describe("2FA Tests", async () => {
	...
	test("should allow a user to enable 2FA and login using 2FA", async ({
		page,
		users,
	}) => {
		// log in trail user
		const user = await test.step("Enable 2FA", async () => {
			...
			const secret = await page
				.locator(`[data-testid=two-factor-secret]`)
				.textContent();
			...
			await expect(
				page.locator('[data-testid="error-submitting-code"]'),
			).toBeVisible();
			...
			await expect(
				page.locator(`[data-testid=two-factor-switch]`).isChecked(),
			).toBeTruthy();
	...
	test("should allow a user to disable 2FA", async ({ page, users }) => {
		...
		const user = await test.step("Enable 2FA", async () => {
			...
			await page.goto("/settings/security/two-factor-auth");
			await page.click(
				`[data-testid=two-factor-switch][data-state="unchecked"]`,
			);
			await page.fill('input[name="password"]', userPassword);
			await page.press('input[name="password"]', "Enter");
			const secret = await page
			...
			expect(download.suggestedFilename()).toBe("sln-backup-codes.txt");
...
async function fillOtp({
	...
	secret,
	noRetry,
}: { page: Page; secret: string; noRetry?: boolean }) {
