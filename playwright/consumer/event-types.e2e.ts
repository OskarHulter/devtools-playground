...
import { randomString } from "@sln/lib/random";
...
testBothFutureAndLegacyRoutes.describe("Event Types A/B tests", () => {
	test("should render the /future/event-types page", async ({
		page,
		users,
	}) => {
...
testBothFutureAndLegacyRoutes.describe("Event Types tests", () => {
	test.describe("user", () => {
		...
		test("enabling recurring event comes with default options", async ({
			page,
		}) => {
			const nonce = randomString(3);
			...
			await expect(
				page.locator("[data-testid=recurring-event-collapsible]"),
			).toBeHidden();
			...
			expect(
				await page
					.locator(
						"[data-testid=recurring-event-collapsible] input[type=number]",
					)
					.nth(0)
					.getAttribute("value"),
			).toBe("1");
			expect(
				await page
					.locator(
						"[data-testid=recurring-event-collapsible] div[class$=singleValue]",
					)
					.textContent(),
			).toBe("week");
			expect(
		...
		test("can duplicate an existing event type", async ({ page }) => {
			...
			const [eventTypeId] = new URL(WEBAPP_URL + href).pathname
				.split("/")
				.reverse();
			const firstTitle = await page
				.locator(`[data-testid=event-type-title-${eventTypeId}]`)
				.innerText();
			const firstFullSlug = await page
				.locator(`[data-testid=event-type-slug-${eventTypeId}]`)
				.innerText();
		...
		test("can add multiple organizer address", async ({ page }) => {
			...
			await expect(page.locator(`[data-testid="where"]`)).toHaveText(
				locationData[0],
			);
		...
		test.describe("Different Locations Tests", () => {
			test("can add Attendee Phone Number location and book with it", async ({
				page,
			}) => {
			...
			test("Can add Organzer Phone Number location and book with it", async ({
				page,
			}) => {
				await gotoFirstEventType(page);
				...
				await page
					.locator(`input[name="${locationInputName}"]`)
					.fill("9199999999");
			...
			test("Can add Cal video location and book with it", async ({ page }) => {
				...
				await page.getByTestId("location-select").click();
				await page.locator(`text="SLN Video (Global)"`).click();
				...
				await expect(page.locator("[data-testid=success-page]")).toBeVisible();
				await expect(page.locator("[data-testid=where] ")).toContainText("SLN Video");
			});

			test("Can add Link Meeting as location and book with it", async ({
				page,
			}) => {
				await gotoFirstEventType(page);
			...
			test.skip("Can remove location from multiple locations that are saved", async ({
				page,
			}) => {
				await gotoFirstEventType(page);
				...
				await addAnotherLocation(page, "SLN Video (Global)");
			...
			test("can select 'display on booking page' option when multiple organizer input type are present", async ({
				...
				const testUrl1 = "https://sln.ai/";
				await page
					.locator(`input[name="${locationInputName(0)}"]`)
					.fill(testUrl1);
				...
				const organizerPhoneNumberInputName = (idx: number) =>
					`locations[${idx}].hostPhoneNumber`;

				const testPhoneInputValue1 = "9199999999";
				await page
					.locator(`input[name="${organizerPhoneNumberInputName(0)}"]`)
					.waitFor();
				await page
					.locator(`input[name="${organizerPhoneNumberInputName(0)}"]`)
					.fill(testPhoneInputValue1);
				await page.locator("[data-testid=display-location]").last().check();
				await checkDisplayLocation(page);
				await unCheckDisplayLocation(page);
				await page.locator("[data-testid=add-location]").click();
				...
				await page
					.locator(`input[name="${organizerPhoneNumberInputName(1)}"]`)
					.waitFor();
				await page
					.locator(`input[name="${organizerPhoneNumberInputName(1)}"]`)
					.fill(testPhoneInputValue2);
				await checkDisplayLocation(page);
				await unCheckDisplayLocation(page);
			});
		});
...
const fillLocation = async (
	...
	await page.locator(`input[name="${locationInputName}"]`).waitFor();
	await page
		.locator(`input[name="locations[${index}].address"]`)
		.fill(inputText);
...
const checkDisplayLocation = async (page: Page) => {
	await page.locator("[data-testid=display-location]").last().check();
	await expect(
		page.locator("[data-testid=display-location]").last(),
	).toBeChecked();
};
