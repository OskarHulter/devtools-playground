...
import { EMBED_LIB_URL, WEBAPP_URL } from "@sln/lib/constants";
...
test.describe("Embed Code Generator Tests", () => {
	test.describe("Non-Organization", () => {
		...
		test.describe("Event Types Page", () => {
			...
			test("open Embed Dialog and choose Inline for First Event Type", async ({
				page,
				users,
			}) => {
				const [pro] = users.get();
			...
			test("open Embed Dialog and choose floating-popup for First Event Type", async ({
				page,
				users,
			}) => {
				const [pro] = users.get();
			...
			test("open Embed Dialog and choose element-click for First Event Type", async ({
				page,
				users,
			}) => {
				const [pro] = users.get();
...
async function expectValidReactEmbedSnippet(
	...
	const embedCode = await page
		.locator("[data-testid=embed-react]")
		.inputValue();
...
async function expectToContainValidPreviewIframe(
	page: Page,
	{
		...
		calLink,
		bookerUrl,
	}: { embedType: EmbedType; calLink: string; bookerUrl?: string },
) {
	bookerUrl = bookerUrl || `${WEBAPP_URL}`;
	expect(
		await page.locator("[data-testid=embed-preview]").getAttribute("src"),
	).toContain(
