...
test.describe("BOOKING_RESCHEDULED", async () => {
	test("can reschedule a booking and get a booking rescheduled event", async ({
		...
		await page.goto(
			`/${user.username}/${eventType.slug}?rescheduleUid=${booking.uid}`,
		);
		...
		const newBooking = await prisma.booking.findFirst({
			where: { fromReschedule: booking?.uid },
		})!;
	...
	test("when rescheduling to a booking that already exists, should send a booking rescheduled event with the existant booking uid", async ({
		...
		const { user, eventType, booking } =
			await createUserWithSeatedEventAndAttendees({ users, bookings }, [
				{
					name: "John First",
					email: "first+seats@sln.com",
					timeZone: "Europe/Berlin",
				},
				{
					name: "Jane Second",
					email: "second+seats@sln.com",
					timeZone: "Europe/Berlin",
				},
			]);
...
test.describe("MEETING_ENDED, MEETING_STARTED", async () => {
	test("should create/remove scheduledWebhookTriggers for existing bookings", async ({
		...
		await Promise.all([
			...
			page.waitForURL((url) =>
				url.pathname.endsWith("/settings/developer/webhooks"),
			),
		]);
		...
		expect(meetingEndedTriggers.map((trigger) => trigger.startAfter)).toEqual(
			expect.arrayContaining(
				existingUserBookings.map((booking) => booking.endTime),
			),
		);
		...
		const scheduledTriggersAfterRemovingTrigger =
			await prisma.webhookScheduledTriggers.findMany({
				where: {
					webhook: {
						userId: user.id,
					},
		...
		const newMeetingStartedTriggers =
			scheduledTriggersAfterRemovingTrigger.filter((trigger) =>
				trigger.payload.includes("MEETING_STARTED"),
			);
		const newMeetingEndedTriggers =
			scheduledTriggersAfterRemovingTrigger.filter((trigger) =>
				trigger.payload.includes("MEETING_ENDED"),
			);
		...
		const scheduledTriggersAfterDisabling =
			await prisma.webhookScheduledTriggers.findMany({
				where: {
					webhook: {
						userId: user.id,
...
test.describe("FORM_SUBMITTED", async () => {
	test("on submitting user form, triggers user webhook", async ({
		page,
		users,
		routingForms,
		webhooks,
	...
	test("on submitting team form, triggers team webhook", async ({
		page,
		users,
		routingForms,
		webhooks,
