...
const addOauthBasedIntegration = async ({
	page,
	slug,
	authorization,
	token,
...
const addLocationIntegrationToFirstEvent = async ({
	user,
...
async function bookEvent(page: Page, calLink: string) {
	...
	expect(
		global.E2E_EMAILS?.every((email) =>
			(email.text as string).includes(`https://zoom.us/j/${meetingId}`),
		),
	).toBe(true);
	return bookingId;
}
...
test.fixme("Integrations", () => {
	...
	const addZoomIntegration = async ({ page }: { page: Page }) => {
	...
	test.describe("Zoom App", () => {
		...
		test("can choose zoom as a location during booking", async ({
			page,
			users,
		}) => {
			const user = await users.create();
       ...
       */
		...
		test("Can disconnect from integration", async ({ page, users }) => {
			...
			await page
				.locator('[data-testid="zoom_video-integration-disconnect-button"]')
				.click();
			await page.locator('[data-testid="confirm-button"]').click();
			await expect(
				page.locator('[data-testid="confirm-integration-disconnect-button"]'),
			).toHaveCount(0);
		});
	});
