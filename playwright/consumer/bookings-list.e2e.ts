...
import { BookingStatus } from "@sln/prisma/client";
	...
	test.describe("Upcoming bookings", () => {
		test("show attendee bookings and organizer bookings in asc order by startDate", async ({
			...
			const bookingWhereFirstUserIsOrganizerFixture = await createBooking({
				...
				attendees: [
					{ name: "First", email: "first@sln.com", timeZone: "Europe/Berlin" },
					{
						name: "Second",
						email: "second@sln.com",
						timeZone: "Europe/Berlin",
					},
					{ name: "Third", email: "third@sln.com", timeZone: "Europe/Berlin" },
				],
			});
			const bookingWhereFirstUserIsOrganizer =
			...
			const bookingWhereFirstUserIsAttendee =
				await bookingWhereFirstUserIsAttendeeFixture.self();

			await firstUser.apiLogin();
			await page.goto(`/bookings/upcoming`);
			const upcomingBookings = page.locator(
				'[data-testid="upcoming-bookings"]',
			);
			const firstUpcomingBooking = upcomingBookings
				.locator('[data-testid="booking-item"]')
				.nth(0);
			const secondUpcomingBooking = upcomingBookings
				.locator('[data-testid="booking-item"]')
				.nth(1);
			await expect(
				// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
				firstUpcomingBooking.locator(
					`text=${bookingWhereFirstUserIsAttendee!.title}`,
				),
			).toBeVisible();
			await expect(
				// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
				secondUpcomingBooking.locator(
					`text=${bookingWhereFirstUserIsOrganizer!.title}`,
				),
			).toBeVisible();
		});
	});
	test.describe("Past bookings", () => {
		test("Mark first guest as no-show", async ({
			page,
			users,
			bookings,
			webhooks,
			...
			const firstPastBooking = pastBookings
				.locator('[data-testid="booking-item"]')
				.nth(0);
			const titleAndAttendees = firstPastBooking.locator(
				'[data-testid="title-and-attendees"]',
			);
			const firstGuest = firstPastBooking
				.locator('[data-testid="guest"]')
				.nth(0);
			await firstGuest.click();
			await expect(
				titleAndAttendees.locator('[data-testid="unmark-no-show"]'),
			).toBeHidden();
			await expect(
				titleAndAttendees.locator('[data-testid="mark-no-show"]'),
			).toBeVisible();
			...
			expect(body).toMatchObject({
				...
				payload: {
					message: "first@sln.com marked as no-show",
					attendees: [
						{ email: "first@sln.com", noShow: true, utcOffset: null },
					],
		...
		test("Mark 3rd attendee as no-show", async ({ page, users, bookings }) => {
			...
			const moreGuests = firstPastBooking.locator(
			...
			const updateNoShow = firstPastBooking.locator(
...
async function createBooking({
	...
	const endTime = new Date(
		Date.now() + relativeDate * DAY_MS + bookingDurationMs,
	);
	return await bookingsFixture.create(
		organizer.id,
		organizer.username,
		organizerEventType.id,
		{
			title,
