...
test.describe("Manage Booking Questions", () => {
	...
	test.describe("For User EventType", () => {
		test("Do a booking with a Address type question and verify a few thing in b/w", async ({
			...
			await runTestStepsCommonForTeamAndUserEventType(
				page,
				context,
				webhookReceiver,
			);
		...
		test("Split 'Full name' into 'First name' and 'Last name'", async ({
			...
			await test.step("Open the 'Name' field dialog", async () => {
				...
				await page
					.locator(
						'[data-testid="field-name"] [data-testid="edit-field-action"]',
					)
					.click();
			});
			...
			await test.step("Book a time slot with firstName and lastName provided separately", async () => {
				await doOnFreshPreview(page, context, async (page) => {
					...
					expect(
						await page
							.locator('[data-testid="attendee-name-John Doe"]')
							.nth(0)
							.textContent(),
					).toBe("John Doe");
			...
			await test.step("Verify that we can prefill name field with no lastname", async () => {
				...
				await doOnFreshPreviewWithSearchParams(
					searchParams,
					page,
					context,
					async (page) => {
		...
		test("Do a booking with a user added question and verify a few thing in b/w", async ({
			...
			await test.step("Go to First Team Event", async () => {
				const $eventTypes = page
					.locator("[data-testid=event-types]")
					.nth(1)
					.locator("li a");
				const firstEventTypeElement = $eventTypes.first();
...
async function runTestStepsCommonForTeamAndUserEventType(
	...
	await test.step("Add Question and see that it's shown on Booking Page at appropriate position", async () => {
		...
		await doOnFreshPreview(page, context, async (page) => {
			const allFieldsLocator = await expectSystemFieldsToBeThereOnBookingPage({
				page,
			});
			const userFieldLocator = allFieldsLocator.nth(5);

			await expect(
				userFieldLocator.locator('[name="how-are-you"]'),
			).toBeVisible();
	...
	await test.step("Hide Question and see that it's not shown on Booking Page", async () => {
		...
		await doOnFreshPreview(page, context, async (page) => {
			const formBuilderFieldLocator = page.locator(
				'[data-fob-field-name="how-are-you"]',
			);
		...
		await test.step("Do a booking and notice that we can book without giving a value for rescheduleReason", async () => {
			return await doOnFreshPreview(
				...
				async (page) => {
					...
					expect(
						await formBuilderFieldLocator
							.locator('[name="how-are-you"]')
							.getAttribute("placeholder"),
					).toBe("I'm fine, thanks");
					expect(await getLabelText(formBuilderFieldLocator)).toBe(
						"How are you?",
					);
					await formBuilderFieldLocator
						.locator('[name="how-are-you"]')
					...
					expect(
						await page
							.locator(
								'[data-testid="field-response"][data-fob-field="how-are-you"]',
							)
							.innerText(),
					).toBe("I am great!");
...
async function expectSystemFieldsToBeThereOnBookingPage({
	...
	if (isFirstAndLastNameVariant) {
		if (values?.name) {
			await expect(nameLocator.locator('[name="firstName"]')).toHaveValue(
				values?.name?.firstName || "",
			);
			await expect(nameLocator.locator('[name="lastName"]')).toHaveValue(
				values?.name?.lastName || "",
			);
			expect(
				await nameLocator.locator(".testid-firstName > label").innerText(),
			).toContain("*");
		} else {
		...
		if (values?.name) {
			await expect(nameLocator.locator('[name="name"]')).toHaveValue(
	...
	if (values?.email) {
		await expect(emailLocator.locator('[name="email"]')).toHaveValue(
			values?.email || "",
		);
	} else {
	...
	if (values?.notes) {
		await expect(additionalNotes.locator('[name="notes"]')).toHaveValue(
			values?.notes,
		);
	} else {
	...
	if (values?.guests) {
		...
		for (let i = 0; i < values.guests.length; i++) {
			await expect(allGuestsLocators.nth(i)).toHaveValue(
				values.guests[i] || "",
			);
		}
		await expect(
			guestsLocator.locator("[data-testid='add-another-guest']"),
		).toBeVisible();
	} else {
		await expect(
			guestsLocator.locator("[data-testid='add-guests']"),
		).toBeVisible();
	}
...
async function expectErrorToBeThereFor({
	page,
	name,
}: { page: Page; name: string }) {
	await expect(page.locator(`[data-testid=error-message-${name}]`)).toHaveCount(
		1,
	);
...
async function toggleQuestionAndSave({
	name,
	page,
}: { name: string; page: Page }) {
	await page
		.locator(`[data-testid="field-${name}"]`)
		.locator('[data-testid="toggle-field"]')
		.click();
	await saveEventType(page);
}
...
async function toggleQuestionRequireStatusAndSave({
	...
	await page
		.locator(`[data-testid="field-${name}"]`)
		.locator('[data-testid="edit-field-action"]')
		.click();
	await page
