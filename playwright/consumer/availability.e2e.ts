...
test.describe("Availablity", () => {
	...
	test("Date Overrides", async ({ page }) => {
		...
		await expect(
			page.locator('[data-testid="date-overrides-list"] > li'),
		).toHaveCount(1);
		await page.locator('[form="availability-form"][type="submit"]').click();
		const response = await page.waitForResponse(
			"**/api/trpc/availability/schedule.update?batch=1",
		);
		...
		await expect(
			page.locator('[data-testid="troubleshooter-busy-time"]'),
		).toHaveCount(1);
	});
	...
	test("it can delete date overrides", async ({ page }) => {
		...
		const dateOverrideList = page.locator(
			'[data-testid="date-overrides-list"] > li',
		);
		...
		const deleteButtonTitle = (await deleteButton.getAttribute(
			"title",
		)) as string;
		...
		await expect(await page.getByTitle(deleteButtonTitle).isVisible()).toBe(
			false,
		);
	});

	test("Can create date override on current day in a negative timezone", async ({
		page,
	}) => {
		await page.getByTestId("schedules").first().click();
	...
	test("Schedule listing", async ({ page }) => {
		await test.step("Can add a new schedule", async () => {
			...
			await expect(page.getByTestId("availablity-title")).toHaveValue(
				"More working hours",
			);
		});
		await test.step("Can delete a schedule", async () => {
			...
			await page
				.locator('[data-testid="schedules"] > li')
				.nth(1)
				.getByTestId("schedule-more")
				.click();
			...
			await expect(
				page.locator('[data-testid="schedules"] > li').nth(1),
			).toHaveCount(0);
		});
		await test.step("Cannot delete the last schedule", async () => {
			...
			await expect(
				page.locator('[data-testid="schedules"] > li').nth(0),
			).toHaveCount(1);
		});
	});
	...
	test("Can manage single schedule", async ({ page }) => {
		...
		await page
			...
			.click();
		await expect(
			page.locator("div").filter({ hasText: "6:00pm" }).nth(1),
		).toBeVisible();
		await page.getByRole("button", { name: save }).click();
		await expect(
			page.getByText("Sun - Tue, Thu - Sat, 9:00 AM - 5:00 PM"),
		).toBeVisible();
		...
		await page.getByTestId("dialog-rejection").click();
		await page
			.getByTestId("date-overrides-list")
			.getByRole("button")
			.nth(1)
			.click();
		await page.getByRole("button", { name: save }).click();
