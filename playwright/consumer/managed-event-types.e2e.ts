	...
	test("Can create managed event type", async ({ page, users }) => {
		...
		await test.step("Managed event option exists for team admin", async () => {
			...
			await page
				...
				.fill(`${adminUser.username}'s Team`);
			...
			await page
				.getByTestId("pending-member-list")
		...
		await test.step("Managed event type exists for added member", async () => {
			...
			await page.goto("/event-types");
			await page
				.getByTestId("event-types")
				.locator('a[title="managed"]')
				.click();
		...
		await test.step("Managed event type can use Organizer's default app as location", async () => {
			...
			const optionText = (await localize("en"))(
		...
		await test.step("Managed event type provides discrete field lock/unlock state for admin", async () => {
			...
			await expect(
				titleLockIndicator.locator("[data-state='checked']"),
			).toHaveCount(1);
			...
			await expect(
				titleLockIndicator.locator("[data-state='unchecked']"),
			).toHaveCount(1);
		...
		await test.step("Managed event type should only update the unlocked fields modified by Admin", async () => {
			...
			await page
				.getByTestId("event-types")
				.locator('a[title="Managed Event Title"]')
				.click();
			await page.waitForURL("event-types/**");

			//match length
			expect(
				await page.locator("[data-testid=duration]").getAttribute("value"),
			).toBe("45");
			//ensure description didn't update
			expect(
				await page.locator(`input[name="title"]`).getAttribute("value"),
			).toBe(`Managed Event Title`);
