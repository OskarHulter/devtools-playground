...
test.describe("Stripe integration skip true", () => {
	...
	test("when enabling Stripe, credentialId is included", async ({
		page,
		users,
	}) => {
		const user = await users.create();
		...
		const eventType = user.eventTypes.find(
			(e) => e.slug === "paid",
		) as Prisma.EventType;
	...
	test("when enabling Stripe, team credentialId is included", async ({
		page,
		users,
	}) => {
	...
	test("Pending payment booking should not be confirmed by default", async ({
		page,
		users,
	}) => {
		const user = await users.create();
	...
	test("Paid booking should be able to be rescheduled", async ({
		page,
		users,
	}) => {
		const user = await users.create();
		...
		await Promise.all([
			...
			page.click('[data-testid="reschedule-link"]'),
		]);
	...
	test("Paid booking should be able to be cancelled", async ({
		page,
		users,
	}) => {
		const user = await users.create();
		...
		await expect(
			await page.locator('[data-testid="cancelled-headline"]').first(),
		).toBeVisible();
	});
	...
	test.describe("When event is paid and confirmed", () => {
		...
		test.beforeEach(async ({ page, users }) => {
			...
			eventType = user.eventTypes.find(
				(e) => e.slug === "paid",
			) as Prisma.EventType;
			await user.apiLogin();
			await page.goto("/apps/installed");
		...
		test("Payment should confirm pending payment booking", async ({
			page,
			users,
		}) => {
			await page.goto("/bookings/upcoming");
		...
		test("Paid and confirmed booking should be able to be rescheduled", async ({
			page,
			users,
		}) => {
			await Promise.all([
...
test.describe("Stripe integration with the new app install flow skip flase", () => {
	...
	test("when enabling Stripe, credentialId is included skip false", async ({
		page,
		users,
	}) => {
		const user = await users.create();
		...
		for (const eventTypeMetadata of eventTypeMetadatas) {
			const metadata = EventTypeMetaDataSchema.parse(
	...
	test("when enabling Stripe, team credentialId is included skip false", async ({
		page,
		users,
	}) => {
		const ownerObj = { username: "pro-user", name: "pro-user" };
		...
		await owner.installStripeTeam({
			skip: false,
			teamId: team.id,
			eventTypeIds: [teamEvent.id],
		});
	...
	test("Can book a paid booking skip false", async ({ page, users }) => {
		...
		await user.installStripePersonal({
			skip: false,
	...
	test("Pending payment booking should not be confirmed by default skip false", async ({
		page,
		users,
	}) => {
		const user = await users.create();
	...
	test("Paid booking should be able to be rescheduled skip false", async ({
		page,
		users,
	}) => {
		const user = await users.create();
	...
	test("Paid booking should be able to be cancelled skip false", async ({
		page,
		users,
	}) => {
		const user = await users.create();
	...
	test.describe("When event is paid and confirmed skip false", () => {
		...
		test("Payment should confirm pending payment booking skip false", async ({
			page,
			users,
		}) => {
			await page.goto("/bookings/upcoming");
	...
	test.describe("Change stripe presented currency skip false", () => {
		test("Should be able to change currency skip false", async ({
			page,
			users,
		}) => {
			const user = await users.create();
