...
test("dynamic booking info prefilled by query params", async ({
	page,
	users,
}) => {
	const pro = await users.create();
	...
	const badgeByDurationTestId = (duration: number) =>
		`multiple-choice-${duration}mins`;
...
test.skip("it contains the right event details", async ({ page }) => {
	const response = await page.goto(`http://acme.sln.local:3000/owner1+member1`);
	expect(response?.status()).toBe(200);

	expect(await page.locator('[data-testid="event-title"]').textContent()).toBe(
		"Group Meeting",
	);
	expect(
...
test.describe("Organization:", () => {
	...
	test("Can book a time slot for an organization", async ({
		page,
		users,
		orgs,
	}) => {
		...
		await doOnOrgDomain(
			...
			async () => {
				...
				await expect(
					page.getByText(user1.name!, { exact: true }),
				).toBeVisible();
				// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
				await expect(
					page.getByText(user2.name!, { exact: true }),
				).toBeVisible();
			},
		);
	});
