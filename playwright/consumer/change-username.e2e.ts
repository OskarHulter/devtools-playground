...
const IS_SELF_HOSTED = !(
	new URL(WEBAPP_URL).hostname.endsWith(".sln.dev") ||
	!!new URL(WEBAPP_URL).hostname.endsWith(".sln.com")
);
...
test.describe("Change username on settings", () => {
	...
	test("User can change username to include periods(or dots)", async ({
		page,
		users,
		prisma,
	}) => {
	...
	test("User can update to PREMIUM username", async ({
		page,
		users,
	}, testInfo) => {
		...
		const currentUsernameText = page
			.locator("[data-testid=current-username]")
			.innerText();
		const newUsernameText = page
			.locator("[data-testid=new-username]")
			.innerText();
	...
	test("User can't take a username that has been migrated to a different username in an organization", async ({
		...
		await test.step("Changing username for another user to the previous username of migrated user - shouldn't be allowed", async () => {
			...
			await page.waitForLoadState("networkidle");
			await expect(
				page.locator("[data-testid=update-username-btn]").nth(0),
			).toBeHidden();
			await expect(
				page.locator("[data-testid=update-username-btn]").nth(1),
			).toBeHidden();
		});
	});
});
