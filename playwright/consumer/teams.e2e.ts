...
testBothFutureAndLegacyRoutes.describe("Teams - NonOrg", (routeVariant) => {
	...
	test("Team Onboarding Invite Members", async ({ page, users }) => {
		...
		await test.step("Can add members", async () => {
			...
			await expect(
				page.locator(`li:has-text("${inviteeEmail}")`),
			).toBeVisible();
			expect(
				await page.locator('[data-testid="pending-member-item"]').count(),
			).toBe(2);
		});
		...
		await test.step("Can disband team", async () => {
			...
			await expect(
				await page.locator(`text=${user.username}'s Team`).count(),
			).toEqual(0);
	...
	test("Can create a booking for Collective EventType", async ({
		page,
		users,
	}) => {
		const teamMatesObj = [
		...
		const { title: teamEventTitle, slug: teamEventSlug } =
		...
		await expect(page.locator("[data-testid=booking-title]")).toHaveText(
			BookingTitle,
		);
		...
		for (const teammate of teamMatesObj) {
			await expect(
				page.getByText(teammate.name, { exact: true }),
			).toBeVisible();
		}
	...
	test("Can create a booking for Round Robin EventType", async ({
		page,
		users,
	}) => {
		const teamMatesObj = [
		...
		const chosenUser = await page
			.getByTestId("booking-host-name")
			.textContent();
		expect(chosenUser).not.toBeNull();
		expect(
			teamMatesObj
				.concat([{ name: owner.name! }])
				.some(({ name }) => name === chosenUser),
		).toBe(true);
		// TODO: Assert whether the user received an email
	});

	test("Non admin team members cannot create team in org", async ({
		page,
		users,
	}) => {
		test.skip(routeVariant === "future", "Future route not ready yet");
		...
		if (memberUser) {
			...
			await expect(
				page.locator("[data-testid=create-team-btn]"),
	...
	test("Can create team with same name as user", async ({ page, users }) => {
		...
		await test.step("Can access user and team with same slug", async () => {
			...
			await expect(page.locator("[data-testid=team-name]")).toHaveText(
				uniqueName,
			);
			...
			await expect(page.locator("[data-testid=name-title]")).toHaveText(
				uniqueName,
			);
	...
	test("Can create a private team", async ({ page, users }) => {
		...
		await Promise.all([
			...
			expect(
				page.locator(
					`[data-testid=make-team-private-check][data-state="checked"]`,
				),
			).toBeVisible(),
		...
		await expect(
			page.locator('[data-testid="book-a-team-member-btn"]'),
		).toBeHidden();

		// Go to members page
		await page.goto(`/team/${team.slug}?members=1`);
		await expect(
			page.locator('[data-testid="you-cannot-see-team-members"]'),
		).toBeVisible();
		await expect(
			page.locator('[data-testid="team-members-container"]'),
		).toBeHidden();
	});
	test("Email Embeds slots are loading for team event types", async ({
		page,
		users,
	}) => {
		const teamMatesObj = [
		...
		// Check Team Url
		const availableTimesUrl = await page
			.getByTestId("see_all_available_times")
			.getAttribute("href");
		await expect(availableTimesUrl).toContain(
			`/team/${team.slug}/${teamEventSlug}`,
		);
	});
