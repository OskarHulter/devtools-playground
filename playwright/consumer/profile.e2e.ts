...
test.describe("Teams", () => {
	test("Profile page is loaded for users in Organization", async ({
		page,
		users,
	}) => {
...
test.describe("Update Profile", () => {
	...
	test.skip("Can update a users email (verification enabled)", async ({
		page,
		users,
		prisma,
		features,
		...
		expect(await page.getByTestId("toast-success").textContent()).toContain(
			email,
		);
	...
	test("Can update a users email (verification disabled)", async ({
		page,
		users,
		prisma,
		features,
	...
	const testEmailVerificationLink = async ({
		...
		await test.step("the user receives the correct invitation link", async () => {
			...
			expect(inviteLink).toEqual(
				`${WEBAPP_URL}/api/auth/verify-email?token=${verificationToken?.token}`,
			);
		});
	};

	test("Can add a new email as a secondary email", async ({
		page,
		users,
		prisma,
		emails,
		...
		const secondaryEmailAddDialog = await page.waitForSelector(
		...
		await test.step("the user receives the correct invitation link", async () => {
			...
			expect(
				inviteLink?.endsWith(
					`/api/auth/verify-email?token=${verificationToken?.token}`,
				),
			).toEqual(true);
		});
	...
	test("Newly added secondary email should show as Unverified", async ({
		page,
		users,
	}) => {
		...
		expect(
			await page.getByTestId("profile-form-email-0-primary-badge").isVisible(),
		).toEqual(true);
		expect(
			await page
				.getByTestId("profile-form-email-0-unverified-badge")
				.isVisible(),
		).toEqual(false);

		expect(
			await page.getByTestId("profile-form-email-1-primary-badge").isVisible(),
		).toEqual(false);
		expect(
			await page
				.getByTestId("profile-form-email-1-unverified-badge")
				.isVisible(),
		).toEqual(true);
	});
	...
	// TODO: This test is extremely flaky and has been failing a lot, blocking many PRs. Fix this.
	// eslint-disable-next-line playwright/no-skipped-test
	test.skip("Can verify the newly added secondary email", async ({
		page,
		users,
		prisma,
	}) => {
	...
	test("Can delete the newly added secondary email", async ({
		page,
		users,
	}) => {
		await createSecondaryEmail({ page, users });
		...
		await page
			.getByTestId("secondary-email-action-group-button")
			.nth(1)
			.click();
		await page.getByTestId("secondary-email-delete-button").click();

		expect(await page.getByTestId("profile-form-email-1").isVisible()).toEqual(
			false,
		);
	});
	...
	test.skip("Can resend verification link if the secondary email is unverified", async ({
		...
		expect(
			await page
				.locator("button[data-testid=resend-verify-email-button]")
				.isDisabled(),
		).toEqual(false);
