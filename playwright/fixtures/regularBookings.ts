...
export async function loginUserWithTeam(
	users: UserFixture,
	role: MembershipRole,
) {
...
export function createBookingPageFixture(page: Page) {
	return {
		...
		checkRequiresConfirmation: async () => {
			...
			await expect(
				page.getByTestId("requires-confirmation-title").locator("svg"),
			).toBeVisible();
		...
		checkRequiresBookerEmailVerification: async () => {
			await expect(
				page
					.getByTestId("requires-booker-email-verification-title")
					.locator("svg"),
			).toBeVisible();

			const emailSwitch = page.getByTestId(
		...
		checkHideNotes: async () => {
			await expect(
				page.getByTestId("disable-notes-title").locator("svg"),
			).toBeVisible();
		...
		checkRedirectOnBooking: async () => {
			await expect(
				page.getByTestId("redirect-success-booking-title").locator("svg"),
			).toBeVisible();
			...
			await page.getByTestId("external-redirect-url").fill("https://sln.com");
			await expect(page.getByTestId("redirect-url-warning")).toBeVisible();
		},
		checkEnablePrivateUrl: async () => {
			await expect(
				page.getByTestId("hashedLinkCheck-title").locator("label div"),
			).toBeVisible();
		...
		toggleOfferSeats: async () => {
			await expect(
				page.getByTestId("offer-seats-toggle-title").locator("svg"),
			).toBeVisible();
		...
		checkLockTimezone: async () => {
			await expect(
				page.getByTestId("lock-timezone-toggle-title").locator("svg"),
			).toBeVisible();
		...
		checkEventType: async () => {
			await expect(
				page.getByTestId("requires-confirmation-badge").last(),
			).toBeVisible();
		},
		checkBufferTime: async () => {
			...
			await expect(
				page.getByText(`15 ${minutes}`, { exact: true }),
			).toBeVisible();
			...
			await expect(
				page.getByText(`10 ${minutes}`, { exact: true }),
			).toBeVisible();
			...
			await expect(
				page.getByText(`60 ${minutes}`, { exact: true }),
			).toBeVisible();
		},
		...
		checkLimitBookingDuration: async () => {
			const limitDuration = (await localize("en"))(
		...
		checkLimitFutureBookings: async () => {
			const limitFutureBookings = (await localize("en"))(
		...
		checkOffsetTimes: async () => {
			...
			await page
				.locator("fieldset")
				.filter({ hasText: offsetStartTimes })
				.getByRole("switch")
				.click();
			...
			await expect(
				page.getByText(
					"e.g. this will show time slots to your bookers at 9:10 AM instead of 9:00 AM",
				),
			).toBeVisible();
		},
