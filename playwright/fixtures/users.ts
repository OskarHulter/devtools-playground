...
import stripe from "@sln/features/ee/payments/server/stripe";
import {
	DEFAULT_SCHEDULE,
	getAvailabilityFromSchedule,
} from "@sln/lib/availability";
...
import type { Schedule } from "@sln/types/schedule";
...
type InstallStripeParamsUnion =
	| InstallStripeParamsSkipTrue
	| InstallStripeParamsSkipFalse;
		...
		data: {
			...
			hosts: {
				create: {
					userId: user.id,
					isFixed:
						scenario?.schedulingType === SchedulingType.COLLECTIVE
							? true
							: false,
				},
			},
...
export const createUsersFixture = (
	...
	return {
		buildForSignup: (
			opts?: Pick<
				CustomUserOpts,
				"email" | "username" | "useExactUsername" | "password"
			>,
		) => {
			const uname =
		...
		create: async (
			...
			if (opts?.eventTypes)
				defaultEventTypes = defaultEventTypes.concat(opts.eventTypes);
							...
							await prisma.team.update({
								...
								data: {
									orgProfiles: _user.profiles.length
										? {
												connect: _user.profiles.map((profile) => ({
													id: profile.id,
												})),
											}
										: {
												create: orgProfilesCreate.filter(
													(profile) =>
														!existingProfiles
															.map((p) => p.userId)
															.includes(profile.user.connect.id),
												),
											},
								},
							});
		...
		deleteAll: async () => {
			...
			if (emails) {
				...
				for (const user of store.trackedEmails.concat(
					store.users.map((u) => ({ email: u.email })),
				)) {
			...
			await prisma.user.deleteMany({
				where: { email: { in: store.trackedEmails.map((e) => e.email) } },
			});
			await prisma.team.deleteMany({
				where: { id: { in: store.teams.map((org) => org.id) } },
			});
...
const createUserFixture = (user: UserWithIncludes, page: Page) => {
	...
	return {
		...
		apiLogin: async (password?: string) =>
			apiLogin(
				{ ...(await self()), password: password || user.username },
				store.page,
			),
		/**
		 ...
		 */
		login: async () =>
			login({ ...(await self()), password: user.username }, store.page),
		logout: async () => {
			await page.goto("/auth/logout");
		},
		...
		setupEventWithPrice: async (
			eventType: Pick<Prisma.EventType, "id">,
			slug: string,
		...
		installStripeTeam: async (
			params: InstallStripeParamsUnion & { teamId: number },
		...
		delete: async () =>
			await prisma.user.delete({ where: { id: store.user.id } }),
			...
			opts?.completedOnboarding ?? true
				? {
						create: {
							...
							availability: {
								createMany: {
									data: getAvailabilityFromSchedule(
...
async function confirmPendingPayment(page: Page) {
	...
	if (response.status() !== 200)
		throw new Error(`Failed to confirm payment. Response: ${response.text()}`);
}
...
export async function setupEventWithPrice(
	eventType: Pick<Prisma.EventType, "id">,
	slug: string,
	page: Page,
) {
...
export async function makePaymentUsingStripe(page: Page) {
	...
	await stripeFrame
		.locator('[name="expiry"]')
		.fill(`${now.getMonth() + 1} / ${now.getFullYear() + 1}`);
	await stripeFrame.locator('[name="cvc"]').fill("111");
	const postcalCodeIsVisible = await stripeFrame
...
const installStripeTeam = async ({
	teamId,
	...params
}: InstallStripeTeamPramas) => {
...
const installStripe = async ({
	...
	for (let index = 0; index < eventTypeIds.length; index++) {
		await page
			.locator('[data-testid="stripe-price-input"]')
			.nth(index)
			.fill(`1${index}`);
	}
	...
	for (let index = 0; index < eventTypeIds.length; index++) {
		...
		await expect(page.getByTestId(`stripe-price-input`)).toHaveValue(
			`1${index}`,
		);
	}
};
