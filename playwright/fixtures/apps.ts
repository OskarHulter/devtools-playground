...
import { shouldRedirectToAppOnboarding } from "@sln/lib/apps/shouldRedirectToAppOnboarding";
...
export function createAppsFixture(page: Page) {
	return {
		...
		installAppSkipConfigure: async (app: string) => {
			...
			const appMetadata =
				appStoreMetadata[app as keyof typeof appStoreMetadata];
		...
		verifyAppsInfo: async (activeApps: number) => {
			await expect(
				page.locator(`text=6 apps, ${activeApps} active`),
			).toBeVisible();
		},
		verifyAppsInfoNew: async (apps: string[], eventTypeId: number) => {
			...
			for (const app of apps) {
				await expect(
					page.locator(
						`[data-testid='${app}-app-switch'][data-state="checked"]`,
					),
				).toBeVisible();
			}
		},
