...
const dayjs = (...args: Parameters<typeof _dayjs>) =>
	_dayjs(...args).tz("Europe/London");

// creates a user fixture instance and stores the collection
export const createBookingsFixture = (page: Page) => {
	const store = { bookings: [], page } as {
	...
	return {
		...
		update: async (args: Prisma.BookingUpdateArgs) =>
			await prisma.booking.update(args),
		get: () => store.bookings,
...
const createBookingFixture = (booking: Booking, page: Page) => {
	...
	return {
		...
		delete: async () =>
			await prisma.booking.delete({ where: { id: store.booking.id } }),
	};
};
