...
import { CAL_URL } from "@sln/lib/constants";
...
vi.mock("@sln/lib/constants", () => {
	return {
		SEO_IMG_DEFAULT: "",
		SEO_IMG_OGIMG: "",
		APP_NAME: "stresslessness.org",
		CAL_URL: "http://stresslessness.org",
	};
});
vi.mock("next-seo", () => {
	return {
		NextSeo: (props: NextSeoProps) => {
			const { images, ...restOpenGraph } = props.openGraph as OpenGraph & {
...
describe("Tests for HeadSeo component", () => {
	test("Should render mocked NextSeo", async () => {
		...
		await waitFor(async () => {
			...
			expect(mockedNextSeoEl?.getAttribute("canonical")).toEqual(
				basicProps.canonical,
			);
			expect(mockedNextSeoEl?.getAttribute("description")).toEqual(
				basicProps.description,
			);
			expect(mockedNextSeoEl?.getAttribute("site_name")).toEqual(
				basicProps.siteName,
			);
			expect(mockedNextSeoEl?.getAttribute("image")).toContain(
				"constructGenericImage",
			);
		});
	});
	...
	describe("Canonical Url", () => {
		test("Should provide `canonical` prop to NextSeo derived from `origin` prop if provided", async () => {
			const props = {
				title: "Test Title",
				description: "Test Description",
				origin: "http://acme.sln.local",
				siteName: "stresslessness.org",
			};
		...
		test("Should provide `canonical` prop to NextSeo from canonical prop if provided", async () => {
			const props = {
				...
				siteName: "stresslessness.org",
				origin: "http://acme.sln.local",
				canonical: "http://acme.sln.local/some-path",
			};
			vi.mocked(usePathname).mockReturnValue("/mocked-path");
			const { container } = render(<HeadSeo {...props} />);
			await waitFor(async () => {
	...
	test("Should render title with brand", async () => {
		...
		await waitFor(async () => {
			const mockedNextSeoEl = container.querySelector("#mocked-next-seo");
			expect(mockedNextSeoEl?.getAttribute("title")).toEqual(
				`${basicProps.title} | stresslessness.org`,
			);
		});
	});
	...
	test("Should render with app props", async () => {
		const { container } = render(
			<HeadSeo {...basicProps} app={{} as HeadSeoProps["app"]} />,
		);
		await waitFor(async () => {
			const mockedNextSeoEl = container.querySelector("#mocked-next-seo");
			expect(mockedNextSeoEl?.getAttribute("image")).toContain(
	...
	test("Should render with meeting props", async () => {
		const { container } = render(
			<HeadSeo {...basicProps} meeting={{} as HeadSeoProps["meeting"]} />,
		);
		await waitFor(async () => {
			const mockedNextSeoEl = container.querySelector("#mocked-next-seo");
			expect(mockedNextSeoEl?.getAttribute("image")).toContain(
