...
import { getDefaultScheduleId } from "@sln/trpc/server/routers/viewer/availability/util";
...
export default async function handler(
	...
	const setNewTimeZone = async (
		timeZone: string,
		user: { id: number; defaultScheduleId: number | null },
	) => {
  ...
  */
	...
	const travelSchedulesCloseToCurrentDate =
		await prisma.travelSchedule.findMany({
			where: {
				OR: [
					{
						startDate: {
	...
	for (const travelSchedule of travelSchedulesCloseToCurrentDate) {
		...
		const startDateUTC = dayjs(travelSchedule.startDate).subtract(
			offset,
			"minute",
		);
