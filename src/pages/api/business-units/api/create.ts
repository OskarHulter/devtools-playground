...
import {
	_MembershipModel as Membership,
	_TeamModel as Team,
} from "@sln/prisma/zod";
...
export const schemaMembershipPublic = Membership.merge(
	z.object({ team: Team }).partial(),
);
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	const checkoutSessionSubscription =
		getCheckoutSessionSubscription(checkoutSession);
	...
	const finalizedTeam = await prisma.team.update({
		...
		data: {
			...
			metadata: {
				...
				subscriptionItemId:
					checkoutSessionSubscription.items.data[0].id || null,
			},
		},
...
function getCheckoutSessionSubscription(
	checkoutSession: Stripe.Response<Stripe.Checkout.Session>,
) {
...
function getCheckoutSessionMetadata(
	...
	const parseCheckoutSessionMetadata = checkoutSessionMetadataSchema.safeParse(
