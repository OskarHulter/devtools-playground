...
import getStripe from "@sln/app-store/stripepayment/lib/client";
...
import { fetchUsername } from "@sln/lib/fetchUsername";
import { pushGTMEvent } from "@sln/lib/gtm";
import { useCompatSearchParams } from "@sln/lib/hooks/useCompatSearchParams";
import { useDebounce } from "@sln/lib/hooks/useDebounce";
import { useLocale } from "@sln/lib/hooks/useLocale";
import {
	collectPageParameters,
	telemetryEventTypes,
	useTelemetry,
} from "@sln/lib/telemetry";
import { signupSchema as apiSignupSchema } from "@sln/prisma/zod-utils";
import type { inferSSRProps } from "@sln/types/inferSSRProps";
import {
	Button,
	HeadSeo,
	PasswordField,
...
const TurnstileCaptcha = dynamic(() => import("@components/auth/Turnstile"), {
	ssr: false,
});
...
export default function Signup({
	...
	return (
		<>
			{IS_SLN && COOKIE_CONSENT && process.env.NEXT_PUBLIC_GTM_ID ? (
				<>
					<Script
						id="gtm-init-script"
			...
			<div
				className={classNames(
					"light bg-muted 2xl:bg-default flex min-h-screen w-full flex-col items-center justify-center [--sln-brand:#111827] dark:[--sln-brand:#FFFFFF]",
					"[--sln-brand-subtle:#9CA3AF]",
					"[--sln-brand-text:#FFFFFF] dark:[--sln-brand-text:#000000]",
					"[--sln-brand-emphasis:#101010] dark:[--sln-brand-emphasis:#e1e1e1] ",
				)}
			>
					...
					<div className="border-subtle lg:bg-subtle mx-auto mt-24 w-full max-w-2xl flex-col justify-between rounded-l-2xl pl-4 dark:bg-none lg:mt-0 lg:flex lg:max-w-full lg:border lg:py-12 lg:pl-12">
								...
								<div className="-mt-4 mb-6 mr-12 grid w-full grid-cols-3 gap-5 pr-4 sm:gap-3 lg:grid-cols-4">
									<div>
										<img
											...
											alt="stresslessness.org was Product of the Day at ProductHunt"
										/>
									</div>
									<div>
										<img
											...
											className="h-[34px] w-full dark:invert"
											alt="stresslessness.org was Product of the Week at ProductHunt"
										/>
									</div>
									<div>
										<img
											...
											className="h-[34px] w-full dark:invert"
											alt="stresslessness.org was Product of the Month at ProductHunt"
										/>
									</div>
								</div>
						...
						<div className="border-default hidden rounded-bl-2xl rounded-br-none rounded-tl-2xl border border-r-0 border-dashed bg-black/[3%] dark:bg-white/5 lg:block lg:py-[6px] lg:pl-[6px]">
							<img
								...
								alt="stresslessness.org Booking Page"
							/>
							<img
