...
import { getAppWithMetadata } from "@sln/app-store/_appRegistry";
import logger from "@sln/lib/logger";
import { prisma } from "@sln/prisma";
import type { AppCategories, Prisma } from "@sln/prisma/client";
...
export default async function handler(
	...
	for await (const dbApp of dbApps) {
		...
		if (Object.keys(updates).length > 0) {
			log.info(
				`🔨 Updating app ${dbApp.slug} with ${Object.keys(updates).join(", ")}`,
			);
			if (!isDryRun) {
