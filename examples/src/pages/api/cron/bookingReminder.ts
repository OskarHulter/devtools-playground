...
import { sendOrganizerRequestReminderEmail } from "@sln/emails";
...
import { BookingStatus, ReminderType } from "@sln/prisma/enums";
...
export default async function handler(
	...
	for (const interval of reminderIntervalMinutes) {
		...
		for (const booking of bookings.filter(
			(b) => !reminders.some((r) => r.referenceId == b.id),
		)) {
			const { user } = booking;
			const name = user?.name || user?.username;
			if (!user || !name || !user.timeZone) {
				console.error(
					`Booking ${booking.id} is missing required properties for booking reminder`,
					{ user },
				);
				continue;
			}
