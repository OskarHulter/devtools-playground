...
import { sendMonthlyDigestEmails } from "@sln/emails/email-manager";
import { EventsInsights } from "@sln/features/insights/server/events";
...
export default async function handler(
	...
	while (true) {
		...
		for (const team of teams) {
			...
			const countGroupedByStatus =
				await EventsInsights.countGroupedByStatus(whereConditional);
			...
			if (userIds.length === 0) {
				...
				EventData["membersWithMostBookings"] = bookingsFromTeam.map(
					(booking) => {
						return {
