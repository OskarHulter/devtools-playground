...
async function getTeamLogos(subdomain: string, isValidOrgDomain: boolean) {
	try {
		...
		// load from DB
		const { default: prisma } = await import("@sln/prisma");
...
export default async function handler(
	...
	// Resolve all icon types to team logos, falling back to stresslessness.org defaults.
	const type: LogoType =
	...
	try {
		...
		if (teamLogos[logoDefinition.source] && logoDefinition.w) {
			const { detectContentType, optimizeImage } = await import(
