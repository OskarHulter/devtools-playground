...
import type { TGetRetellLLMSchema } from "@sln/prisma/zod-utils";
...
const getEventTypeIdFromRetellLLM = (
	...
	const generalTool = general_tools.find(
		(tool) => tool.event_type_id && tool.timezone,
	);

	if (generalTool) {
		return {
			eventTypeId: generalTool.event_type_id,
			timezone: generalTool.timezone,
		};
	}
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	const retellLLM = await fetcher(`/get-retell-llm/${body.llm_id}`).then(
	...
	if (!eventTypeId || !timezone)
		return res
			.status(404)
			.json({ message: "eventTypeId or Timezone not found" });
	...
	if (!eventType)
		return res.status(404).json({ message: "eventType not found id" });
	...
	return res.status(200).json({
		next_available: firstSlot
			? dayjs
					.utc(firstSlot)
					.tz(timezone)
					.format(`dddd [the] Do [at] h:mma [${timezone} timezone]`)
			: undefined,
	});
}
