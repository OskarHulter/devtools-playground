...
export const questionUtils = {
	addQuestion: async (props: QuestionProps) => {
		...
		fireEvent.keyDown(screen.getByTestId("test-field-type"), {
			key: "ArrowDown",
			code: "ArrowDown",
		});
		fireEvent.click(screen.getByTestId("select-option-email"));
		fireEvent.change(screen.getAllByRole("textbox")[0], {
			target: { value: props.identifier },
		});
		...
		await waitFor(() => {
			expect(
				screen.queryByTestId(`field-${props.identifier}`),
			).toBeInTheDocument();
		});
	},
	editQuestion: async (props: {
		...
		await waitFor(() => {
			...
			expect(
				screen.queryByTestId(`field-${props.existingQuestionId}`),
			).not.toBeInTheDocument();
		});
	},
	deleteQuestion: async (existingQuestionId: string) => {
		expect(
			screen.queryByTestId(`field-${existingQuestionId}`),
		).toBeInTheDocument();
