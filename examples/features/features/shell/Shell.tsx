...
import UnconfirmedBookingBadge from "@sln/features/bookings/UnconfirmedBookingBadge";
import ImpersonatingBanner, {
  type ImpersonatingBannerProps,
} from "@sln/features/ee/impersonation/components/ImpersonatingBanner";
import {
  OrgUpgradeBanner,
  type OrgUpgradeBannerProps,
} from "@sln/features/ee/organizations/components/OrgUpgradeBanner";
import { getOrgFullOrigin } from "@sln/features/ee/organizations/lib/orgDomains";
import HelpMenuItem from "@sln/features/ee/support/components/HelpMenuItem";
import useIntercom, {
  isInterComEnabled,
} from "@sln/features/ee/support/lib/intercom/useIntercom";
import {
  TeamsUpgradeBanner,
  type TeamsUpgradeBannerProps,
} from "@sln/features/ee/teams/components";
import { useFlagMap } from "@sln/features/flags/context/provider";
import { KBarContent, KBarRoot, KBarTrigger } from "@sln/features/kbar/Kbar";
import TimezoneChangeDialog from "@sln/features/settings/TimezoneChangeDialog";
import AdminPasswordBanner, {
  type AdminPasswordBannerProps,
} from "@sln/features/users/components/AdminPasswordBanner";
import CalendarCredentialBanner, {
  type CalendarCredentialBannerProps,
} from "@sln/features/users/components/CalendarCredentialBanner";
import {
  InvalidAppCredentialBanners,
  type InvalidAppCredentialBannersProps,
} from "@sln/features/users/components/InvalidAppCredentialsBanner";
import VerifyEmailBanner, {
  type VerifyEmailBannerProps,
} from "@sln/features/users/components/VerifyEmailBanner";
...
import { useFormbricks } from "@sln/lib/formbricks-client";
import getBrandColours from "@sln/lib/getBrandColours";
import { useBookerUrl } from "@sln/lib/hooks/useBookerUrl";
import { useLocale } from "@sln/lib/hooks/useLocale";
import useTheme from "@sln/lib/hooks/useTheme";
import { isKeyInObject } from "@sln/lib/isKeyInObject";
import { localStorage } from "@sln/lib/webstorage";
import type { User } from "@sln/prisma/client";
import { trpc } from "@sln/trpc/react";
import useEmailVerifyCheck from "@sln/trpc/react/hooks/useEmailVerifyCheck";
import useMeQuery from "@sln/trpc/react/hooks/useMeQuery";
import {
  Avatar,
  Button,
  ButtonOrLink,
...
import { Discord } from "@sln/ui/components/icon/Discord";
import { useGetUserAttributes } from "@sln/web/components/settings/platform/hooks/useGetUserAttributes";
...
const Tips = dynamic(
  () => import("@sln/features/tips").then((mod) => mod.Tips),
  {
    ssr: false,
  }
);
...
const BannerComponent: BannerComponent = {
  teamUpgradeBanner: (props: TeamsUpgradeBannerProps) => (
    <TeamsUpgradeBanner {...props} />
  ),
  orgUpgradeBanner: (props: OrgUpgradeBannerProps) => (
    <OrgUpgradeBanner {...props} />
  ),
  verifyEmailBanner: (props: VerifyEmailBannerProps) => (
    <VerifyEmailBanner {...props} />
  ),
  adminPasswordBanner: (props: AdminPasswordBannerProps) => (
    <AdminPasswordBanner {...props} />
  ),
  impersonationBanner: (props: ImpersonatingBannerProps) => (
    <ImpersonatingBanner {...props} />
  ),
  calendarCredentialBanner: (props: CalendarCredentialBannerProps) => (
...
function useRedirectToOnboardingIfNeeded() {
  ...
  const needsEmailVerification =
    !email?.isVerified && flags["email-verification"];
...
const useBanners = () => {
  const { data: getUserTopBanners, isPending } =
    trpc.viewer.getUserTopBanners.useQuery();
  ...
  const allBanners: allBannerProps = Object.assign(
    {},
    getUserTopBanners,
    userSessionBanners
  );
            ...
            {helpOpen ? (
              ...
              <>
                ...
                <DropdownMenuItem>
                  <DropdownItem
                    CustomStartIcon={
                      <Discord className="text-default h-4 w-4" />
                    }
                    target="_blank"
                    rel="noreferrer"
                    href={JOIN_DISCORD}
...
const navigation: NavigationItemType[] = [
  ...
  },
  {
    ...
    icon: "file-text",
    isCurrent: ({ pathname }) =>
      pathname?.startsWith("/apps/routing-forms/") ?? false,
  },
  {
    name: "workflows",
    href: "/workflows",
...
const platformNavigation: NavigationItemType[] = [
  ...
  {
    name: "Documentation",
    href: "https://docs.oskarhulter.com/docs/platform",
    icon: "bar-chart",
    target: "_blank",
  },
  {
    name: "API reference",
    href: "https://api.oskarhulter.com/v2/docs#/",
    icon: "terminal",
    target: "_blank",
  },
  {
    name: "Atoms",
    href: "https://docs.oskarhulter.com/docs/platform#atoms",
    icon: "atom",
    target: "_blank",
  },
  {
    name: MORE_SEPARATOR_NAME,
    href: "https://docs.oskarhulter.com/docs/platform/faq",
    icon: "ellipsis",
    target: "_blank",
  },
];
...
const getDesktopNavigationItems = (isPlatformNavigation = false) => {
  const navigationType = !isPlatformNavigation
    ? navigation
    : platformNavigation;
  ...
  } = (!isPlatformNavigation ? navigation : platformNavigation).reduce<
...
const Navigation = ({
  ...
  const { desktopNavigationItems } =
    getDesktopNavigationItems(isPlatformNavigation);
...
function SideBar({
  ...
  const publicPageUrl = useMemo(() => {
    ...
    const publicPageUrl = orgBranding?.slug
      ? getOrgFullOrigin(orgBranding.slug)
      : "";
          ...
          <header className="todesktop:-mt-3 todesktop:flex-col-reverse todesktop:[-webkit-app-region:drag] items-center justify-between md:hidden lg:flex">
            {orgBranding ? (
              ...
              <div
                ...
                className="todesktop:mt-4 w-full"
              >
...
function ProfileDropdown() {
  ...
  const currentOption =
    options.find((option) => option.value === sessionData.upId) || options[0];
