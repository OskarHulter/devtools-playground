...
import processExternalId from "@sln/app-store/_utils/calendars/processExternalId";
import { metadata as GoogleMeetMetadata } from "@sln/app-store/googlevideo/_metadata";
import {
	MeetLocationType,
	OrganizerDefaultConferencingAppType,
	getLocationValueForDB,
} from "@sln/app-store/locations";
import type { EventTypeAppsList } from "@sln/app-store/utils";
import { getAppFromSlug } from "@sln/app-store/utils";
import EventManager from "@sln/core/EventManager";
import { getEventName } from "@sln/core/event";
import { getBusyTimesForLimitChecks } from "@sln/core/getBusyTimes";
import { getUsersAvailability } from "@sln/core/getUserAvailability";
import dayjs from "@sln/dayjs";
import { scheduleMandatoryReminder } from "@sln/ee/workflows/lib/reminders/scheduleMandatoryReminder";
import {
...
import getICalUID from "@sln/emails/lib/getICalUID";
import { getReviewFieldsWithSystemFields } from "@sln/features/bookings/lib/getReviewFields";
import { getCalEventResponses } from "@sln/features/bookings/lib/getCalEventResponses";
import { handleWebhookTrigger } from "@sln/features/bookings/lib/handleWebhookTrigger";
import { isEventTypeLoggingEnabled } from "@sln/features/bookings/lib/isEventTypeLoggingEnabled";
import {
	allowDisablingAttendeeConfirmationEmails,
	allowDisablingHostConfirmationEmails,
} from "@sln/features/ee/workflows/lib/allowDisablingStandardEmails";
import {
	cancelWorkflowReminders,
	scheduleWorkflowReminders,
} from "@sln/features/ee/workflows/lib/reminders/reminderScheduler";
import { getFullName } from "@sln/features/form-builder/utils";
...
import { getVideoCallUrlFromCalEvent } from "@sln/lib/CalEventParser";
import { getUTCOffsetByTimezone } from "@sln/lib/date-fns";
import { getDefaultEvent, getUsernameList } from "@sln/lib/defaultEvents";
import { ErrorCode } from "@sln/lib/errorCodes";
import { getErrorFromUnknown } from "@sln/lib/errors";
import { extractBaseEmail } from "@sln/lib/extract-base-email";
import { getBookerBaseUrl } from "@sln/lib/getBookerUrl/server";
import getOrgIdFromMemberOrTeamId from "@sln/lib/getOrgIdFromMemberOrTeamId";
import getPaymentAppData from "@sln/lib/getPaymentAppData";
import { getTeamIdFromEventType } from "@sln/lib/getTeamIdFromEventType";
import { HttpError } from "@sln/lib/http-error";
import isOutOfBounds, { BookingDateInPastError } from "@sln/lib/isOutOfBounds";
import logger from "@sln/lib/logger";
import { handlePayment } from "@sln/lib/payment/handlePayment";
import {
	getPiiFreeCalendarEvent,
	getPiiFreeEventType,
	getPiiFreeUser,
} from "@sln/lib/piiFreeData";
...
import { updateWebUser as syncServicesUpdateWebUser } from "@sln/lib/sync/SyncServiceManager";
import { getTimeFormatStringFromUserTimeFormat } from "@sln/lib/timeFormat";
import prisma, { userSelect } from "@sln/prisma";
import type { BookingReference } from "@sln/prisma/client";
import {
	BookingStatus,
	SchedulingType,
...
import type { EventResult, PartialReference } from "@sln/types/EventManager";

import type { EventTypeInfo } from "../../webhooks/lib/sendPayload";
import { checkForConflicts } from "./conflictChecker/checkForConflicts";
import { getAllCredentials } from "./getAllCredentialsForUsersOnEvent/getAllCredentials";
import { refreshCredentials } from "./getAllCredentialsForUsersOnEvent/refreshCredentials";
import getReviewDataSchema from "./getReviewDataSchema";
...
type ReqBodyWithoutEnd = z.infer<ReturnType<typeof getReviewDataSchema>>;
...
type AwaitedBookingData = Awaited<ReturnType<typeof getReviewData>>;
...
export async function ensureAvailableUsers(
	...
	const endDateTimeUtc =
		input.timeZone === "Etc/GMT"
			? dayjs.utc(input.dateTo)
			: dayjs(input.dateTo).tz(input.timeZone).utc();
	...
	(
		...
		({ oooExcludedDateRanges: dateRanges, busy: bufferedBusyTimes }, index) => {
			...
			try {
				foundConflict = checkForConflicts(
					bufferedBusyTimes,
					startDateTimeUtc,
					duration,
				);
...
async function getOriginalRescheduledBooking(
	uid: string,
...
export async function getReviewData<T extends z.ZodType>({
	req,
	eventType,
	schema,
}: {
	...
	if (reqBody.customInputs) {
		...
		handleCustomInputs(
			eventType.customInputs as EventTypeCustomInput[],
			reqBody.customInputs,
		);
		const reqBodyWithLegacyProps =
			bookingCreateSchemaLegacyPropsForApi.parse(reqBody);
		return {
			...reqBody,
...
async function createBooking({
	...
	rescheduleReason: Awaited<
		ReturnType<typeof getReviewData>
	>["rescheduleReason"];
	bookerEmail: Awaited<ReturnType<typeof getReviewData>>["email"];
	...
	if (originalRescheduledBooking) {
		...
		evt.description =
			originalRescheduledBooking?.description || evt.description;
		evt.location = originalRescheduledBooking?.location || evt.location;
		evt.location = changedOrganizer
			? evt.location
	...
	const dynamicGroupSlugRef = !eventTypeId
		? (reqBodyUser as string).toLowerCase()
		: null;
	...
	const newBookingData: Prisma.BookingCreateInput = {
		...
		responses:
			responses === null || evt.seatsPerTimeSlot ? Prisma.JsonNull : responses,
		title: evt.title,
		startTime: dayjs.utc(evt.startTime).toDate(),
		endTime: dayjs.utc(evt.endTime).toDate(),
		description: evt.seatsPerTimeSlot ? null : evt.additionalNotes,
		customInputs: isPrismaObjOrUndefined(evt.customInputs),
		status: isConfirmedByDefault
			? BookingStatus.ACCEPTED
			: BookingStatus.PENDING,
	...
	if (originalRescheduledBooking) {
		...
		if (originalRescheduledBooking.recurringEventId) {
			newBookingData.recurringEventId =
				originalRescheduledBooking.recurringEventId;
		}
	}
	...
	if (originalRescheduledBooking?.paid && originalRescheduledBooking?.payment) {
		const bookingPayment = originalRescheduledBooking?.payment?.find(
...
export function getCustomInputsResponses(
	reqBody: {
		...
		customInputs?: z.infer<
			typeof bookingCreateSchemaLegacyPropsForApi
		>["customInputs"];
	},
	eventTypeCustomInputs: getEventTypeResponse["customInputs"],
) {
	const customInputsResponses = {} as NonNullable<
		CalendarEvent["customInputs"]
	>;
	if (reqBody.customInputs && (reqBody.customInputs.length || 0) > 0) {
		...
		for (const [fieldName, fieldValue] of Object.entries(responses)) {
			...
			if (foundACustomInputForTheResponse) {
				customInputsResponses[foundACustomInputForTheResponse.label] =
					fieldValue;
			}
		}
	}
 ...
 */
export const addVideoCallDataToEvent = (
	bookingReferences: BookingReference[],
	evt: CalendarEvent,
) => {
	const videoCallReference = bookingReferences.find((reference) =>
...
type BookingDataSchemaGetter =
	| typeof getReviewDataSchema
	| typeof import("@sln/features/bookings/lib/getReviewDataSchemaForApi").default;
...
async function handler(
	...
	bookingDataSchemaGetter: BookingDataSchemaGetter = getReviewDataSchema,
) {
	const {
		userId,
		platformClientId,
	...
	eventType = {
		...eventType,
		bookingFields: getReviewFieldsWithSystemFields(eventType),
	};
	...
	const bookingData = await getReviewData({
		req,
		eventType,
	...
	try {
		...
		if (error instanceof BookingDateInPastError) {
			...
			loggerWithEventDetails.info(
				`Booking eventType ${eventTypeId} failed`,
	...
	const reqEventLength = dayjs(reqBody.end).diff(
		dayjs(reqBody.start),
		"minutes",
	);
	...
	if (!isDynamicAllowed && !eventTypeId) {
		loggerWithEventDetails.warn({
			message:
				"NewBooking: Some of the users in this group do not allow dynamic booking",
		});
		throw new HttpError({
	...
	if (!users.length && eventType.userId) {
		...
		if (!eventTypeUser) {
			loggerWithEventDetails.warn({
				message: "NewBooking: eventTypeUser.notFound",
			});
			throw new HttpError({
				statusCode: 404,
	...
	if (dynamicUserList.length > 1) {
		...
		locationBodyString =
			firstUsersMetadata?.defaultConferencingApp?.appLink || locationBodyString;
	...
	if (
		...
		if (
			...
			/* Empty object is truthy */ Object.keys(eventType.bookingLimits).length >
				0
		) {
	...
	if (rescheduleUid) {
		...
		if (!originalRescheduledBooking) {
			throw new HttpError({
				statusCode: 404,
				message: "Could not find original booking",
			});
		}
	...
	if (isFirstSeat) {
		...
		if (!req.body.allRecurringDates || req.body.isFirstRecurringSlot) {
			...
			if (reqBody.teamMemberEmail) {
				...
				if (
					!fixedUserPool.some((user) => user.email === reqBody.teamMemberEmail)
				) {
					const teamMember = availableUsers.find(
						(user) => user.email === reqBody.teamMemberEmail,
					);
					if (teamMember) {
	...
	// use host default
	if (
		(isManagedEventType || isTeamEventType) &&
	...
	const teamMemberPromises = users
		...
		.map(async (user) => {
			// TODO: Add back once EventManager tests are ready https://github.com/sln/stresslessness.org/pull/14610#discussion_r1567817120
      ...
      */
			if (eventType.schedulingType === SchedulingType.ROUND_ROBIN) {
				...
				const newBookedMembers = newBookingMemberEmails.filter(
					(member) =>
						!originalBookingMemberEmails.find(
							(originalMember) => originalMember.email === member.email,
						),
				);
				// cancelled Emails
				const cancelledMembers = originalBookingMemberEmails.filter(
					(member) =>
						!newBookingMemberEmails.find(
							(newMember) => newMember.email === member.email,
						),
				);
				// rescheduled Emails
				const rescheduledMembers = newBookingMemberEmails.filter((member) =>
					originalBookingMemberEmails.find(
						(orignalMember) => orignalMember.email === member.email,
					),
				);
	...
	try {
		await scheduleWorkflowReminders({
			...
			isFirstRecurringEvent: req.body.allRecurringDates
				? req.body.isFirstRecurringSlot
				: undefined,
...
function getVideoCallDetails({
	...
	const firstVideoResult = results.find((result) =>
		result.type.includes("_video"),
	);
	const metadata: AdditionalInformation = {};
