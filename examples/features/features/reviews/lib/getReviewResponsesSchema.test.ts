...
import type { eventTypeBookingFields } from "@sln/prisma/zod-utils";
import { test } from "@sln/web/test/fixtures/fixtures";

import getReviewResponsesSchema, {
	getReviewResponsesPartialSchema,
} from "./getReviewResponsesSchema";
...
describe("getReviewResponsesSchema", () => {
	test(`should parse booking responses`, async ({}) => {
		const schema = getReviewResponsesSchema({
			bookingFields: [
				{
					name: "name",
					type: "name",
			...
			] as z.infer<typeof eventTypeBookingFields> &
			...
			test(`'email' must be validated `, async () => {
				...
				expect(parsedResponses.error.issues[0]).toEqual(
					expect.objectContaining({
						...
						// So, the error comes from superRefine in getReviewResponsesSchema. We should change this to zod email validation error
...
describe("getReviewResponsesPartialSchema - Prefill validation", () => {
	test(`should be able to get fields prefilled even when name is empty string`, async ({}) => {
		const schema = getReviewResponsesPartialSchema({
			bookingFields: [
				{
					name: "name",
					type: "name",
