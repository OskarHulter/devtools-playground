...
import {
	...
	BookingLocations,
} from "@sln/web/test/utils/bookingScenario/bookingScenario";
import { createMockNextJsRequest } from "@sln/web/test/utils/bookingScenario/createMockNextJsRequest";
import {
...
} from "@sln/web/test/utils/bookingScenario/expects";
import { getMockRequestDataForBooking } from "@sln/web/test/utils/bookingScenario/getMockRequestDataForBooking";
import { setupAndTeardown } from "@sln/web/test/utils/bookingScenario/setupAndTeardown";
import { testWithAndWithoutOrg } from "@sln/web/test/utils/bookingScenario/test";
...
import { WEBSITE_URL, WEBAPP_URL } from "@sln/lib/constants";
import { ErrorCode } from "@sln/lib/errorCodes";
import { resetTestEmails } from "@sln/lib/testEmails";
...
describe("handleNewBooking", () => {
    ...
    `,
			async ({ emails, org }) => {
				const handleNewBooking = (
					await import("@sln/features/bookings/lib/handleNewBooking")
				).default;
				...
				const organizerDestinationCalendarEmailOnEventType =
					"organizerEventTypeEmail@example.com";
    ...
    `,
				async ({ emails }) => {
					...
					expectSuccessfulCalendarEventCreationInCalendar(calendarMock, {
						...
						// Google Calendar in this case fallbacks to the "primary" calendar - https://github.com/sln/stresslessness.org/blob/7d5dad7fea78ff24dddbe44f1da5d7e08e1ff568/packages/app-store/googlecalendar/lib/CalendarService.ts#L217
    ...
    `,
		...
		describe("Event length check during booking", () => {
			test(
				...
				async () => {
					...
					await expect(
						async () => await handleNewBooking(req),
		...
		describe(
			...
			() => {
				test(
					`should fail a booking if there is already a stresslessness.org booking overlapping the time`,
					async ({}) => {
						const handleNewBooking = (
							await import("@sln/features/bookings/lib/handleNewBooking")
						).default;
						...
						const { dateString: plus1DateString } = getDate({
							dateIncrement: 1,
						});
