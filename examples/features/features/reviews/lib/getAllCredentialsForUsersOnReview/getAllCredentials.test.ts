...
// vi.mock("@sln/lib/server/repository/user", () => {
//   return {
...
describe("getAllCredentials", () => {
	test("Get an individual's credentials", async () => {
		...
		const getAllCredentials = (await import("./getAllCredentials"))
			.getAllCredentials;
		...
		expect(credentials).toContainEqual(
			expect.objectContaining({ type: "user-credential" }),
		);
	});
		...
		describe("If CRM is enabled on the event type", () => {
			describe("With _crm credentials", () => {
				test("For users", async () => {
					...
					expect(credentials).toContainEqual(
						expect.objectContaining({ userId: 1, type: "salesforce_crm" }),
					);
				});
				test("For teams", async () => {
					...
					expect(credentials).toContainEqual(
						expect.objectContaining({ teamId: 1, type: "salesforce_crm" }),
					);
				});
				test("For child of managed event type", async () => {
					...
					expect(credentials).toContainEqual(
						expect.objectContaining({ teamId, type: "salesforce_crm" }),
					);
				});
					...
					await addUsersToDb([
						{
							...
							profiles: {
								create: [
									{ organizationId: orgId, uid: "MOCK_UID", username: "test" },
								],
							},
						},
					]);
