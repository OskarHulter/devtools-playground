export function isEventTypeLoggingEnabled({
	...
	const usernameOrTeamnamesList =
		usernameOrTeamName instanceof Array
			? usernameOrTeamName
			: [usernameOrTeamName];
	...
	// eslint-disable-next-line turbo/no-undeclared-env-vars
	const bookingLoggingUsername =
		process.env.BOOKING_LOGGING_USER_OR_TEAM_NAME || "";
	return bookingLoggingUsername.split(",").some((u) => {
		return usernameOrTeamnamesList.some(
			(foundUsername) => foundUsername === u.trim(),
		);
	});
}
