...
import { getRichDescription } from "@sln/lib/CalEventParser";
import { EMAIL_FROM_NAME } from "@sln/lib/constants";
...
export default class OrganizerScheduledEmail extends BaseEmail {
  ...
  protected async getNodeMailerPayload(): Promise<Record<string, unknown>> {
    ...
    const toAddresses = [
      this.teamMember?.email || this.calEvent.organizer.email,
    ];
    ...
    return {
      ...
      subject: `${this.newSeat ? `${this.t("new_attendee")}: ` : ""}${
        this.calEvent.title
      }`,
...
`.trim();
  ...
  protected getFormattedDate() {
    const organizerTimeFormat =
      this.calEvent.organizer.timeFormat || TimeFormat.TWELVE_HOUR;
    return `${this.getOrganizerStart(
      organizerTimeFormat
    )} - ${this.getOrganizerEnd(organizerTimeFormat)}, ${this.t(
