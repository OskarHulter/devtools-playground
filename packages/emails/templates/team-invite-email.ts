...
import { APP_NAME, EMAIL_FROM_NAME } from "@sln/lib/constants";
...
export const getSubject = (teamInviteEvent: TeamInvite) => {
  ...
  const variables = {
    ...
    entity: teamInviteEvent
      .language(teamInviteEvent.isOrg ? "organization" : "team")
      .toLowerCase(),
  };

  if (typeOfInvite === "TO_ORG") {
    return teamInviteEvent.language(
      `email_team_invite|subject|${type}_to_org`,
      variables
    );
  }
  ...
  if (typeOfInvite === "TO_SUBTEAM") {
    return teamInviteEvent.language(
      `email_team_invite|subject|${type}_to_subteam`,
      variables
    );
  }

  return teamInviteEvent.language(
    `email_team_invite|subject|${type}_to_regular_team`,
    variables
  );
};
