...
import { APP_NAME, WEBAPP_URL, IS_PRODUCTION } from "@sln/lib/constants";
      ...
      >
        <p style={{ fontWeight: 400, margin: 0 }}>
          <>
            ...
            <a
              href="mailto:support@oskarhulter.com"
              style={{ color: "#3E3E3E" }}
              target="_blank"
              rel="noreferrer"
            >
  ...
  function getHeading() {
    ...
    if (typeOfInvite === "TO_ORG") {
      return props.language(
        `email_team_invite|heading|${autoJoinType}_to_org`,
        variables
      );
    }
    ...
    if (typeOfInvite === "TO_SUBTEAM") {
      return props.language(
        `email_team_invite|heading|${autoJoinType}_to_subteam`,
        variables
      );
    }

    return props.language(
      `email_team_invite|heading|invited_to_regular_team`,
      variables
    );
  }
    ...
    if (typeOfInvite === "TO_ORG") {
            ...
            {autoJoinType == "added" ? (
              <>
                <Trans i18nKey="email_team_invite|content|added_to_org">
                  {invitedBy} has added you to the <strong>{teamName}</strong>{" "}
                  organization.
                </Trans>{" "}
                <Trans
                ...
                >
                  ...
                  <a href={prevLink ?? ""}>{prevLinkWithoutProtocol}</a> to{" "}
                  <a href={newLink ?? ""}>{newLinkWithoutProtocol}</a> but
                  don&apos;t worry, all previous links still work and redirect
                  appropriately.
                  <br />
                  <br />
                  Please note: All of your personal event types have been moved
                  into the <strong>{teamName}</strong> organisation, which may
                  also include potential personal link.
                  <br />
                  <br />
                  Please log in and make sure you have no private events on your
                  new organisational account.
                  <br />
                  <br />
                  For personal events we recommend creating a new account with a
              ...
              <>
                <Trans i18nKey="email_team_invite|content|invited_to_org">
                  ...
                  <strong>{teamName}</strong> organization.
                </Trans>{" "}
                <Trans
                ...
                >
                  On accepting the invite, your link will change to your
                  organization domain but don&apos;t worry, all previous links
                  will still work and redirect appropriately.
                  <br />
                  <br />
                  Please note: All of your personal event types will be moved
                  into the <strong>{teamName}</strong> organisation, which may
                  also include potential personal link.
                  <br />
                  <br />
      ...
      return (
        <>
          ...
          <Trans>
            {appName} is the event-juggling scheduler that enables you and your
            team to schedule meetings without the email tennis.
          </Trans>
        </>
      );
    }
        ...
        <>
          {autoJoinType === "added" ? (
            <Trans i18nKey="email_team_invite|content|added_to_subteam">
              {invitedBy} has added you to the team <strong>{teamName}</strong>{" "}
              in their organization <strong>{parentTeamName}</strong>.
            </Trans>
          ) : (
            <Trans i18nKey="email_team_invite|content|invited_to_subteam">
              {invitedBy} has invited you to the team{" "}
              <strong>{teamName}</strong> in their organization{" "}
