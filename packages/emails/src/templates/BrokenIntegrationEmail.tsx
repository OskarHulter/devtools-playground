...
import { AppStoreLocationType } from "@sln/app-store/locations";
...
const BrokenVideoIntegration = (props: {
  ...
  return (
    <Trans i18nKey="broken_video_action" t={props.t}>
      We could not add the <span>{props.location}</span> meeting link to your
      scheduled event. Contact your invitees or update your calendar event to
      add the details. You can either&nbsp;
      <a
        href={
          props.eventTypeId
...
const BrokenCalendarIntegration = (props: {
  ...
  return (
    <Trans i18nKey="broken_calendar_action" t={props.t}>
      ...
      <a href={`${WEBAPP_URL}/apps/installed`}>
        Please check your calendar settings or remove and add your calendar
        again
      </a>
    </Trans>
  );
...
export const BrokenIntegrationEmail = (
  ...
  if (type === "video") {
    let location = calEvent.location
      ? getEnumKeyByEnumValue(AppStoreLocationType, calEvent.location)
      : " ";
    if (location === "Daily") {
      location = "Cal Video";
    }
  ...
  if (type === "calendar") {
    ...
    if (Array.isArray(calendar)) {
      const calendarCap = calendar.map(
