...
import type { EventNameObjectType } from "@sln/core/event";
import { getEventName } from "@sln/core/event";
import type BaseEmail from "@sln/emails/templates/_base-email";
import { formatCalEvent } from "@sln/lib/formatCalendarEvent";
import type { CalendarEvent, Person } from "@sln/types/Calendar";
...
export const sendScheduledEmails = async (
  ...
  if (!hostEmailDisabled) {
    emailsToSend.push(
      sendEmail(
        () => new OrganizerScheduledEmail({ calEvent: formattedCalEvent })
      )
    );
        ...
        return sendEmail(
          () =>
            new AttendeeScheduledEmail(
              {
                ...formattedCalEvent,
                ...(formattedCalEvent.hideCalendarNotes && {
...
export const sendRoundRobinRescheduledEmails = async (
  ...
  for (const teamMember of members) {
    emailsToSend.push(
      sendEmail(
        () =>
          new OrganizerRescheduledEmail({ calEvent: calendarEvent, teamMember })
      )
    );
  }
...
export const sendRoundRobinCancelledEmails = async (
  ...
  for (const teamMember of members) {
    emailsToSend.push(
      sendEmail(
        () =>
          new OrganizerCancelledEmail({ calEvent: calendarEvent, teamMember })
      )
    );
  }
...
export const sendRescheduledEmails = async (calEvent: CalendarEvent) => {
  ...
  emailsToSend.push(
    ...calendarEvent.attendees.map((attendee) => {
      return sendEmail(
        () => new AttendeeRescheduledEmail(calendarEvent, attendee)
      );
    })
  );
...
export const sendScheduledSeatsEmails = async (
  ...
  if (!hostEmailDisabled) {
    emailsToSend.push(
      sendEmail(
        () => new OrganizerScheduledEmail({ calEvent: calendarEvent, newSeat })
      )
    );
      ...
      sendEmail(
        () =>
          new AttendeeScheduledEmail(
            {
              ...calendarEvent,
              ...(calendarEvent.hideCalendarNotes && {
                additionalNotes: undefined,
              }),
            },
            invitee,
...
export const sendCancelledSeatEmails = async (
  ...
  await Promise.all([
    sendEmail(
      () => new AttendeeCancelledSeatEmail(clonedCalEvent, cancelledAttendee)
    ),
    sendEmail(
      () =>
        new OrganizerAttendeeCancelledSeatEmail({ calEvent: formattedCalEvent })
    ),
  ]);
};
...
export const sendOrganizerRequestEmail = async (calEvent: CalendarEvent) => {
  ...
  emailsToSend.push(
    sendEmail(() => new OrganizerRequestEmail({ calEvent: calendarEvent }))
  );
  ...
  if (calendarEvent.team?.members) {
    for (const teamMember of calendarEvent.team.members) {
      emailsToSend.push(
        sendEmail(
          () =>
            new OrganizerRequestEmail({ calEvent: calendarEvent, teamMember })
        )
      );
    }
  }
...
export const sendDeclinedEmails = async (calEvent: CalendarEvent) => {
  ...
  emailsToSend.push(
    ...calendarEvent.attendees.map((attendee) => {
      return sendEmail(
        () => new AttendeeDeclinedEmail(calendarEvent, attendee)
      );
    })
  );
...
export const sendCancelledEmails = async (
  ...
  emailsToSend.push(
    sendEmail(() => new OrganizerCancelledEmail({ calEvent: calendarEvent }))
  );
          ...
          new AttendeeCancelledEmail(
            {
              ...
              title: getEventName({
                ...
                ...(calendarEvent.responses && {
                  bookingFields: calendarEvent.responses,
                }),
...
export const sendOrganizerRequestReminderEmail = async (
  ...
  emailsToSend.push(
    sendEmail(
      () => new OrganizerRequestReminderEmail({ calEvent: calendarEvent })
    )
  );
  ...
  if (calendarEvent.team?.members) {
    for (const teamMember of calendarEvent.team.members) {
      emailsToSend.push(
        sendEmail(
          () =>
            new OrganizerRequestReminderEmail({
              calEvent: calendarEvent,
              teamMember,
            })
        )
...
export const sendAwaitingPaymentEmail = async (calEvent: CalendarEvent) => {
  ...
  emailsToSend.push(
    ...calEvent.attendees.map((attendee) => {
      return sendEmail(
        () => new AttendeeAwaitingPaymentEmail(calEvent, attendee)
      );
    })
  );
  await Promise.all(emailsToSend);
...
export const sendOrganizationCreationEmail = async (
  organizationCreationEvent: OrganizationCreation
) => {
  await sendEmail(
    () => new OrganizationCreationEmail(organizationCreationEvent)
  );
};
...
export const sendChangeOfEmailVerificationLink = async (
  verificationInput: ChangeOfEmailVerifyLink
) => {
...
export const sendRequestRescheduleEmail = async (
  ...
  emailsToSend.push(
    sendEmail(
      () => new OrganizerRequestedToRescheduleEmail(calendarEvent, metadata)
    )
  );

  emailsToSend.push(
    sendEmail(
      () => new AttendeeWasRequestedToRescheduleEmail(calendarEvent, metadata)
    )
  );
...
export const sendLocationChangeEmails = async (calEvent: CalendarEvent) => {
  ...
  emailsToSend.push(
    sendEmail(
      () => new OrganizerLocationChangeEmail({ calEvent: calendarEvent })
    )
  );
  ...
  if (calendarEvent.team?.members) {
    for (const teamMember of calendarEvent.team.members) {
      emailsToSend.push(
        sendEmail(
          () =>
            new OrganizerLocationChangeEmail({
              calEvent: calendarEvent,
              teamMember,
            })
        )
  ...
  emailsToSend.push(
    ...calendarEvent.attendees.map((attendee) => {
      return sendEmail(
        () => new AttendeeLocationChangeEmail(calendarEvent, attendee)
      );
    })
  );
...
export const sendDailyVideoRecordingEmails = async (
  ...
  emailsToSend.push(
    sendEmail(
      () =>
        new OrganizerDailyVideoDownloadRecordingEmail(
          calendarEvent,
          downloadLink
        )
    )
  ...
  for (const attendee of calendarEvent.attendees) {
    emailsToSend.push(
      sendEmail(
        () =>
          new AttendeeDailyVideoDownloadRecordingEmail(
            calendarEvent,
            attendee,
            downloadLink
          )
...
export const sendDailyVideoTranscriptEmails = async (
  ...
  emailsToSend.push(
    sendEmail(
      () =>
        new OrganizerDailyVideoDownloadTranscriptEmail(calEvent, transcripts)
    )
  );
  ...
  for (const attendee of calEvent.attendees) {
    emailsToSend.push(
      sendEmail(
        () =>
          new AttendeeDailyVideoDownloadTranscriptEmail(
            calEvent,
            attendee,
            transcripts
          )
...
export const sendBookingRedirectNotification = async (
