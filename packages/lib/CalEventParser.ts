...
export const getAppsStatus = (calEvent: CalendarEvent, t: TFunction) => {
  ...
  return `\n${t("apps_status")}
      ${calEvent.appsStatus.map((app) => {
        return `\n- ${app.appName} ${
          app.success >= 1
            ? `✅ ${app.success > 1 ? `(x${app.success})` : ""}`
            : ""
        }${
          app.warnings && app.warnings.length >= 1
            ? app.warnings.map((warning) => `\n   - ${warning}`)
            : ""
        } ${
          app.failures && app.failures >= 1
            ? `❌ ${app.failures > 1 ? `(x${app.failures})` : ""}`
            : ""
        } ${
          app.errors && app.errors.length >= 1
...
export const getProviderName = (calEvent: CalendarEvent): string => {
  // TODO: use getAppName from @sln/app-store
...
export const getUid = (calEvent: CalendarEvent): string => {
  ...
  return (
    uid ?? translator.fromUUID(uuidv5(JSON.stringify(calEvent), uuidv5.URL))
  );
};
...
export const getBookingUrl = (calEvent: CalendarEvent) => {
  if (calEvent.platformClientId) {
    ...
    return `${calEvent.platformBookingUrl}/${getUid(calEvent)}?slug=${
      calEvent.type
    }&username=${calEvent.organizer.username}&changes=true`;
  }

  return `${calEvent.bookerUrl ?? WEBAPP_URL}/booking/${getUid(
    calEvent
  )}?changes=true`;
};
...
export const getPlatformManageLink = (
  ...
  const shouldDisplayReschedule =
    !calEvent.recurringEvent && calEvent.platformRescheduleUrl;
  let res =
    calEvent.platformBookingUrl ||
    shouldDisplayReschedule ||
    calEvent.platformCancelUrl
      ? `${t("need_to_reschedule_or_cancel")} `
      : ``;
  if (calEvent.platformBookingUrl) {
    res += `Check Here: ${calEvent.platformBookingUrl}/${getUid(
      calEvent
    )}?slug=${calEvent.type}&username=${
      calEvent.organizer.username
    }&changes=true${
      calEvent.platformCancelUrl || shouldDisplayReschedule
        ? ` ${t("or_lowercase")} `
        : ""
    }`;
  }
...
export const getManageLink = (calEvent: CalendarEvent, t: TFunction) => {
  ...
  return `${t("need_to_reschedule_or_cancel")} ${
    calEvent.bookerUrl ?? WEBAPP_URL
  }/booking/${getUid(calEvent)}?changes=true`;
};
...
export const getPlatformCancelLink = (
  ...
  if (calEvent.platformCancelUrl) {
    const platformCancelLink = new URL(
      `${calEvent.platformCancelUrl}/${bookingUid}`
    );
    platformCancelLink.searchParams.append("slug", calEvent.type);
    calEvent.organizer.username &&
      platformCancelLink.searchParams.append(
        "username",
        calEvent.organizer.username
      );
    ...
    platformCancelLink.searchParams.append(
      ...
      String(!!calEvent.recurringEvent)
    );
    if (seatUid)
      platformCancelLink.searchParams.append("seatReferenceUid", seatUid);
...
export const getCancelLink = (calEvent: CalendarEvent): string => {
  ...
  const cancelLink = new URL(
    `${calEvent.bookerUrl ?? WEBAPP_URL}/booking/${Uid}`
  );
  cancelLink.searchParams.append("cancel", "true");
  cancelLink.searchParams.append(
    "allRemainingBookings",
    String(!!calEvent.recurringEvent)
  );
  if (seatReferenceUid)
...
export const getPlatformRescheduleLink = (
  ...
  if (calEvent.platformRescheduleUrl) {
    ...
    calEvent.organizer.username &&
      platformRescheduleLink.searchParams.append(
        "username",
        calEvent.organizer.username
      );
...
export const getRescheduleLink = (calEvent: CalendarEvent): string => {
  ...
  return `${calEvent.bookerUrl ?? WEBAPP_URL}/reschedule/${
    seatUid ? seatUid : Uid
  }`;
};
...
export const getVideoCallPassword = (calEvent: CalendarEvent): string => {
  return isDailyVideoCall(calEvent)
    ? ""
    : calEvent?.videoCallData?.password ?? "";
};
