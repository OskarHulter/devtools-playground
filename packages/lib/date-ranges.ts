...
import type { Availability } from "@sln/prisma/client";
...
export function processWorkingHours({
  ...
  for (
    ...
    utcDateTo.isAfter(date);
    date = date.add(1, "day")
  ) {
    const fromOffset = dateFrom.startOf("day").utcOffset();

    const adjustedTimezone = getAdjustedTimezone(
      date,
      timeZone,
      travelSchedules
    );
...
export function buildDateRanges({
  ...
  const OOOdates = outOfOffice
    ? Object.keys(outOfOffice).map((outOfOffice) =>
        processOOO(dayjs(outOfOffice), timeZone)
      )
    : [];
...
function getIntersection(range1: DateRange, range2: DateRange) {
  const start = range1.start.utc().isAfter(range2.start)
    ? range1.start
    : range2.start;
