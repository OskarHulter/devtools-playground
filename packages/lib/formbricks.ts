...
import type { Feedback } from "@sln/emails/templates/feedback-email";
...
export const sendFeedbackFormbricks = async (
  userId: number,
  feedback: Feedback
) => {
  if (
    ...
    throw new Error(
      "Missing FORMBRICKS_HOST_URL or FORMBRICKS_ENVIRONMENT_ID env variable"
    );
