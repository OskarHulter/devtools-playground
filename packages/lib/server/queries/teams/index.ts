...
import prisma, { baseEventTypeSelect } from "@sln/prisma";
    ...
    return {
      ...
      connectedApps: !isTeamView
        ? credentials?.map((cred) => {
            ...
            const isCalendar =
              cred?.app?.categories?.includes("calendar") ?? false;
            const externalId = isCalendar
              ? cred.destinationCalendars?.[0]?.externalId
              : null;
            return {
...
export async function updateNewTeamMemberEventTypes(
  ...
  const allManagedEventTypePropsZod = _EventTypeModel.pick(
    allManagedEventTypeProps
  );
    ...
    (await prisma.$transaction(
      eventTypesToAdd.map((eventType) => {
        if (eventType.schedulingType === "MANAGED") {
          ...
          const currentWorkflowIds = eventType.workflows?.map(
            (wf) => wf.workflowId
          );
          ...
          return prisma.eventType.create({
            data: {
              ...
              workflows: currentWorkflowIds && {
                create: currentWorkflowIds.map((wfId) => ({
                  workflowId: wfId,
                })),
              },
            },
