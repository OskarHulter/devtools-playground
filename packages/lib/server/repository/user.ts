...
import { whereClauseForOrgWithSlugOrRequestedSlug } from "@sln/ee/organizations/lib/orgDomains";
...
import type { User as UserType } from "@sln/prisma/client";
import { MembershipRole } from "@sln/prisma/enums";
import type { UpId, UserProfile } from "@sln/types/UserProfile";
...
export class UserRepository {
  ...
  static async findByEmailAndIncludeProfilesAndPassword({
    email,
  }: {
    email: string;
  }) {
  ...
  static async findManyByOrganization({
    organizationId,
  }: {
  ...
  static async findIfAMemberOfSomeOrganization({
    user,
  }: {
    user: { id: number };
  }) {
  ...
  static async isMovedToAProfile({
    user,
  }: {
  ...
  static async enrichUserWithTheProfile<
   ...
   */
  static async enrichUserWithItsProfile<
