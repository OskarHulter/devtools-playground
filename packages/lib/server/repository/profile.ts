...
export class ProfileRepository {
  ...
  private static getInheritedDataFromUser({
    ...
    user: Pick<
      PrismaUser,
      "name" | "avatarUrl" | "startTime" | "endTime" | "bufferTime"
    >;
  }) {
    return {
      name: user.name,
  ...
  private static async _create({
    ...
    log.debug(
      "_create",
      safeStringify({ userId, organizationId, username, email })
    );
  ...
  static async findManyByOrgSlugOrRequestedSlug({
    ...
    logger.debug(
      ...
      safeStringify({ usernames, orgSlug })
    );
  ...
  static _getPrismaWhereForProfilesOfOrg({
    orgSlug,
  }: {
    orgSlug: string | null;
  }) {
