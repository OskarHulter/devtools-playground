...
export class OrganizationRepository {
  static async createWithExistingUserAsOwner({
    ...
    logger.debug(
      "createWithExistingUserAsOwner",
      safeStringify({ orgData, owner })
    );
  ...
  static async createWithNonExistentOwner({
    ...
    logger.debug(
      "createWithNonExistentOwner",
      safeStringify({ orgData, owner })
    );
    const organization = await this.create(orgData);
    const ownerUsernameInOrg = getOrgUsernameFromEmail(
      owner.email,
      orgData.autoAcceptEmail
    );
