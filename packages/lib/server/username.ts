...
export const isPremiumUserName = IS_PREMIUM_USERNAME_ENABLED
  ...
  : // outside of oskarhulter.com the concept of premium username needs not exist.
    () => Promise.resolve(false);

export const generateUsernameSuggestion = async (
  users: string[],
  username: string
) => {
  ...
  while (
    users.includes(
      username + String(rand).padStart(4 - rand.toString().length, "0")
    )
  ) {
...
{
  statusCode: RequestWithUsernameStatus["usernameStatus"]["statusCode"];
  message: string;
} => {
...
const usernameHandler =
  ...
  async (
    ...
    const check = await usernameCheckForSignup({
      username,
...
const usernameCheckForSignup = async ({
  ...
  if (user) {
    ...
    if (!userIsAMemberOfAnOrg) {
      ...
      response.available =
        isClaimingUnsetUsername || isClaimingAlreadySetUsername;
