...
import sanitizeCalendarObject from "@sln/lib/sanitizeCalendarObject";
import type { Calendar, CalendarEvent, CalendarEventType, EventBusyDate, IntegrationCalendar, NewCalendarEventType } from "@sln/types/Calendar";
...
import { createAccount, createCalendarObject, deleteCalendarObject, fetchCalendarObjects, fetchCalendars, getBasicAuthHeaders, updateCalendarObject } from "tsdav";
...
const getTravelDurationInSeconds = (
  vevent: ICAL.Component,
  log: typeof logger
) => {
  const travelDuration: ICAL.Duration = vevent.getFirstPropertyValue(
...
export default abstract class BaseCalendarService implements Calendar {
  ...
  async createEvent(
    ...
    try {
      ...
      if (error || !iCalString)
        throw new Error(
          `Error creating iCalString:=> ${error?.message} : ${error?.name} `
        );
        ...
        responses.map((response) => {
          if (response.status >= 200 && response.status < 300) {
            return {
              ...
              id:
                typeof calendarEvent.uid === "string"
                  ? calendarEvent.uid
                  : "-1",
              password: "",
              url: calendarEvent.url,
              additionalInfo:
                typeof event.additionalInformation === "string"
  ...
  async getAvailability(
    ...
    const userTimeZone = userId
      ? await this.getUserTimezoneFromDB(userId)
      : "Europe/London";
    const events: { start: string; end: string }[] = [];
    objects.forEach((object) => {
      if (!object || object.data == null || JSON.stringify(object.data) == "{}")
        return;
      ...
      vevents.forEach((vevent) => {
        ...
        const dtstart: { [key: string]: string } | undefined =
        ...
        const tzid: string | undefined =
          vevent?.getFirstPropertyValue("tzid") || isUTC ? "UTC" : timezone;
        ...
        if (!vcalendar.getFirstSubcomponent("vtimezone")) {
          ...
          if (timezoneToUse) {
            try {
              ...
              // get timezone offset
              const tzoffsetfrom = dayjs(event.startDate.toJSDate())
                .tz(timezoneToUse)
                .format("Z");
              const tzoffsetto = dayjs(event.endDate.toJSDate())
                .tz(timezoneToUse)
                .format("Z");
        ...
        applyTravelDuration(
          event,
          getTravelDurationInSeconds(vevent, this.log)
        );
      ...
      return calendars.reduce<IntegrationCalendar[]>(
        (newCalendars, calendar) => {
          ...
          newCalendars.push({
            ...
            /** @url https://github.com/calcom/oskarhulter.com/issues/7186 */
            name:
