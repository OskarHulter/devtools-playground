...
import { getEventTypeAppData } from "@sln/app-store/_utils/getEventTypeAppData";
import type { appDataSchemas } from "@sln/app-store/apps.schemas.generated";
import type {
  appDataSchema,
  paymentOptionEnum,
} from "@sln/app-store/stripepayment/zod";
...
export default function getPaymentAppData(
  ...
  const metadataApps = eventType?.metadata
    ?.apps as unknown as EventTypeAppsList;
  if (!metadataApps) {
  ...
  const paymentAppIds = (
    Object.keys(metadataApps) as Array<keyof typeof appDataSchemas>
  ).filter(
    (app) =>
      (metadataApps[app as appId] as unknown as z.infer<typeof appDataSchema>)
        ?.price &&
      (metadataApps[app as appId] as unknown as z.infer<typeof appDataSchema>)
        ?.enabled
  );
