...
export async function getCloseComContactIds(
  ...
  if (closeComContacts.data.length < persons.length && leadFromCalComId) {
    ...
    const existingContactsEmails = closeComContacts.data.flatMap((cont) =>
      cont.emails
        .filter((em) => personsEmails.includes(em.email))
        .map((ems) => ems.email)
    );
    const nonExistingContacts = persons.filter(
      (person) => !existingContactsEmails.includes(person.email)
    );
    ...
    if (createdContacts.length === nonExistingContacts.length) {
      ...
      return Promise.resolve(
        closeComContacts.data
          .map((cont) => cont.id)
          .concat(createdContacts.map((cont) => cont.id))
      );
    } else {
      return Promise.reject(
        "Some contacts were not possible to create in Close.com"
      );
    }
  } else {
...
export async function getCustomActivityTypeInstanceData(
  ...
  // Get Contacts ids
  const contactsIds = await getCloseComContactIds(
    event.attendees,
    closeCom,
    leadFromCalComId
  );
  // Get Custom Activity Type id
  const customActivityTypeAndFieldsIds =
    await getCloseComCustomActivityTypeFieldsIds(customFields, closeCom);
  ...
  return Object.assign(
    ...
    ...customActivityTypeAndFieldsIds.fields.map(
...
export async function getCustomFieldsIds(
  ...
  const allFields:
    | CloseComCustomActivityFieldGet
    | CloseComCustomContactFieldGet = await closeCom.customField[entity].get({
    query: {
      _fields: ["name", "id"].concat(
        entity === "activity" ? ["custom_activity_type_id"] : []
      ),
    },
  });
...
export async function getCloseComCustomActivityTypeFieldsIds(
  ...
  const calComCustomActivity = customActivities.data.filter(
    (act) => act.name === `${APP_NAME} Activity`
  );
  if (calComCustomActivity.length > 0) {
    // Cal.com Custom Activity type exist
    // Get Custom Activity Type fields ids
    const fields = await getCustomFieldsIds(
      "activity",
      customFields,
      closeCom,
...
export async function getCloseComLeadId(
  ...
  const closeComLeadNames = await closeCom.lead.list({
    query: { _fields: ["name", "id"] },
  });
