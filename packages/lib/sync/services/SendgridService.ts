  ...
  implements ISyncService
{
  ...
  upsert = async (user: WebUserInfoType | ConsoleUserInfoType) => {
    ...
    const customFieldsIds = await this.service.getSendgridCustomFieldsIds(
    ...
    this.log.debug(
      "sync:sendgrid:contact:customContactFieldsValues",
      customContactFieldsValues
    );
  ...
  public web = {
    user: {
      ...
      delete: async (webUser: WebUserInfoType) => {
        const [contactId] = await this.service.getSendgridContactId(
          webUser.email
        );
        if (contactId) {
