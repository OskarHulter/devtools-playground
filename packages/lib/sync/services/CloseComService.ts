import type { CloseComFieldOptions, CloseComLead } from "@sln/lib/CloseCom";
import CloseCom from "@sln/lib/CloseCom";
import {
  getCloseComContactIds,
  getCloseComLeadId,
  getCustomFieldsIds,
} from "@sln/lib/CloseComeUtils";
import logger from "@sln/lib/logger";
import type { TeamInfoType } from "@sln/lib/sync/ISyncService";
import SyncServiceCore from "@sln/lib/sync/ISyncService";
import type {
  ConsoleUserInfoType,
  WebUserInfoType,
} from "@sln/lib/sync/ISyncService";
import type ISyncService from "@sln/lib/sync/ISyncService";
...
const calComSharedFields: CloseComFieldOptions = [
  ["Contact Role", "text", false, false],
];

const serviceName = "closecom_service";

export default class CloseComService
  ...
  implements ISyncService
{
  ...
  constructor() {
    super(
      serviceName,
      CloseCom,
      logger.getSubLogger({ prefix: [`[[sync] ${serviceName}`] })
    );
  }
  ...
  upsertAnyUser = async (
    ...
    const [contactId] = await getCloseComContactIds(
      [user],
      this.service,
      leadId
    );
    ...
    const customFieldsIds = await getCustomFieldsIds(
      "contact",
      calComCustomContactFields,
      this.service
    );
    this.log.debug("sync:closecom:user:customFieldsIds", { customFieldsIds });
    // Get shared fields ids
    const sharedFieldsIds = await getCustomFieldsIds(
      "shared",
      calComSharedFields,
      this.service
    );
    ...
    const lastBooking =
      "email" in user ? await this.getUserLastBooking(user) : null;
    ...
    this.log.debug("sync:closecom:contact:allFieldsValues", {
      allFieldsValues,
    });
  ...
  public web = {
    user: {
      ...
      delete: async (webUser: WebUserInfoType) => {
        ...
        this.log.debug("sync:closecom:web:user:delete:contactId", {
          contactId,
        });
        if (contactId) {
    ...
    team: {
      ...
      delete: async (team: TeamInfoType) => {
        ...
        const leadId = await getCloseComLeadId(this.service, {
          companyName: team.name,
        });
      ...
      update: async (prevTeam: TeamInfoType, updatedTeam: TeamInfoType) => {
        this.log.debug("sync:closecom:web:team:update", {
          prevTeam,
          updatedTeam,
        });
        const leadId = await getCloseComLeadId(this.service, {
          companyName: prevTeam.name,
        });
    ...
    membership: {
      delete: async (webUser: WebUserInfoType) => {
        ...
        this.log.debug("sync:closecom:web:membership:delete:contactId", {
          contactId,
        });
        if (contactId) {
          return this.service.contact.delete(contactId);
