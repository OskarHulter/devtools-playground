...
function transformApiEventTypeLocations(
...
const IntegrationSchema = z.object({
  type: z.union([
    integrationsMappingSchema["cal-video"],
    integrationsMappingSchema["cal-video"],
  ]),
});
...
function transformApiEventTypeBookingFields(
  ...
  const customBookingFields = inputBookingFields.map((field) => {
    const commonFields: CommonField = {
      ...
      placeholder:
        "placeholder" in field && field.placeholder ? field.placeholder : "",
    };

    const options =
      "options" in field && field.options
        ? transformSelectOptions(field.options)
        : undefined;
...
const SystemFieldsSchema = z.object({
  ...
  defaultPlaceholder: z
    .enum(["", "share_additional_notes", "email", "reschedule_placeholder"])
    .optional(),
