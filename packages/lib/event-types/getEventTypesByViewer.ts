...
import { EventTypeRepository } from "@sln/lib/server/repository/eventType";
import { MembershipRepository } from "@sln/lib/server/repository/membership";
import { ProfileRepository } from "@sln/lib/server/repository/profile";
import { UserRepository } from "@sln/lib/server/repository/user";
import { MembershipRole, SchedulingType } from "@sln/prisma/enums";
...
export type EventTypesByViewer = Awaited<
  ReturnType<typeof getEventTypesByViewer>
>;
...
export const getEventTypesByViewer = async (
  ...
  const userEventTypes = (
    await Promise.all(profileEventTypes.map(mapEventType))
  ).filter((eventType) => {
  ...
  const filterByTeamIds = async (
    eventType: Awaited<ReturnType<typeof mapEventType>>
  ) => {
  ...
  const filterBySchedulingTypes = (
    evType: Awaited<ReturnType<typeof mapEventType>>
  ) => {
      ...
      memberships
        ...
        .map(async (membership) => {
          ...
          if (forRoutingForms) {
            ...
            // Once we solve this https://github.com/calcom/oskarhulter.com/issues/12399, we can remove this conditional change in slug
            slug = `team/${team.slug}`;
          } else {
          ...
          const eventTypes = await Promise.all(
            team.eventTypes.map(mapEventType)
          );
          const teamParentMetadata = team.parent
            ? teamMetadataSchema.parse(team.parent.metadata)
            : null;
          return {
            teamId: team.id,
            parentId: team.parentId,
            bookerUrl: getBookerBaseUrlSync(
              team.parent?.slug ?? teamParentMetadata?.requestedSlug ?? null
            ),
            membershipRole:
            ...
            metadata: {
              ...
              readOnly:
                ...
                (team.parentId
                  ? orgMembership &&
                    compareMembership(orgMembership, membership.role)
                    ? orgMembership
...
export function compareMembership(
  mship1: MembershipRole,
  mship2: MembershipRole
) {
