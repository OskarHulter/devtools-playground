...
> & {
  ...
  switch (periodType) {
    case PeriodType.ROLLING: {
      ...
      return {
        rollingEndDay: rollingEndDay.endOf("day"),
        rangeStartDay: null,
        rangeEndDay: null,
      };
    }
    ...
    case PeriodType.RANGE: {
      ...
      const rangeStartDay = dayjs(periodStartDate)
        .utcOffset(utcOffset)
        .startOf("day");
      const rangeEndDay = dayjs(periodEndDate)
        .utcOffset(utcOffset)
        .endOf("day");
...
export function getRollingWindowEndDate({
  ...
  log.debug(
    "called:",
    safeStringify({ startDay: startDate.format(), daysNeeded })
  );
  let counter = 1;
  let rollingEndDay;
  ...
  while (bookableDaysCount < daysNeeded) {
    ...
    const isBookable =
      !!allDatesWithBookabilityStatus[currentDate.format("YYYY-MM-DD")]
        ?.isBookable;
    ...
    currentDate = countNonBusinessDays
      ? currentDate.add(1, "days")
      : currentDate.businessDaysAdd(1);
  ...
  log.debug(
    "Returning rollingEndDay",
    rollingEndDayOrLastPossibleDayAsPerLimit.format()
  );
...
export function isTimeViolatingFutureLimit({
  ...
  if (periodLimits.rangeStartDay && periodLimits.rangeEndDay) {
    return (
      date.isBefore(periodLimits.rangeStartDay) ||
      date.isAfter(periodLimits.rangeEndDay)
    );
  }
  return false;
}
