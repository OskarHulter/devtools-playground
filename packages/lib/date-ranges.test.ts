  ...
  it("should show working hours in correct timezone with existing travel schedules", () => {
    ...
    expect(
      resultWithOriginalTz.every(
        (result) =>
          result.start.utc().hour() === 7 && result.end.utc().hour() === 16
      )
    ).toBeTruthy();
    ...
    expect(
      resultsWithNewYorkTz.every((result) => {
        return (
          result.start.utc().hour() === 13 && result.end.utc().hour() === 22
        );
      })
    ).toBeTruthy();
    ...
    expect(
      resultsWithKolkataTz.every((result) => {
        const correctStartTime =
          result.start.utc().hour() === 2 && result.start.utc().minute() === 30;
        const correctEndTime =
          result.end.utc().hour() === 11 && result.end.utc().minute() === 30;
