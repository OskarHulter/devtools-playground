...
export type CloseComCustomActivityFieldCreate =
  CloseComCustomContactFieldCreate & {
...
export default class CloseCom {
  ...
  constructor(providedApiKey = "") {
    ...
    if (!providedApiKey && !environmentApiKey)
      throw Error("Close.com Api Key not present");
  ...
  public contact = {
    ...
    create: async (data: {
      ...
      return this._post({
        urlPath: "/contact/",
        data: closeComQueries.contact.create(data),
      });
    },
    update: async ({
      contactId,
  ...
  public lead = {
    ...
    update: async (
      leadId: string,
      data: CloseComLead
    ): Promise<CloseComLeadCreateResult> => {
      return this._put({
        urlPath: `/lead/${leadId}`,
        data,
      });
  ...
  private _request = async ({
    ...
    const queryString = query
      ? `?${new URLSearchParams(String(query)).toString()}`
      : "";
