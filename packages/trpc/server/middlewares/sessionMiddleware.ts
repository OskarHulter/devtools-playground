...
export async function getUserFromSession(
  ...
  if (isOrgAdmin) {
    ...
    logger.debug(
      "User is not an org admin",
      safeStringify({ userId: user.id })
    );
  }
  ...
  return {
    ...user,
    avatar:
      `${WEBAPP_URL}/${user.username}/avatar.png?${organization.id}` &&
...
const getSession = async (ctx: TRPCContextInner) => {
  ...
  const { getServerSession } = await import(
    "@sln/features/auth/lib/getServerSession"
  );
  ...
  if (session) {
    ...
    if (!upId) {
      throw new TRPCError({
        ...
        message: "No upId found for session",
      });
    }
    sessionWithUpId = {
      ...session,
