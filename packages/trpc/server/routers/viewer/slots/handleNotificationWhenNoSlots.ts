...
import { sendOrganizationAdminNoSlotsNotification } from "@sln/emails";
import { RedisService } from "@sln/features/redis/RedisService";
import { IS_PRODUCTION, WEBAPP_URL } from "@sln/lib/constants";
...
const constructRedisKey = (eventDetails: EventDetails, orgSlug?: string) => {
  return `${REDIS_KEY_VERSION}.${eventDetails.username}:${
...
export const handleNotificationWhenNoSlots = async ({
  ...
  const usersUniqueKey = constructRedisKey(
    eventDetails,
    orgDetails.currentOrgDomain
  );
  ...
  const usersExistingNoSlots =
    (await redis.lrange(
      usersUniqueKey,
      0,
      NO_SLOTS_COUNT_FOR_NOTIFICATION - 1
    )) ?? [];
