...
import * as CalcomEmails from "@sln/emails";
...
vi.mock("@sln/features/redis/RedisService", () => {
...
vi.mock("@sln/features/flags/server/utils", () => {
...
describe("(Orgs) Send admin notifications when a user has no availability", () => {
  ...
  it("Should send a notification after 2 times if the org has them enabled", async () => {
    ...
    expect(
      CalcomEmails.sendOrganizationAdminNoSlotsNotification
