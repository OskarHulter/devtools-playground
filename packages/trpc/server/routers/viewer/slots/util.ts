...
import CrmManager from "@sln/core/crmManager/crmManager";
import { getAggregatedAvailability } from "@sln/core/getAggregatedAvailability";
import { getBusyTimesForLimitChecks } from "@sln/core/getBusyTimes";
import type {
  CurrentSeats,
  IFromUser,
  IToUser,
} from "@sln/core/getUserAvailability";
...
import {
  ...
  orgDomainConfig,
} from "@sln/ee/organizations/lib/orgDomains";
import { isEventTypeLoggingEnabled } from "@sln/features/bookings/lib/isEventTypeLoggingEnabled";
import { parseBookingLimit, parseDurationLimit } from "@sln/lib";
import { RESERVED_SUBDOMAINS } from "@sln/lib/constants";
import { getUTCOffsetByTimezone } from "@sln/lib/date-fns";
import { getDefaultEvent } from "@sln/lib/defaultEvents";
import {
  isTimeOutOfBounds,
  calculatePeriodLimits,
  isTimeViolatingFutureLimit,
} from "@sln/lib/isOutOfBounds";
import logger from "@sln/lib/logger";
import { safeStringify } from "@sln/lib/safeStringify";
import { performance } from "@sln/lib/server/perfObserver";
import { UserRepository } from "@sln/lib/server/repository/user";
import getSlots from "@sln/lib/slots";
import prisma, { availabilityUserSelect } from "@sln/prisma";
import { PeriodType, Prisma } from "@sln/prisma/client";
...
export async function getDynamicEventType(
  ...
  const { where } =
    await UserRepository._getWhereClauseForFindingUsersByUsername({
...
async function getCRMManagerWithRRLeadSkip(
  apps: z.infer<typeof EventTypeAppMetadataSchema>
) {
...
export async function getAvailableSlots({
  ...
  loggerWithEventDetails.debug(
    `Prisma eventType get took ${
      endPrismaEventTypeGet - startPrismaEventTypeGet
    }ms for event:${input.eventTypeId}`
  );
  const getStartTime = (startTimeInput: string, timeZone?: string) => {
    const startTimeMin = dayjs
      .utc()
      .add(eventType.minimumBookingNotice || 1, "minutes");
    const startTime =
      timeZone === "Etc/GMT"
  ...
  let usersWithCredentials = hosts.map(({ isFixed, user }) => ({
    isFixed,
    ...user,
  }));
  ...
  if (
    ...
    const contactOwnerHost = hosts.find(
      (host) => host.user.email === crmContactOwner
    );
    ...
    if (contactOwnerHost) {
      ...
      usersWithCredentials = usersWithCredentials.filter(
        (user) =>
          user.email !== contactOwnerHost.user.email &&
          (!contactOwnerIsRRHost || user.isFixed)
      );
  ...
  const utcOffset = input.timeZone
  ...
  const withinBoundsSlotsMappedToDate = Object.entries(
    slotsMappedToDate
  ).reduce((withinBoundsSlotsMappedToDate, [date, slots]) => {
  ...
  loggerWithEventDetails.debug(
    `getSlots took ${getSlotsTime}ms and executed ${getSlotsCount} times`
  );
  ...
  loggerWithEventDetails.debug(
    `Available slots: ${JSON.stringify(withinBoundsSlotsMappedToDate)}`
  );
