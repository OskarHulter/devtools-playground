import { compareMembership } from "@sln/lib/event-types/getEventTypesByViewer";
import { getBookerBaseUrl } from "@sln/lib/getBookerUrl/server";
import { prisma } from "@sln/prisma";
import type { Webhook } from "@sln/prisma/client";
...
export const getByViewerHandler = async ({ ctx }: GetByViewerOptions) => {
  ...
  const bookerUrl = await getBookerBaseUrl(
    ctx.user.profile?.organizationId ?? null
  );

  const image = user?.username
    ? `${bookerUrl}/${user.username}/avatar.png`
    : undefined;
  webhookGroups.push({
    teamId: null,
    profile: {
