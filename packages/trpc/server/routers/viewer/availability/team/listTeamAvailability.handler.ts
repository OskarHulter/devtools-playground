...
import { buildDateRanges } from "@sln/lib/date-ranges";
...
async function getInfoForAllTeams({ ctx, input }: GetOptions) {
  ...
  if (!teamIds.length) {
    throw new TRPCError({
      code: "NOT_FOUND",
      message: "User is not part of any organization or team.",
    });
  }
...
export const listTeamAvailabilityHandler = async ({
  ctx,
  input,
}: GetOptions) => {
  const { cursor, limit } = input;
