...
import { ENABLE_PROFILE_SWITCHER, WEBAPP_URL } from "@sln/lib/constants";
...
import { isTeamAdmin } from "@sln/lib/server/queries";
...
import { getParsedTeam } from "@sln/lib/server/repository/teamUtils";
...
import type { Profile as ProfileType } from "@sln/prisma/client";
...
type ExistingUserWithInviteStatus = Awaited<
...
export async function getUniqueInvitationsOrThrowIfEmpty(
...
export function canBeInvited(
  invitee: UserWithMembership,
  team: TeamWithParent
) {
  const myLog = log.getSubLogger({ prefix: ["canBeInvited"] });
  myLog.debug(
    "Checking if user can be invited",
    safeStringify({ invitee, team })
  );
  const alreadyInvited = invitee.teams?.find(
    ({ teamId: membershipTeamId }) => team.id === membershipTeamId
  );
  if (alreadyInvited) {
    return INVITE_STATUS.USER_ALREADY_INVITED_OR_MEMBER;
  }

  const orgMembership = invitee.teams?.find(
    (membership) => membership.teamId === team.parentId
  );
...
export async function findUsersWithInviteStatus({
  ...
  const usernamesOrEmails = invitations.map(
    (invitation) => invitation.usernameOrEmail
  );
  ...
  return inviteesFromDb.map((inviteeFromDb) => {
    const newRole = getRoleForUser({
      email: inviteeFromDb.email,
      username: inviteeFromDb.username,
    });
  ...
  function getRoleForUser({
    ...
    return (
      userToRoleMap.get(email) ||
      (username ? userToRoleMap.get(username) : defaultMemberRole)
    );
  }
}
...
export async function createNewUsersConnectToOrgIfExists({
  ...
  orgConnectInfoByUsernameOrEmail: Record<
    string,
    ReturnType<typeof getOrgConnectionInfo>
  >;
  ...
  invitations.forEach((invitation) =>
    checkInputEmailIsValid(invitation.usernameOrEmail)
  );
  ...
  const createdUsers = await prisma.$transaction(
    async (tx) => {
      ...
      for (let index = 0; index < invitations.length; index++) {
        ...
        const { orgId, autoAccept } =
          orgConnectInfoByUsernameOrEmail[invitation.usernameOrEmail];
        ...
        const createdUser = await tx.user.create({
          data: {
            username: isBecomingAnOrgMember
              ? orgMemberUsername
              : regularTeamMemberUsername,
...
export async function createMemberships({
  ...
  log.debug(
    "Creating memberships for",
    safeStringify({ input, invitees, parentId, accepted })
  );
  try {
...
export const sendExistingUserTeamInviteEmails = async ({
  ...
  language: TFunction;
  isAutoJoin: boolean;
  existingUsersWithMemberships: Omit<
    ExistingUserWithInviteStatusAndProfile,
    "canBeInvited" | "newRole"
  >[];
  ...
  const sendEmailsPromises = existingUsersWithMemberships.map(async (user) => {
    ...
    log.debug(
      ...
      safeStringify({ user, currentUserName, currentUserTeamName })
    );
    ...
    if (currentUserTeamName) {
      ...
      return sendTeamInviteEmail({
        ...
        newLink: user.profile
          ? `${getOrgFullOrigin(orgSlug ?? "")}/${user.profile.username}`
          : null,
      });
    }
  });
...
export async function handleExistingUsersInvites({
  ...
  orgConnectInfoByUsernameOrEmail: Record<
    string,
    { orgId: number | undefined; autoAccept: boolean }
  >;
    ...
    const existingUsersWithMembershipsNew = await Promise.all(
      invitableExistingUsers.map(async (user) => {
        const shouldAutoAccept =
          orgConnectInfoByUsernameOrEmail[user.email].autoAccept;
        let profile = null;
        if (shouldAutoAccept) {
