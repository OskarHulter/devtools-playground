...
import { default as paymentsMock } from "@sln/features/ee/teams/lib/__mocks__/payments";
import { constantsScenarios } from "@sln/lib/__mocks__/constants";
import type { Profile } from "@sln/prisma/client";
...
describe("inviteMemberHandler", () => {
  ...
  describe("Regular Team", () => {
    describe("with 2 emails in input and when there are no users matching the emails", () => {
      it("should call appropriate utilities to send email, add users and update in stripe. It should return `numUsersInvited`=2", async () => {
        ...
        const retValueOfGetTeamOrThrowError =
        ...
        expect(
          inviteMemberUtilsMock.handleNewUsersInvites
        ...
        ).toHaveBeenCalledWith(input.teamId);

        expect(
          inviteMemberUtilsMock.handleExistingUsersInvites
        ).not.toHaveBeenCalled();

        expect(
          inviteMemberUtilsMock.getUniqueInvitationsOrThrowIfEmpty
        ).toHaveBeenCalledWith([
          {
            role: input.role,
            usernameOrEmail: usersToBeInvited[0].email,
  ...
  it("When rate limit exceeded, it should throw error", async () => {
    ...
    const errorMessageForRateLimit =
      checkRateLimitAndThrowErrorScenarios.fakeRateLimitFailed();
