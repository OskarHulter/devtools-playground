...
import { generateTeamCheckoutSession } from "@sln/features/ee/teams/lib/payments";
...
const generateCheckoutSession = async ({
  ...
  if (!IS_TEAM_BILLING_ENABLED) {
    console.info(
      "Team billing is disabled, not generating a checkout session."
    );
    return;
  }
...
export const createHandler = async ({ ctx, input }: CreateOptions) => {
  ...
  if (user.profile?.organizationId && !user.organization.isOrgAdmin) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "org_admins_can_create_new_teams",
    });
  }
