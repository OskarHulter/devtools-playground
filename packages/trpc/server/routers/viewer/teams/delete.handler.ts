...
import { cancelTeamSubscriptionFromStripe } from "@sln/features/ee/teams/lib/payments";
import { IS_TEAM_BILLING_ENABLED } from "@sln/lib/constants";
import { deleteDomain } from "@sln/lib/domainManager/organization";
import { isTeamOwner } from "@sln/lib/server/queries/teams";
import { closeComDeleteTeam } from "@sln/lib/sync/SyncServiceManager";
...
export const deleteHandler = async ({ ctx, input }: DeleteOptions) => {
  ...
  if (IS_TEAM_BILLING_ENABLED)
    await cancelTeamSubscriptionFromStripe(input.teamId);
