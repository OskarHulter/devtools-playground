...
import { isTeamMember } from "@sln/lib/server/queries/teams";
import { availabilityUserSelect } from "@sln/prisma";
...
export const getMemberAvailabilityHandler = async ({
  ctx,
  input,
}: GetMemberAvailabilityOptions) => {
  ...
  if (!member.user.username)
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Member doesn't have a username",
    });
