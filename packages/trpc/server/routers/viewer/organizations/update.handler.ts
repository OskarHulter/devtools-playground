...
import { getMetadataHelpers } from "@sln/lib/getMetadataHelpers";
import { uploadLogo } from "@sln/lib/server/avatar";
...
export const updateHandler = async ({ ctx, input }: UpdateOptions) => {
  ...
  const isUserOrganizationAdmin =
    currentOrgId && (await isOrganisationAdmin(ctx.user?.id, currentOrgId));
  const isUserRoleAdmin = ctx.user.role === UserPermissionRole.ADMIN;

  const isUserAuthorizedToUpdate = !!(
    isUserOrganizationAdmin || isUserRoleAdmin
  );

  if (!currentOrgId || !isUserAuthorizedToUpdate)
  ...
  if (prevOrganisation)
    closeComUpdateTeam(prevOrganisation, updatedOrganisation);
