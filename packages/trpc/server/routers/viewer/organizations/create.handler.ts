...
import { createDomain } from "@sln/lib/domainManager/organization";
import { getTranslation } from "@sln/lib/server/i18n";
import { OrganizationRepository } from "@sln/lib/server/repository/organization";
...
export const createHandler = async ({ input, ctx }: CreateOptions) => {
  ...
  const billingPeriod =
    (IS_USER_ADMIN ? billingPeriodRaw : BillingPeriod.MONTHLY) ??
  ...
  if (
    ...
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "You need to have minimum published teams.",
    });
  }
  ...
  const loggedInUserTranslation = await getTranslation(
    ctx.user.locale,
    "common"
  );
  const inputLanguageTranslation = await getTranslation(
    input.language ?? "en",
    "common"
  );
  ...
  if (!isOrganizationConfigured) {
    ...
    if (instanceAdmins.length) {
      ...
      console.warn(
        "Organization created: subdomain not configured and couldn't notify adminnistrators"
      );
    }
  }
  ...
  if (!orgOwner) {
    ...
    if (!isPlatform) {
      await sendOrganizationCreationEmail({
        ...
        prevLink: null,
        newLink: `${getOrgFullOrigin(slug, { protocol: true })}/${
    ...
    if (ctx.user.profile.organizationId && isLoggedInUserOrgOwner) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "User is part of an organization already",
      });
    }

    const nonOrgUsernameForOwner = orgOwner.username || "";
    const { organization, ownerProfile } =
      await OrganizationRepository.createWithExistingUserAsOwner({
        orgData,
        owner: {
          id: orgOwner.id,
          email: orgOwnerEmail,
