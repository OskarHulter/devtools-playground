...
import { fetcher } from "@sln/lib/retellAIFetcher";
import type { PrismaClient } from "@sln/prisma";
import { getRetellLLMSchema } from "@sln/prisma/zod-utils";
import type { AIPhoneSettingSchema } from "@sln/prisma/zod-utils";
...
const createPhoneCallHandler = async ({ input, ctx }: CreatePhoneCallProps) => {
  ...
  if (!aiPhoneCallConfig.llmId) {
    ...
    const retellLLM = await fetcher(
      `/get-retell-llm/${aiPhoneCallConfig.llmId}`
    ).then(getRetellLLMSchema.parse);

    if (
      retellLLM.general_prompt !== generalPrompt ||
      retellLLM.begin_message !== beginMessage
    ) {
      const updatedRetellLLM = await fetcher(
        `/update-retell-llm/${aiPhoneCallConfig.llmId}`,
        {
          method: "PATCH",
          body: JSON.stringify({
            general_prompt: generalPrompt,
  ...
  if (llmWebSocketUrlToBeUpdated) {
    const getPhoneNumberDetails = await fetcher(
      `/get-phone-number/${yourPhoneNumber}`
    ).then(getPhoneNumberSchema.parse);

    const updated = await fetcher(
      `/update-agent/${getPhoneNumberDetails.agent_id}`,
      {
        method: "PATCH",
        body: JSON.stringify({
