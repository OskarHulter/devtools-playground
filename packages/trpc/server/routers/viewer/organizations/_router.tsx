...
import { ZVerifyCodeInputSchema } from "@sln/prisma/zod-utils";
import { AIPhoneSettingSchema } from "@sln/prisma/zod-utils";
...
export const viewerOrganizationsRouter = router({
  ...
  verifyCode: authedProcedure
    ...
    .mutation(async (opts) => {
      const handler = await importHandler(
        namespaced("verifyCode"),
        () => import("./verifyCode.handler")
      );
      return handler(opts);
    }),
  createTeams: authedProcedure
    ...
    .mutation(async (opts) => {
      const handler = await importHandler(
        namespaced("createTeams"),
        () => import("./createTeams.handler")
      );
      return handler(opts);
    }),
  ...
  setPassword: authedProcedure
    ...
    .mutation(async (opts) => {
      const handler = await importHandler(
        namespaced("setPassword"),
        () => import("./setPassword.handler")
      );
      return handler(opts);
    }),
  getMembers: authedProcedure.input(ZGetMembersInput).query(async (opts) => {
    const handler = await importHandler(
      namespaced("getMembers"),
      () => import("./getMembers.handler")
    );
    return handler(opts);
  }),
  listMembers: authedProcedure.input(ZListMembersSchema).query(async (opts) => {
    const handler = await importHandler(
      namespaced("listMembers"),
      () => import("./listMembers.handler")
    );
    return handler(opts);
  }),
  ...
  updateUser: authedProcedure
    ...
    .mutation(async (opts) => {
      const handler = await importHandler(
        namespaced("updateUser"),
        () => import("./updateUser.handler")
      );
      return handler(opts);
    }),
  ...
  getOtherTeam: authedOrgAdminProcedure
    ...
    .query(async (opts) => {
      const handler = await importHandler(
        namespaced("getOtherTeam"),
        () => import("./getOtherTeam.handler")
      );
      return handler(opts);
    }),
  ...
  deleteTeam: authedOrgAdminProcedure
    ...
    .mutation(async (opts) => {
      const handler = await importHandler(
        namespaced("deleteTeam"),
        () => import("./deleteTeam.handler")
      );
      return handler(opts);
    }),
  ...
  adminGetAll: authedAdminProcedure.query(async (opts) => {
    const handler = await importHandler(
      namespaced("adminGetAll"),
      () => import("./adminGetAll.handler")
    );
    return handler(opts);
  }),
  ...
  adminUpdate: authedAdminProcedure
    ...
    .mutation(async (opts) => {
      const handler = await importHandler(
        namespaced("adminUpdate"),
        () => import("./adminUpdate.handler")
      );
      return handler(opts);
    }),
  adminVerify: authedAdminProcedure
    ...
    .mutation(async (opts) => {
      const handler = await importHandler(
        namespaced("adminVerify"),
        () => import("./adminVerify.handler")
      );
      return handler(opts);
    }),
  adminDelete: authedAdminProcedure
    ...
    .mutation(async (opts) => {
      const handler = await importHandler(
        namespaced("adminDelete"),
        () => import("./adminDelete.handler")
      );
      return handler(opts);
    }),
