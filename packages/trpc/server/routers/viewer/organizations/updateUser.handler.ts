...
import { checkRegularUsername } from "@sln/lib/server/checkRegularUsername";
import {
  isOrganisationAdmin,
  isOrganisationOwner,
} from "@sln/lib/server/queries/organisations";
...
const applyRoleToAllTeams = async (
  userId: number,
  teamIds: number[],
  role: MembershipRole
) => {
...
export const updateUserHandler = async ({ ctx, input }: UpdateUserOptions) => {
  ...
  if (!organizationId)
    throw new TRPCError({
      code: "UNAUTHORIZED",
      message: "You must be a memeber of an organizaiton",
    });

  if (!(await isOrganisationAdmin(userId, organizationId)))
  ...
  const isUserBeingUpdatedOwner = await isOrganisationOwner(
    input.userId,
    organizationId
  );
  ...
  if (input.username && hasUsernameUpdated && user.profile.organization?.slug) {
    ...
    if (!checkRegularUsernameRes.available) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: checkRegularUsernameRes.message,
      });
    }
  }
  ...
  if (
    ...
    const teamIds = requestedMember.team.children
      .map(
        (sub_team) =>
          sub_team.members.find((item) => item.userId === input.userId)?.teamId
      )
