...
import { MembershipRole, RedirectType } from "@sln/prisma/enums";
import { teamMetadataSchema } from "@sln/prisma/zod-utils";
import { TRPCError } from "@trpc/server";

const log = logger.getSubLogger({
  prefix: ["viewer/organizations/createTeams.handler"],
});
...
async function moveTeam({
  ...
  log.debug(
    "Moving team",
    safeStringify({ teamId, newSlug, org, oldSlug: team.slug })
  );
  ...
  const invitableMembers = team.members
    .filter(isMembershipNotWithOwner)
    .map((membership) => ({
...
function getSubscriptionId(metadata: Prisma.JsonValue) {
  ...
  if (parsedMetadata.success) {
    ...
    if (!subscriptionId) {
      log.warn(
        "No subscriptionId found in team metadata",
        safeStringify({ metadata, parsedMetadata })
      );
    }
