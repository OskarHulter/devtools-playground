...
import { deleteDomain } from "@sln/lib/domainManager/organization";
...
export const adminDeleteHandler = async ({ input }: AdminDeleteOption) => {
  ...
  if (foundOrg.slug) {
    try {
      ...
      log.error(
        `Failed to delete domain ${foundOrg.slug}. Do a manual deletion if needed`
      );
    }
  }

  await deleteAllRedirectsForUsers(
    foundOrg.members.map((member) => member.user)
  );

  await renameUsersToAvoidUsernameConflicts(
    foundOrg.members.map((member) => member.user)
  );
...
async function renameUsersToAvoidUsernameConflicts(
  ...
  for (const user of users) {
    ...
    if (!currentUsername) {
      ...
      log.warn(
        `User ${user.id} has no username, defaulting to ${currentUsername}`
      );
    }
...
async function deleteAllRedirectsForUsers(
