...
export const eventTypesRouter = router({
  ...
  listWithTeam: authedProcedure.query(async ({ ctx }) => {
    if (!UNSTABLE_HANDLER_CACHE.listWithTeam) {
      UNSTABLE_HANDLER_CACHE.listWithTeam = await import(
        "./listWithTeam.handler"
      ).then((mod) => mod.listWithTeamHandler);
    }
  ...
  duplicate: eventOwnerProcedure
    .input(ZDuplicateInputSchema)
    .mutation(async ({ ctx, input }) => {
      if (!UNSTABLE_HANDLER_CACHE.duplicate) {
        UNSTABLE_HANDLER_CACHE.duplicate = await import(
          "./duplicate.handler"
        ).then((mod) => mod.duplicateHandler);
      }
  ...
  bulkEventFetch: authedProcedure.query(async ({ ctx }) => {
    if (!UNSTABLE_HANDLER_CACHE.bulkEventFetch) {
      UNSTABLE_HANDLER_CACHE.bulkEventFetch = await import(
        "./bulkEventFetch.handler"
      ).then((mod) => mod.bulkEventFetchHandler);
    }
