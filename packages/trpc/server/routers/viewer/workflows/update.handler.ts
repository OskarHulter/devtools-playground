...
import { isSMSOrWhatsappAction } from "@sln/features/ee/workflows/lib/actionHelperFunctions";
...
export const updateHandler = async ({ ctx, input }: UpdateOptions) => {
  const { user } = ctx;
  const { id, name, activeOn, steps, trigger, time, timeUnit, isActiveOnAll } =
    input;
  ...
  if (!isOrg) {
    ...
    if (userWorkflow.isActiveOnAll) {
      ...
      oldActiveOnEventTypes = (
        ...
        return {
          id: eventTypeRel.eventTypeId,
          children: eventTypeRel.eventType.children,
        };
      });
    }
    ...
    newActiveOn = activeOn.filter(
      (eventTypeId) => !oldActiveOnIds.includes(eventTypeId)
    );
    ...
    removedActiveOnIds = oldActiveOnIds.filter(
    ...
    removedActiveOnIds = oldActiveOnIds.filter(
      (teamId) => !activeOn.includes(teamId)
    );
    ...
    await deleteRemindersOfActiveOnIds({
      ...
      workflowSteps: userWorkflow.steps,
      isOrg,
      activeOnIds: activeOn.filter(
        (activeOn) => !newActiveOn.includes(activeOn)
      ),
    });
  ...
  userWorkflow.steps.map(async (oldStep) => {
    ...
    if (!newStep) {
      ...
      if (
        ...
        throw new TRPCError({
          code: "UNAUTHORIZED",
          message: "Not available on free plan",
        });
      }
  ...
  if (!smsReminderNumberNeeded) {
    ...
    await upsertSmsReminderFieldForEventTypes({
      ...
      isSmsReminderNumberRequired: steps.some(
        (s) =>
          (s.action === WorkflowActions.SMS_ATTENDEE ||
            s.action === WorkflowActions.WHATSAPP_ATTENDEE) &&
          s.numberRequired
      ),
      isOrg,
    });
