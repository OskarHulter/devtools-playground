...
import { scheduleSMSReminder } from "@sln/features/ee/workflows/lib/reminders/smsReminderManager";
import { scheduleWhatsappReminder } from "@sln/features/ee/workflows/lib/reminders/whatsappReminderManager";
import { getTimeFormatStringFromUserTimeFormat } from "@sln/lib/timeFormat";
import { prisma } from "@sln/prisma";
import { BookingStatus } from "@sln/prisma/client";
import { MembershipRole, WorkflowActions } from "@sln/prisma/enums";
...
export const activateEventTypeHandler = async ({
  ctx,
  input,
}: ActivateEventTypeOptions) => {
  ...
  if (!userEventType)
    throw new TRPCError({
      code: "UNAUTHORIZED",
      message: "Not authorized to edit this event type",
    });
  ...
  const activeOn = [eventTypeId].concat(
    userEventType.children.map((ch) => ch.id)
  );
  ...
  if (isActive) {
        ...
        allEventTypes = allEventTypesWithLocked.filter(
          (eventType) =>
            ...
            EventTypeMetaDataSchema.parse(eventType.metadata)
              ?.managedEventConfig?.unlockedFields?.workflows
        );
      }
    ...
    await prisma.workflowsOnEventTypes.createMany({
      data: [
        ...
        userEventType.children.map((ch) => ({ workflowId, eventTypeId: ch.id }))
      ),
    });
    ...
    if (
      eventTypeWorkflow.steps.some((step) =>
        requiresAttendeeNumber(step.action)
      )
    ) {
      const isSmsReminderNumberRequired = eventTypeWorkflow.steps.some(
        (step) => {
