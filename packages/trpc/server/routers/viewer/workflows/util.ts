...
import { isSMSOrWhatsappAction } from "@sln/ee/workflows/lib/actionHelperFunctions";
import { getAllWorkflows } from "@sln/ee/workflows/lib/getAllWorkflows";
import {
  deleteScheduledEmailReminder,
  scheduleEmailReminder,
} from "@sln/ee/workflows/lib/reminders/emailReminderManager";
import {
  deleteScheduledSMSReminder,
  scheduleSMSReminder,
} from "@sln/ee/workflows/lib/reminders/smsReminderManager";
import {
  deleteScheduledWhatsappReminder,
  scheduleWhatsappReminder,
} from "@sln/ee/workflows/lib/reminders/whatsappReminderManager";
import type { Workflow as WorkflowType } from "@sln/ee/workflows/lib/types";
import { SMS_REMINDER_NUMBER_FIELD } from "@sln/features/bookings/lib/SystemField";
import {
  getSmsReminderNumberField,
  getSmsReminderNumberSource,
} from "@sln/features/bookings/lib/getBookingFields";
import {
  removeBookingField,
  upsertBookingField,
} from "@sln/features/eventtypes/lib/bookingFieldsManager";
import { SENDER_ID, SENDER_NAME } from "@sln/lib/constants";
...
import type { Prisma, WorkflowStep } from "@sln/prisma/client";
import type { TimeUnit } from "@sln/prisma/enums";
import {
  BookingStatus,
  MembershipRole,
  WorkflowActions,
...
export function getSender(
  ...
  return isSMSOrWhatsappAction(step.action)
...
export async function removeSmsReminderFieldForEventTypes({
  ...
  if (isOrg) {
    allEventTypeIds = await getAllUserAndTeamEventTypes(
      activeOnToRemove,
      activeOn
    );
  }
    ...
    if (isOrg) {
      ...
      if (newEventType) {
        ...
        if (
          ...
          !newEventType?.users.find(
            (eventTypeUser) => eventTypeUser.id === userId
          )
        ) {
          return false;
        }
...
const reminderMethods: {
  ...
  [WorkflowMethods.EMAIL]: (id, referenceId) =>
    deleteScheduledEmailReminder(id, referenceId),
  [WorkflowMethods.SMS]: (id, referenceId) =>
    deleteScheduledSMSReminder(id, referenceId),
  [WorkflowMethods.WHATSAPP]: (id, referenceId) =>
    deleteScheduledWhatsappReminder(id, referenceId),
};
...
export async function deleteAllWorkflowReminders(
  ...
  const results = await Promise.allSettled(
    remindersToDelete.map((reminder) => {
      return reminderMethods[reminder.method](
        reminder.id,
...
export async function deleteRemindersOfActiveOnIds({
  ...
  const remindersToDelete = !isOrg
    ...
    : await getRemindersFromRemovedTeams(
        removedActiveOnIds,
        workflowSteps,
        activeOnIds
      );
...
async function getRemindersFromRemovedTeams(
  ...
  const remindersToDelete = (
    await Promise.all(remindersToDeletePromise)
  ).flat();
...
export function isStepEdited(oldStep: WorkflowStep, newStep: WorkflowStep) {
  ...
  for (const key of oldStepKeys) {
    if (
      oldStep[key as keyof WorkflowStep] !== newStep[key as keyof WorkflowStep]
    ) {
      return true;
    }
  }
...
export async function getAllWorkflowsFromEventType(
  ...
  const eventTypeMetadata = EventTypeMetaDataSchema.parse(
    eventType?.metadata || {}
  );
