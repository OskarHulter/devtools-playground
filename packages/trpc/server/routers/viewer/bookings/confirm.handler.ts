...
import { getLocationValueForDB } from "@sln/app-store/locations";
import type { LocationObject } from "@sln/app-store/locations";
import { workflowSelect } from "@sln/ee/workflows/lib/getAllWorkflows";
import { sendDeclinedEmails } from "@sln/emails";
import { getCalEventResponses } from "@sln/features/bookings/lib/getCalEventResponses";
import { handleConfirmation } from "@sln/features/bookings/lib/handleConfirmation";
...
export const confirmHandler = async ({ ctx, input }: ConfirmOptions) => {
  ...
  if (booking.userId !== user.id && booking.eventTypeId) {
    ...
    if (
      eventType &&
      !eventType.users.find((user) => booking.userId === user.id)
    ) {
  ...
  if (confirmed) {
    ...
    if (recurringEventId) {
      ...
      if (!!booking.payment.length) {
        const successPayment = booking.payment.find(
        ...
        if (!successPayment) {
          ...
          if (!eventTypeOwnerId) {
            throw new Error(
              "Event Type owner not found for obtaining payment app credentials"
            );
          }
          ...
          const paymentAppCredential = paymentAppCredentials.find(
            (credential) => {
          ...
          if (!paymentApp?.lib?.PaymentService) {
            console.warn(
              `payment App service of type ${paymentApp} is not implemented`
            );
            return null;
          }
          ...
          const paymentInstance = new PaymentService(
            paymentAppCredential
          ) as IAbstractPaymentService;
