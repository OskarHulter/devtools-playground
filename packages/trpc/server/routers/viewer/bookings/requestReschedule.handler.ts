...
import { CalendarEventBuilder } from "@sln/core/builders/CalendarEvent/builder";
import { CalendarEventDirector } from "@sln/core/builders/CalendarEvent/director";
import { deleteMeeting } from "@sln/core/videoClient";
import dayjs from "@sln/dayjs";
import { sendRequestRescheduleEmail } from "@sln/emails";
...
export const requestRescheduleHandler = async ({
  ...
  webhookPromises.push(
    deleteWebhookScheduledTriggers({ booking: bookingToReschedule })
  );

  await Promise.all(webhookPromises).catch((error) => {
    log.error(
      "Error while deleting scheduled webhook triggers",
  ...
  const bookingRefsFiltered: BookingReference[] =
    bookingToReschedule.references.filter((ref) =>
    ...
    bookingRefsFiltered.map(async (bookingRef) => {
      ...
      if (bookingRef.type.endsWith("_calendar")) {
        const calendar = await getCalendar(
          credentials.find((cred) => cred.id === bookingRef?.credentialId) ||
            null
        );
  ...
  const evt: CalendarEvent = {
    ...
    startTime: bookingToReschedule?.startTime
      ? dayjs(bookingToReschedule.startTime).format()
      : "",
    endTime: bookingToReschedule?.endTime
      ? dayjs(bookingToReschedule.endTime).format()
      : "",
    organizer,
    attendees: usersToPeopleType(
      // username field doesn't exists on attendee but could be in the future
  ...
  const triggerForUser =
    !teamId || (teamId && bookingToReschedule.eventType?.parentId);
