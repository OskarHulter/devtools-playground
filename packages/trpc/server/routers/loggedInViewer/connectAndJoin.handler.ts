...
export const Handler = async ({ ctx, input }: Options) => {
  ...
  if (!isLoggedInUserPartOfOrg) {
    throw new TRPCError({
      code: "UNAUTHORIZED",
      message: "Logged in user is not member of Organization",
    });
  }
  ...
  if (!locationVideoCallUrl) {
    throw new TRPCError({
      ...
      message: "meeting_url_not_found",
    });
  }

  const videoCallReference = updatedBooking.references.find((reference) =>
  ...
  const attendeesListPromises = updatedBooking.attendees.map(
    async (attendee) => {
  ...
  return {
    isBookingAlreadyAcceptedBySomeoneElse,
    meetingUrl: locationVideoCallUrl,
  };
};
