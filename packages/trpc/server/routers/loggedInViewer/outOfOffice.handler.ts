...
import { sendBookingRedirectNotification } from "@sln/emails";
...
export const outOfOfficeCreate = async ({ ctx, input }: TBookingRedirect) => {
  ...
  if (!startDate || !endDate) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "start_date_and_end_date_required",
    });
  }
  ...
  if (
    ...
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "start_date_must_be_in_the_future",
    });
  }
  ...
  if (outOfOfficeEntry) {
    throw new TRPCError({
      code: "CONFLICT",
      message: "out_of_office_entry_already_exists",
    });
  }
  ...
  if (existingOutOfOfficeEntry) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "booking_redirect_infinite_not_allowed",
    });
  }
...
export const outOfOfficeEntryDelete = async ({
  ctx,
  input,
}: TBookingRedirectDelete) => {
  if (!input.outOfOfficeUid) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "out_of_office_id_required",
    });
  }
  ...
  if (!deletedOutOfOfficeEntry) {
    throw new TRPCError({
      code: "NOT_FOUND",
      message: "booking_redirect_not_found",
    });
  }
...
export const outOfOfficeEntriesList = async ({
  ctx,
}: {
