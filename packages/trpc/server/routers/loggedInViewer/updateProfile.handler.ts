...
import {
  updateUserMetadataAllowedKeys,
  type TUpdateProfileInputSchema,
} from "./updateProfile.schema";
import { Prisma } from "@prisma/client";
import stripe from "@sln/app-store/stripepayment/lib/server";
import { getPremiumMonthlyPlanPriceId } from "@sln/app-store/stripepayment/lib/utils";
import { sendChangeOfEmailVerification } from "@sln/features/auth/lib/verifyEmail";
...
import { uploadAvatar } from "@sln/lib/server/avatar";
import { checkUsername } from "@sln/lib/server/checkUsername";
...
import { validateBookerLayouts } from "@sln/lib/validateBookerLayouts";
...
export const updateProfileHandler = async ({
  ...
  const layoutError = validateBookerLayouts(
    input?.metadata?.defaultBookerLayouts || null
  );
  if (layoutError) {
  ...
  if (input.username && !user.organizationId) {
    ...
    if (username !== user.username) {
      ...
      if (!response.available) {
        ...
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: t("username_already_taken"),
        });
      }
    }
    ...
    const isPremiumUsernameSubscriptionActive = stripeSubscriptions.data.some(
      (subscription) =>
        subscription.items.data[0].price.id ===
          getPremiumMonthlyPlanPriceId() && subscription.status === "active"
    );
    ...
    const schedulesToDelete = existingSchedules.filter(
      (schedule) =>
        ...
        !travelSchedules.find(
          (scheduleInput) => scheduleInput.id === schedule.id
        )
    );
  ...
  if (
    ...
    hasKeyInMetadata(updatedUser, "stripeCustomerId")
  ) {
  ...
  if (secondaryEmails.length) {
    ...
    const modifiedRecords = secondaryEmails.filter(
      (secondaryEmail) => !secondaryEmail.isDeleted
    );
    if (modifiedRecords.length) {
      ...
      const recordsToModifyQueue = modifiedRecords.map((updated) => {
        let emailVerified =
          keyedSecondaryEmailsFromDB[updated.id].emailVerified;
        if (secondaryEmail?.id === updated.id) {
          emailVerified = primaryEmailVerified;
        } else if (
          updated.email !== keyedSecondaryEmailsFromDB[updated.id].email
        ) {
          emailVerified = null;
        }
  ...
  return {
    ...input,
    email:
      emailVerification && !secondaryEmail?.emailVerified
        ? user.email
        : input.email,
...
const cleanMetadataAllowedUpdateKeys = (
  metadata: TUpdateProfileInputSchema["metadata"]
) => {
  if (!metadata) {
    return {};
  }
