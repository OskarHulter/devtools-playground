...
export const AverageEventDurationChart = () => {
  ...
  const { data, isSuccess, isPending } =
    trpc.viewer.insights.averageEventDuration.useQuery(
      {
  ...
  if (!isSuccess || !startDate || !endDate || (!teamId && !selectedUserId))
    return null;
  const isNoData =
    (data && data.length === 0) || data.every((item) => item["Average"] === 0);
  return (
    <CardInsights>
