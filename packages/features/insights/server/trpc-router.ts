...
import { rawDataInputSchema } from "@sln/features/insights/server/raw-data.schema";
...
>(
  ...
  const userHashMap = new Map<
    number | null,
    Omit<T, "avatarUrl"> & { avatarUrl: string }
  >();
...
const userBelongsToTeamProcedure = authedProcedure.use(
  async ({ ctx, next, getRawInput }) => {
    ...
    if (
      (parse.data.isAll && ctx.user.organizationId) ||
...
export const insightsRouter = router({
  eventsByStatus: userBelongsToTeamProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (
        isAll &&
        ctx.user.isOwnerAdminOfParentTeam &&
      ...
      const lastPeriodBaseBookingsCount =
        lastPeriodCountGroupedByStatus["_all"];
      const lastPeriodTotalRescheduled =
        lastPeriodCountGroupedByStatus["rescheduled"];
      const lastPeriodTotalCancelled =
        lastPeriodCountGroupedByStatus["cancelled"];

      const lastPeriodAverageRating = lastPeriodTotalRatingsAggregate._avg
        .rating
  ...
  eventsTimeline: userBelongsToTeamProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // Get timeline data
      const timeline = await EventsInsights.getTimeLine(
        timeView,
        dayjs(startDate),
        dayjs(endDate)
      );
  ...
  popularEventTypes: userBelongsToTeamProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const bookingsFromSelected =
        await ctx.insightsDb.bookingTimeStatus.groupBy({
          by: ["eventTypeId"],
          where: bookingWhere,
          _count: {
            id: true,
      ...
      const result = bookingsFromSelected.map((booking) => {
        const eventTypeSelected = eventTypeHashMap.get(
  ...
  averageEventDuration: userBelongsToTeamProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      for (const date of timeLine) {
        ...
        const bookingsInTimeRange =
          await ctx.insightsDb.bookingTimeStatus.findMany({
            select: {
              eventLength: true,
            },
            where: {
  ...
  rawData: userBelongsToTeamProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        const downloadAs = `Insights-${dayjs(startDate).format(
          "YYYY-MM-DD"
        )}-${dayjs(endDate).format("YYYY-MM-DD")}-${randomString(10)}.csv`;
