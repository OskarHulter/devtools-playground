...
import { readonlyPrisma as prisma } from "@sln/prisma";
...
class EventsInsights {
  static countGroupedByStatus = async (
  ...
  static getTotalCSAT = async (
    ...
    const satisfactoryResponses = result.filter(
      (item) => item.rating && item.rating > 3
    ).length;
    const csat =
      totalResponses > 0 ? (satisfactoryResponses / totalResponses) * 100 : 0;
