...
import { AnimatedPopover, Avatar } from "@sln/ui";
...
export const UserListInTeam = () => {
  ...
  const { selectedFilter, selectedTeamId, selectedMemberUserId, isAll } =
    filter;
  ...
  const selectedTeamUser = data?.find(
    (item) => item.id === selectedMemberUserId
  );
