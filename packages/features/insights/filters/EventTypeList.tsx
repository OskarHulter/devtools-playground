...
import { FilterCheckboxFieldsContainer } from "@sln/features/filters/components/TeamsFilter";
import { useLocale } from "@sln/lib/hooks/useLocale";
import type { RouterOutputs } from "@sln/trpc";
import { trpc } from "@sln/trpc";
import { AnimatedPopover, CheckboxField } from "@sln/ui";
...
export const EventTypeList = memo(() => {
  ...
  const selectedEventType = data?.find(
    (item) => item.id === selectedEventTypeId
  );
  const eventTypeValue = selectedEventType
    ? mapEventTypeToOption(selectedEventType)
    : null;
        ...
        {filterOptions?.map((eventType) => (
          ...
          >
            <CheckboxField
              ...
              onChange={(e) => {
                if (e.target.checked) {
                  const selectedEventTypeId = data.find(
                    (item) => item.id.toString() === eventType.value
                  )?.id;
                  !!selectedEventTypeId &&
                    setConfigFilters({
                      selectedEventTypeId,
