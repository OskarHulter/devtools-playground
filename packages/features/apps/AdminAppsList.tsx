...
const AdminAppsList = ({
  ...
  return (
    <form
      ...
      className={
        classNames?.form ??
        "max-w-80 bg-default mb-4 rounded-md px-0 pt-0 md:max-w-full md:px-8 md:pt-10"
      }
      onSubmit={(e) => {
        e.preventDefault();
        onSubmit();
    ...
    >
      <AppCategoryNavigation
        ...
        classNames={{
          ...
          container: cs(
            "min-w-0 w-full",
            classNames?.appCategoryNavigationContainer ?? "max-w-[500px]"
          ),
        }}
      >
...
const EditKeysModal: FC<{
  ...
  const saveKeysMutation = trpc.viewer.appsRouter.saveKeys.useMutation({
    onSuccess: () => {
      showToast(
        fromEnabled
          ? t("app_is_enabled", { appName })
          : t("keys_have_been_saved"),
        "success"
      );
...
const AdminAppsListContainer = () => {
  ...
  const [modalState, setModalState] = useReducer(
    (data: EditModalState, partialData: Partial<EditModalState>) => ({
      ...data,
      ...partialData,
    }),
    {
