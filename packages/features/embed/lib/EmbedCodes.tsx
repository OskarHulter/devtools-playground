...
import { WEBSITE_URL, IS_SELF_HOSTED, WEBAPP_URL } from "@sln/lib/constants";

export const doWeNeedCalOriginProp = (embedCalOrigin: string) => {
  // If we are self hosted, calOrigin won't be app.oskarhulter.com so we need to pass it
  ...
  return (
    IS_SELF_HOSTED ||
    (embedCalOrigin !== WEBAPP_URL && embedCalOrigin !== WEBSITE_URL)
  );
};
...
export const Codes = {
  react: {
    inline: ({
      ...
      return code`
  import Cal, { getCalApi } from "@sln/embed-react";
  import { useEffect } from "react";
  export default function MyApp() {
	useEffect(()=>{
	  (async function () {
		const cal = await getCalApi(${
      argumentForGetCalApi ? JSON.stringify(argumentForGetCalApi) : ""
    });
		${uiInstructionCode}
	  })();
	}, [])
    ...
    ${
      doWeNeedCalOriginProp(embedCalOrigin)
    ...
    "floating-popup": ({
      ...
      const argumentForGetCalApi = getArgumentForGetCalApi(namespace);
      return code`
  import { getCalApi } from "@sln/embed-react";
  import { useEffect } from "react";
  export default function MyApp() {
	useEffect(()=>{
	  (async function () {
 ...
 */
const code = (
  partsWithoutBlock: TemplateStringsArray,
  ...blocksOrVariables: string[]
) => {
