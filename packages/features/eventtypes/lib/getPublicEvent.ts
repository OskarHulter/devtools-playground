...
import { privacyFilteredLocations } from "@sln/app-store/locations";
...
import { getSlugOrRequestedSlug } from "@sln/features/ee/organizations/lib/orgDomains";
import { isRecurringEvent, parseRecurringEvent } from "@sln/lib";
import { getOrgOrTeamAvatar } from "@sln/lib/defaultAvatarImage";
...
import type { BookerLayoutSettings } from "@sln/prisma/zod-utils";
import {
  BookerLayouts,
...
export const getPublicEvent = async (
  ...
  if (usernameList.length > 1) {
    ...
    let locations = defaultEvent.locations
      ? (defaultEvent.locations as LocationObject[])
      : [];
    ...
    if (preferedLocationType?.appSlug) {
      ...
      if (appType) {
        ...
        locations = [
          { type: appType, link: preferedLocationType.appLink },
        ] as LocationObject[];
      }
    }
  ...
  return [
    {
      ...
      profile,
      bookerUrl: getBookerBaseUrlSync(
        owner.profile?.organization?.slug ?? null
      ),
    },
  ];
}
