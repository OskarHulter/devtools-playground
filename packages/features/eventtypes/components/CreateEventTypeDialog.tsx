...
import { useTypedQuery } from "@sln/lib/hooks/useTypedQuery";
...
import { unlockedManagedEventTypeProps } from "@sln/prisma/zod-utils";
    ...
    onError: (err) => {
      ...
      if (err.data?.code === "BAD_REQUEST") {
        const message = `${err.data.code}: ${t(
          "error_event_type_url_duplicate"
        )}`;
        showToast(message, "error");
      }
            ...
            {teamId && (
              <div className="mb-4">
                <label
                  ...
                  className="text-default block text-sm font-bold"
                >
                  {t("assignment")}
                </label>
                  ...
                  <>
                    {isAdmin && (
                      <RadioArea.Item
                        ...
                        className={classNames("text-sm", !isAdmin && "w-1/2")}
                        classNames={{
                          container: classNames(isAdmin && "w-full"),
                        }}
