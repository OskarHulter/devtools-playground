...
import type {
  ...
  Host,
  TeamMember,
} from "@sln/features/eventtypes/lib/types";
import { useLocale } from "@sln/lib/hooks/useLocale";
import { Label } from "@sln/ui";
          ...
          value={(value || [])
            ...
            .map((host) => {
              const option = options.find(
                (member) => member.value === host.userId.toString()
              );
              return option
                ? { ...option, priority: host.priority ?? 2, isFixed }
                : options[0];
            })
            .filter(Boolean)}
...
const AddMembersWithSwitch = ({
  ...
  return (
    <div className="rounded-md ">
      <div
        className={`flex flex-col rounded-md px-6 pb-2 pt-6 ${containerClassName}`}
      >
        {automaticAddAllEnabled ? (
          <div className="mb-2">
            <AssignAllTeamMembers
