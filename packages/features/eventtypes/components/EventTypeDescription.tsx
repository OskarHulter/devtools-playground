...
import { PriceIcon } from "@sln/features/bookings/components/event-meta/PriceIcon";
import { classNames, parseRecurringEvent } from "@sln/lib";
import getPaymentAppData from "@sln/lib/getPaymentAppData";
import { useLocale } from "@sln/lib/hooks/useLocale";
import type { baseEventTypeSelect } from "@sln/prisma";
import { SchedulingType } from "@sln/prisma/enums";
import type { EventTypeModel } from "@sln/prisma/zod";
        ...
        <ul className="mt-2 flex flex-wrap gap-x-2 gap-y-1">
          ...
          {eventType.requiresConfirmation && (
            <li
              ...
              data-testid="requires-confirmation-badge"
            >
          ...
          {eventType.requiresConfirmation ||
          (recurringEvent?.count && recurringEvent.count) ? (
          ...
          {eventType?.seatsPerTimeSlot ? (
            <li>
              <Badge variant="gray" startIcon="user">
                <p>
                  {t("event_type_seats", {
                    numberOfSeats: eventType.seatsPerTimeSlot,
                  })}{" "}
                </p>
              </Badge>
            </li>
