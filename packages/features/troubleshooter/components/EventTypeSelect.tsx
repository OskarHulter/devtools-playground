...
import { SelectField } from "@sln/ui";
...
export function EventTypeSelect() {
  ...
  const setSelectedEventType = useTroubleshooterStore(
    ...
    if (selectedEventQueryParam) {
      ...
      startTransition(() => {
        const foundEventType = eventTypes?.find(
          (et) => et.slug === selectedEventQueryParam
        );
        if (foundEventType) {
  ...
  return (
    <SelectField
      ...
      value={
        options.find((option) => option.value === selectedEventType?.slug) ||
        options[0]
      }
      onChange={(option) => {
        if (!option) return;
