...
export function TroubleshooterHeader({
  ...
  const selectedDateString = useTroubleshooterStore(
    (state) => state.selectedDate
  );
  const setSelectedDate = useTroubleshooterStore(
    (state) => state.setSelectedDate
  );
  const addToSelectedDate = useTroubleshooterStore(
    (state) => state.addToSelectedDate
  );
  ...
  const formattedMonth = new Intl.DateTimeFormat(i18n.language, {
    month: "short",
  });
