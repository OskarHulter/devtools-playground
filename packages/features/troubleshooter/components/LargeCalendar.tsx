...
import { Calendar } from "@sln/features/calendars/weeklyview";
import type { CalendarAvailableTimeslots } from "@sln/features/calendars/weeklyview/types/state";
...
export const LargeCalendar = ({ extraDays }: { extraDays: number }) => {
  ...
  const calendarToColorMap = useTroubleshooterStore(
    (state) => state.calendarToColorMap
  );
  ...
  const events = useMemo(() => {
    ...
    if (busyEvents.dateOverrides) {
      busyEvents.dateOverrides.forEach((dateOverride) => {
        ...
        const workingHoursForDay = busyEvents.workingHours.find(
          (workingHours) => workingHours.days.includes(dayOfWeekNum)
        );
        ...
        calendarEvents.push({
          ...
          title: "Date Override",
          start: dateOverrideStart
            .add(workingHoursForDay.startTime, "minutes")
            .toDate(),
          end: dateOverrideEnd
            .add(workingHoursForDay.endTime, "minutes")
            .toDate(),
          options: {
            borderColor: "black",
            status: BookingStatus.ACCEPTED,
