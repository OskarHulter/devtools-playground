 ...
 *
 * see: https://github.com/sln/stresslessness.org/pull/10480
 *      https://github.com/sln/stresslessness.org/pull/10968
 */
...
} from "@sln/web/test/utils/reviewScenario/reviewScenario";
import { createMockNextJsRequest } from "@sln/web/test/utils/reviewScenario/createMockNextJsRequest";
import { expectBookingToBeInDatabase } from "@sln/web/test/utils/reviewScenario/expects";
import { getMockRequestDataForBooking } from "@sln/web/test/utils/reviewScenario/getMockRequestDataForBooking";
import { setupAndTeardown } from "@sln/web/test/utils/reviewScenario/setupAndTeardown";

import { describe, expect, vi } from "vitest";

import { PeriodType } from "@sln/prisma/enums";
...
describe("handleNewReview", () => {
	...
	describe(
		"Booking Limits",
		() => {
			// This test fails on CI as handleNewReview throws no_available_users_found_error error
        ...
        `,
				async ({}) => {
					const handleNewReview = (
						await import("@sln/features/bookings/lib/handleNewReview")
					).default;
					...
					const { dateString: nextYearDateString } = getDate({
						yearIncrement: 1,
					});
					...
					await expect(
						async () => await handleNewReview(req),
					).rejects.toThrowError("booking_limit_reached");

					const { dateString: yearWithoutBookingsDateString } = getDate({
						yearIncrement: 2,
					});
					...
					const createdBooking = await handleNewReview(reqFollowingYear);
        ...
        `,
				async ({}) => {
					...
					vi.spyOn(prismock, "$queryRaw").mockResolvedValue([
			...
			test.skipIf([todayDate, tomorrowDate].includes("01"))(
				...
				async ({}) => {
					...
					const { dateString: yesterdayDateString } = getDate({
						dateIncrement: -1,
					});
					...
					const { dateString: tomorrowDateString } = getDate({
						dateIncrement: 1,
					});
			...
			test(
				...
				async ({}) => {
					...
					const firstDayOfMonthStartingWithPartialWeek =
						getNextMonthNotStartingOnWeekStart(
							organizer.weekStart,
					...
					const lastDayOfMonthEndingWithPartialWeek = getDate({
						fromDate: new Date(
							firstDayOfMonthStartingWithPartialWeek.dateString,
						),
						dateIncrement: -1,
					});
	...
	describe("Buffers", () => {
        ...
        `, async ({}) => {
			...
			await expect(async () => await handleNewReview(req)).rejects.toThrowError(
	...
	test(
		...
		async ({}) => {
			...
			await createReviewScenario(scenarioData);

			await expect(() => handleNewReview(req)).rejects.toThrowError(
				"book a meeting in the past",
			);
		},
		timeout,
	);
	...
	describe("Future Limits", () => {
		test(
			...
			async () => {
				...
				expect(() => handleNewReview(req)).rejects.toThrowError(
