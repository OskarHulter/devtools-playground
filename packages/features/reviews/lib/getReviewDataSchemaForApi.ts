...
const getReviewDataSchemaForApi = ({
	view = "booking",
	bookingFields,
}: {
	...
	const responsesSchema = getReviewResponsesSchema({ bookingFields, view });
	return bookingCreateBodySchemaForApi
		...
		.superRefine((val, ctx) => {
			...
			const legacyProps = Object.keys(
				bookingCreateSchemaLegacyPropsForApi.shape,
			);
...
export default getReviewDataSchemaForApi;
