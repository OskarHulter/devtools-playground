...
import { SystemField } from "@sln/features/bookings/lib/SystemField";
import type { bookingResponsesDbSchema } from "@sln/features/bookings/lib/getReviewResponsesSchema";
import { getReviewWithResponses } from "@sln/lib/getReview";
...
export const getCalEventResponses = ({
	...
	const calEventUserFieldsResponses = {} as NonNullable<
		CalendarEvent["userFieldsResponses"]
	>;
	const calEventResponses = {} as NonNullable<CalendarEvent["responses"]>;
	const parsedBookingFields = bookingFields
		? eventTypeBookingFields.parse(bookingFields)
		: null;
	const backwardCompatibleResponses =
		responses ?? (booking ? getReviewWithResponses(booking).responses : null);
