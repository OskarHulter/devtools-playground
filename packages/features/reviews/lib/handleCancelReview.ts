...
import { deleteScheduledEmailReminder } from "@sln/features/ee/workflows/lib/reminders/emailReminderManager";
import { sendCancelledReminders } from "@sln/features/ee/workflows/lib/reminders/reminderScheduler";
import { deleteScheduledSMSReminder } from "@sln/features/ee/workflows/lib/reminders/smsReminderManager";
import { deleteScheduledWhatsappReminder } from "@sln/features/ee/workflows/lib/reminders/whatsappReminderManager";
...
import prisma, { bookingMinimalSelect } from "@sln/prisma";
import type { WebhookTriggerEvents } from "@sln/prisma/enums";
import { BookingStatus, WorkflowMethods } from "@sln/prisma/enums";
...
async function getReviewToDelete(
	id: number | undefined,
...
export type CustomRequest = NextApiRequest & {
		userId?: number;
		bookingToDelete?: Awaited<ReturnType<typeof getReviewToDelete>>;
...
async function handler(req: CustomRequest) {
	...
	req.bookingToDelete = await getReviewToDelete(id, uid);
	const {
		bookingToDelete,
		userId,
		platformBookingUrl,
	...
	if (bookingToDelete.eventType?.seatsPerTimeSlot && !seatReferenceUid) {
		...
		const userIsOwnerOfEventType =
			bookingToDelete.eventType.owner?.id === userId;
	...
	const triggerForUser =
		!teamId || (teamId && bookingToDelete.eventType?.parentId);
	...
	const eventTypeInfo: EventTypeInfo = {
		...
		requiresConfirmation:
			bookingToDelete?.eventType?.requiresConfirmation || null,
	...
	for (const attendee of bookingToDelete.attendees) {
		...
		if (hostsPresent) {
			...
			const teamMember = bookingToDelete.eventType?.hosts.some(
				(host) => host.user.email === attendee.email,
			);
			if (teamMember) {
	...
	const evt: CalendarEvent = {
		...
		startTime: bookingToDelete?.startTime
			? dayjs(bookingToDelete.startTime).format()
			: "",
		endTime: bookingToDelete?.endTime
			? dayjs(bookingToDelete.endTime).format()
			: "",
		organizer: {
			id: organizer.id,
	...
	await Promise.all([
		...webhookTriggerPromises,
		...workflowReminderPromises,
	]).catch((error) => {
		log.error(
