...
import { HOSTED_CAL_FEATURES, WEBAPP_URL } from "@sln/lib/constants";
...
import type { VerticalTabItemProps } from "@sln/ui";
import {
  Badge,
  Button,
  ErrorBoundary,
...
const tabs: VerticalTabItemProps[] = [
  ...
  },
  {
    ...
    children: [
      ...
      {
        name: "admin_api",
        href: "https://oskarhulter.com/docs/enterprise-features/api/api-reference/bookings#admin-access",
      },
    ],
  },
  {
...
const useTabs = () => {
  ...
  const isOrgAdminOrOwner =
    orgBranding?.role === MembershipRole.ADMIN ||
    orgBranding?.role === MembershipRole.OWNER;
      ...
      if (tab.href === "/settings/my-account") {
        if (!!session.data?.user?.org?.id) {
          tab.children = tab?.children?.filter(
            (child) => child.href !== "/settings/my-account/appearance"
          );
        }
        return {
          ...tab,
  ...
  return (
    ...
    >
      ...
      <Skeleton
        title={name}
        as="p"
        className="max-w-36 min-h-4 truncate"
...
const SettingsSidebarContainer = ({
  ...
  const isOrgAdminOrOwner =
    currentOrg &&
    currentOrg?.user?.role &&
    ["OWNER", "ADMIN"].includes(currentOrg?.user?.role);
              ...
              {tab.name === "teams" && (
                <React.Fragment key={tab.href}>
                  ...
                  >
                    ...
                    <TeamListCollapsible />
                    {(!currentOrg ||
                      (currentOrg && currentOrg?.user?.role !== "MEMBER")) && (
                      <VerticalTabItem
                        name={t("add_a_team")}
