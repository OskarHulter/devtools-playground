...
import { Label, CheckboxField, Button } from "@sln/ui";
...
export const BookerLayoutSelector = ({
  ...
  const shouldShowUserSettings =
    (fallbackToUserSettings && !getValues(name || defaultFieldName)) || false;
  ...
  return (
    ...
    >
      <div
        className={classNames(
          isOuterBorder ? "pb-5" : "border-subtle rounded-t-xl border p-6"
        )}
      >
        <Label
          className={classNames(
            "mb-1 font-semibold",
            isOuterBorder ? "text-sm" : "text-base"
          )}
        >
...
const BookerLayoutFields = ({
  ...
  const defaultLayout =
    shownSettings?.defaultLayout || BookerLayouts.MONTH_VIEW;

  // Converts the settings array into a boolean object, which can be used as form values.
  const toggleValues: BookerLayoutState = bookerLayoutOptions.reduce(
    (layouts, layout) => {
  ...
  const onLayoutToggleChange = useCallback(
    (changedLayout: BookerLayouts, checked: boolean) => {
      ...
      const isDefaultLayoutToggledOff =
        newEnabledLayouts.indexOf(defaultLayout) === -1;
  ...
  return (
    <div
      className={classNames(
        "space-y-5",
        !isOuterBorder && "border-subtle border-x px-6 py-8"
      )}
    >
      <div
        className={classNames(
          ...
          <div className="w-full" key={layout}>
            <label>
              ...
              <CheckboxField
                ...
                onChange={(ev) =>
                  onLayoutToggleChange(layout, ev.target.checked)
                }
              />
            </label>
          </div>
      ...
      <div
        hidden={
          Object.values(toggleValues).filter((value) => value === true)
            .length <= 1
        }
        className={classNames(
          "transition-opacity",
      ...
      >
        ...
        <RadioGroup.Root
          ...
          onValueChange={(layout: BookerLayouts) =>
