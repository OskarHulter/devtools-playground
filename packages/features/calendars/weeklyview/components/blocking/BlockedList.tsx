      ...
      {day.isBefore(dayjs(), "day") && (
        <div
          ...
          style={{
            ...
            zIndex: 60,
            height: `calc(${
              (endHour + 1 - startHour) * 60
            } * var(--one-minute-height))`, // Add 1 to endHour to include the last hour that we add to display the last vertical line
          }}
        >
          <BlockedTimeCell />
        </div>
...
function BlockedToday({
  ...
  const dayStart = useMemo(
    () => day.startOf("day").hour(startHour),
    [day, startHour]
  );
  const dayEnd = useMemo(
    () => day.startOf("day").hour(endHour),
    [day, endHour]
  );
  const dayEndInMinutes = useMemo(
    () => dayEnd.diff(dayStart, "minutes"),
    [dayEnd, dayStart]
  );
  let nowComparedToDayStart = useMemo(
    () => dayjs().diff(dayStart, "minutes"),
    [dayStart]
  );

  if (nowComparedToDayStart > dayEndInMinutes)
    nowComparedToDayStart = dayEndInMinutes;
...
export function BlockedList({ day }: Props) {
  const { startHour, blockingDates, endHour, gridCellsPerHour } =
    useCalendarStore(
      (state) => ({
  ...
  return (
    <>
      ...
      <BlockedToday
        gridCellsPerHour={gridCellsPerHour}
        day={day}
        startHour={startHour}
        endHour={endHour}
      />
      {blockingDates &&
        blockingDates.map((event, i) => {
          ...
          const eventStart = dayStart.isAfter(blockingStart)
            ? dayStart
            : blockingStart;
