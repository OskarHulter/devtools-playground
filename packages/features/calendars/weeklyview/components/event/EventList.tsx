  ...
  return (
    <>
      {events
        .filter((event) => {
          return (
            dayjs(event.start).isSame(day, "day") && !event.options?.allDay
          ); // Filter all events that are not allDay and that are on the current day
        })
