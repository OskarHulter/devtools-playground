...
export function EmptyCell(props: EmptyCellProps) {
  ...
  const minutesFromStart =
    (cellToDate.hour() - props.startHour) * 60 + cellToDate.minute();

  return (
    <Cell
      topOffsetMinutes={minutesFromStart}
      timeSlot={dayjs(cellToDate).tz(props.timezone)}
    />
  );
}
  ...
  const slots = useMemo(() => {
    ...
    slotsForToday?.forEach((slot, index) => {
      ...
      const topOffsetMinutes =
        (startTime.hour() - startHour) * 60 + startTime.minute();
    ...
    if (areAllSlotsAway && firstSlotIndex !== -1) {
      ...
      startEndTimeDuration = dayjs(lastSlot.end).diff(
        dayjs(firstSlot.start),
        "minutes"
      );
