...
export function getHoursToDisplay(
  startHour: number,
  endHour: number,
  timezone?: string
) {
...
export function mergeOverlappingDateRanges(dateRanges: TimeRange[]) {
  ...
  for (let i = 0; i < dateRanges.length; i++) {
    ...
    if (mergedDateRanges.length === 0) {
      ...
      if (
        lastMergedDateRange.end.getTime() >= currentDateRange.start.getTime()
      ) {
...
export function calculateHourSizeInPx(
  ...
  return (
    (window.innerHeight - offsetFromTop - gapOnBottom) / (endHour - startHour)
  );
}
