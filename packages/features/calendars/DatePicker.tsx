import type { IFromUser, IToUser } from "@sln/core/getUserAvailability";
import type { Dayjs } from "@sln/dayjs";
import dayjs from "@sln/dayjs";
import { useEmbedStyles } from "@sln/embed-core/embed-iframe";
import { useBookerStore } from "@sln/features/bookings/Booker/store";
import { getAvailableDatesInMonth } from "@sln/features/calendars/lib/getAvailableDatesInMonth";
import classNames from "@sln/lib/classNames";
import { daysInMonth, yyyymmdd } from "@sln/lib/date-fns";
import { useLocale } from "@sln/lib/hooks/useLocale";
import { weekdayNames } from "@sln/lib/weekday";
import { Button, SkeletonText } from "@sln/ui";
...
const NoAvailabilityOverlay = ({
  ...
  return (
    <div className="bg-muted border-subtle absolute left-1/2 top-40 -mt-10 w-max -translate-x-1/2 -translate-y-1/2 transform rounded-md border p-8 shadow-sm">
      <h4 className="text-emphasis mb-4 font-medium">
...
const ReschedulingNotPossibleOverlay = () => {
  ...
  return (
    <div className="bg-muted border-subtle absolute left-1/2 top-40 -mt-10 w-2/3 -translate-x-1/2 -translate-y-1/2 transform break-all rounded-md border p-8 shadow-sm">
      <h4 className="text-emphasis font-medium">
