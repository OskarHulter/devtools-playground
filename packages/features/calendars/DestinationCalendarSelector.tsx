...
import type { DestinationCalendar } from "@sln/prisma/client";
import { trpc } from "@sln/trpc/react";
import { Badge, Icon, Select } from "@sln/ui";
...
const DestinationCalendarSelector = ({
  ...
  useEffect(() => {
    ...
    if (selected) {
      const selectedIntegration = query.data?.connectedCalendars.find(
        (integration) =>
          integration.calendars?.some(
            (calendar) => calendar.externalId === selected.externalId
          )
      );
      ...
      setSelectedOption({
        ...
        label: `${selected.name} ` || "",
        subtitle: `(${selectedIntegration?.integration.title?.replace(
          /calendar/i,
          ""
  ...
  const options =
    query.data.connectedCalendars.map((selectedCalendar) => ({
      ...
      label: `${selectedCalendar.integration.title?.replace(
        /calendar/i,
        ""
      )} (${
      ...
      options: (selectedCalendar.calendars ?? [])
        ...
        .map((cal) => ({
          label: ` ${cal.name} `,
          subtitle: `(${selectedCalendar?.integration.title?.replace(
            /calendar/i,
            ""
          )} - ${selectedCalendar?.primary?.name})`,
