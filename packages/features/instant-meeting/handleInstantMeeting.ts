...
import { createInstantMeetingWithCalVideo } from "@sln/core/videoClient";
import dayjs from "@sln/dayjs";
import getBookingDataSchema from "@sln/features/bookings/lib/getBookingDataSchema";
import { getBookingFieldsWithSystemFields } from "@sln/features/bookings/lib/getBookingFields";
import { getCustomInputsResponses } from "@sln/features/bookings/lib/handleNewBooking";
import { getBookingData } from "@sln/features/bookings/lib/handleNewBooking/getBookingData";
import { getEventTypesFromDB } from "@sln/features/bookings/lib/handleNewBooking/getEventTypesFromDB";
import { getFullName } from "@sln/features/form-builder/utils";
import { sendGenericWebhookPayload } from "@sln/features/webhooks/lib/sendPayload";
import { isPrismaObjOrUndefined } from "@sln/lib";
...
import { getTranslation } from "@sln/lib/server";
...
const handleInstantMeetingWebhookTrigger = async (args: {
  ...
  const orgId =
    (await getOrgIdFromMemberOrTeamId({ teamId: args.teamId })) ?? 0;
...
async function handler(req: NextApiRequest) {
  ...
  const seed = `${reqBody.email}:${dayjs(reqBody.start)
    .utc()
  ...
  const calVideoMeeting = await createInstantMeetingWithCalVideo(
    dayjs.utc(reqBody.end).toISOString()
  );
  ...
  const newBookingData: Prisma.BookingCreateInput = {
    ...
    metadata: {
      ...reqBody.metadata,
      videoCallUrl: `${WEBAPP_URL}/video/${uid}`,
    },
    attendees: {
      createMany: {
        data: attendeesList,
  ...
  const eventTypeWithExpiryTimeOffset =
  ...
  const instantMeetingExpiryTimeOffsetInSeconds =
    eventTypeWithExpiryTimeOffset?.instantMeetingExpiryTimeOffsetInSeconds ??
    90;
  ...
  const instantMeetingToken = await prisma.instantMeetingToken.create({
    data: {
      ...
      expires: new Date(
        new Date().getTime() + 1000 * instantMeetingExpiryTimeOffsetInSeconds
      ),
      team: {
        connect: {
