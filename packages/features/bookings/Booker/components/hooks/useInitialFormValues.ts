...
import type getBookingResponsesSchema from "@sln/features/bookings/lib/getBookingResponsesSchema";
import { getBookingResponsesPartialSchema } from "@sln/features/bookings/lib/getBookingResponsesSchema";
...
export function useInitialFormValues({
  ...
  const key = `${Object.keys(initialValues).length}_${hasSession ? 1 : 0}_${
    initialValues?.bookingId ?? 0
  }`;
