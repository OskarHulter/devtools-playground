...
import { AnimatedPopover, Divider, Icon } from "@sln/ui";
import { useSession } from "next-auth/react";
import { Fragment, useMemo } from "react";

export type IEventTypesFilters =
  RouterOutputs["viewer"]["eventTypes"]["listWithTeam"];
...
export const EventTypeFilter = () => {
  ...
  const groupedEventTypes: GroupedEventTypeState | null = useMemo(() => {
    ...
    return individualEvents.length > 0
      ? { user_own_event_types: individualEvents, ...grouped }
      : grouped;
  }, [eventTypes.data]);
