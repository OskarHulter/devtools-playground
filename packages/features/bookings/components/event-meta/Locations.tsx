...
export const EventLocations = ({ event }: { event: BookerEvent }) => {
  ...
  const iconUrl =
    locations.length > 1 || !eventLocationType?.iconUrl
      ? undefined
      : eventLocationType.iconUrl;
  const icon =
    locations.length > 1 || !eventLocationType?.iconUrl ? "map-pin" : undefined;
    ...
    >
      ...
      {locations.length > 1 && (
        ...
        >
                      ...
                      <div className="flex flex-row items-center">
                        <img
                          ...
                          className={classNames(
                            ...
                            !getEventLocationType(
                              location.type
                            )?.iconUrl?.startsWith("/app-store")
                              ? "dark:invert-[.65]"
                              : ""
                          )}
                          alt={`${
                            getEventLocationType(location.type)?.label
                          } icon`}
                        />
          ...
          >
            <span className="relative z-[2] py-2">
