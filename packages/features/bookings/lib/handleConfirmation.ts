...
import type { EventManagerUser } from "@sln/core/EventManager";
...
import { sendScheduledEmails } from "@sln/emails";
import {
...
import { scheduleTrigger } from "@sln/features/webhooks/lib/scheduleTrigger";
...
import { BookingStatus, WebhookTriggerEvents } from "@sln/prisma/enums";
import { EventTypeMetaDataSchema } from "@sln/prisma/zod-utils";
import { getAllWorkflowsFromEventType } from "@sln/trpc/server/routers/viewer/workflows/util";
import type { AdditionalInformation, CalendarEvent } from "@sln/types/Calendar";
...
export async function handleConfirmation(args: {
  ...
  const { user, evt, recurringEventId, prisma, bookingId, booking, paid } =
    args;
  ...
  const videoCallUrl = metadata.hangoutLink
  ...
  if (recurringEventId) {
    ...
    const updateBookingsPromise = unconfirmedRecurringBookings.map(
      (recurringBooking) =>
        prisma.booking.update({
          ...
          data: {
            ...
            paid,
            metadata: {
              ...(typeof recurringBooking.metadata === "object"
                ? recurringBooking.metadata
                : {}),
  ...
  try {
    ...
    if (paid) {
      ...
      const successPayment = bookingWithPayment?.payment?.find(
        (item) => item.success
      );
      if (successPayment) {
