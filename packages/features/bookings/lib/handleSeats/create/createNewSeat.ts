...
import { sendScheduledSeatsEmails } from "@sln/emails";
import { refreshCredentials } from "@sln/features/bookings/lib/getAllCredentialsForUsersOnEvent/refreshCredentials";
import {
...
const createNewSeat = async (
  ...
  if (
    ...
    eventType.seatsPerTimeSlot <=
      seatedBooking.attendees.filter((attendee) => !!attendee.bookingSeat)
        .length
  ) {
    throw new HttpError({
      statusCode: 409,
  ...
  await prisma.booking.update({
    ...
    data: {
      ...
      ...(seatedBooking.status === BookingStatus.CANCELLED && {
