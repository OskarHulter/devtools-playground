...
import { appStoreMetadata } from "@sln/app-store/apps.metadata.generated";
...
import { getMockRequestDataForCancelBooking } from "@sln/web/test/utils/bookingScenario/getMockRequestDataForCancelBooking";
  ...
  describe("As an attendee", () => {
    ...
    describe("Rescheduling a booking", () => {
      test("When rescheduling to an existing booking, move attendee", async () => {
        ...
        expect(oldBookingAttendees).not.toContain({
          id: attendeeToReschedule.id,
        });
        ...
        expect(newBookingAttendees).toContainEqual({
          email: attendeeToReschedule.email,
        });
    ...
    describe("Canceling a booking", async () => {
      test("When canceling a booking, only remove that single attendee", async () => {
        const handleCancelBooking = (
          await import("@sln/features/bookings/lib/handleCancelBooking")
        ).default;
        ...
        expect(bookingSeats).not.toContain({
          referenceUid: bookingSeatToBeCancelledUid,
        });
      });
  ...
  describe("As an owner", () => {
    describe("Rescheduling a booking", () => {
      ...
      test("When rescheduling to existing booking, merge attendees ", async () => {
        ...
        // Ensure that the booking has been moved
        expect(rescheduledBooking?.startTime).toEqual(
          new Date(secondBookingStartTime)
        );
        expect(rescheduledBooking?.endTime).toEqual(
          new Date(secondBookingEndTime)
        );
