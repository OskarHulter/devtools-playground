...
import type EventManager from "@sln/core/EventManager";
import { sendRescheduledEmails } from "@sln/emails";
...
const combineTwoSeatedBookings = async (
  ...
  for (const attendee of seatedBooking.attendees) {
    ...
    if (
      ...
      attendeesToMove.push({
        id: attendee.id,
        seatReferenceId: attendee.bookingSeat?.id,
      });
    }
  }
  ...
  if (
    ...
    attendeesToMove.length +
      newTimeSlotBooking.attendees.filter((attendee) => attendee.bookingSeat)
        .length >
      eventType.seatsPerTimeSlot
  ) {
    throw new HttpError({
      statusCode: 409,
      message: ErrorCode.NotEnoughAvailableSeats,
    });
  }
  ...
  const updatedBookingAttendees = updatedNewBooking.attendees.map(
    (attendee) => {
      const evtAttendee = {
        ...attendee,
  ...
  if (noEmail !== true && isConfirmedByDefault) {
    ...
    loggerWithEventDetails.debug(
      "Emails: Sending reschedule emails - handleSeats"
    );
