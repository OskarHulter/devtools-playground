...
import { sendRescheduledSeatEmail } from "@sln/emails";
import { getTranslation } from "@sln/lib/server/i18n";
import prisma from "@sln/prisma";
import type { Person, CalendarEvent } from "@sln/types/Calendar";
...
const attendeeRescheduleSeatedBooking = async (
  ...
  const { tAttendees, bookingSeat, bookerEmail, evt } =
    rescheduleSeatedBookingObject;
  let { originalRescheduledBooking } = rescheduleSeatedBookingObject;

  seatAttendee["language"] = {
    translate: tAttendees,
    locale: bookingSeat?.attendee.locale ?? "en",
  };
