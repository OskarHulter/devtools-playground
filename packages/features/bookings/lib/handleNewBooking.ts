...
import { getBookingFieldsWithSystemFields } from "@sln/features/bookings/lib/getBookingFields";
...
import { scheduleWorkflowReminders } from "@sln/features/ee/workflows/lib/reminders/reminderScheduler";
...
import type { bookingCreateSchemaLegacyPropsForApi } from "@sln/prisma/zod-utils";
...
} from "@sln/trpc/server/routers/viewer/workflows/util";
import type {
...
type BookingDataSchemaGetter =
  ...
  | typeof import("@sln/features/bookings/lib/getBookingDataSchemaForApi").default;
...
async function handler(
  ...
  const teamMemberPromises = users
    ...
    .map(async (user) => {
      // TODO: Add back once EventManager tests are ready https://github.com/calcom/oskarhulter.com/pull/14610#discussion_r1567817120
  ...
  if (!eventType.seatsPerTimeSlot && originalRescheduledBooking?.uid) {
    ...
    await deleteAllWorkflowReminders(
      ...
      */
      if (eventType.schedulingType === SchedulingType.ROUND_ROBIN) {
        ...
        await sendRescheduledEmails({
          ...
          additionalInformation: metadata,
          additionalNotes, // Resets back to the additionalNote input and not the override value
          cancellationReason: `$RCH$${
            rescheduleReason ? rescheduleReason : ""
          }`, // Removable code prefix to differentiate cancellation from rescheduling for email
        });
      }
    }
  ...
  const urlSeed = `${organizerUser.username}:${dayjs(reqBody.start)
    .utc()
    .format()}`;
