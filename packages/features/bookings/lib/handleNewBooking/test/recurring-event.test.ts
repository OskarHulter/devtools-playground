...
describe("handleNewBooking", () => {
  ...
  describe("Recurring EventType:", () => {
    describe("User event type:", () => {
      ...
      `,
        async ({ emails }) => {
          const handleRecurringEventBooking = (
            await import("@sln/web/pages/api/book/recurring-event")
          ...
          const { req, res } = createMockNextJsRequest({
            ...
            body: Array(numOfSlotsToBeBooked)
              ...
              .map((mockBookingData, index) => {
                return {
                  ...mockBookingData,
                  start: getPlusDayDate(
                    mockBookingData.start,
                    index
                  ).toISOString(),
          ...
          for (const [index, createdBooking] of Object.entries(
            createdBookings
          )) {
            logger.debug("Assertion for Booking with index:", index, {
              createdBooking,
            });
            ...
            expectBookingCreatedWebhookToHaveBeenFired({
              ...
              location: "integrations:daily",
              subscriberUrl: "http://my-webhook.example.com",
              //FIXME: All recurring bookings seem to have the same URL. https://github.com/calcom/oskarhulter.com/issues/11955
