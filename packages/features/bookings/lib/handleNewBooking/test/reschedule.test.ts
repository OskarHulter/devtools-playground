...
import { BookingStatus, SchedulingType } from "@sln/prisma/enums";
  ...
  describe("Reschedule", () => {
    ...
    `,
        async ({ emails }) => {
           ...
           */
          expect(createdBooking.startTime?.toISOString()).toBe(
            `${plus1DateString}T04:00:00.000Z`
          );
          expect(createdBooking.endTime?.toISOString()).toBe(
            `${plus1DateString}T04:15:00.000Z`
          );
          ...
          expectSuccessfulBookingRescheduledEmails({
            ...
            appsStatus: [
              getMockPassingAppStatus({
                slug: appStoreMetadata.dailyvideo.slug,
              }),
              getMockPassingAppStatus({
                slug: appStoreMetadata.googlecalendar.slug,
              }),
            ],
          });
    ...
    `,
      ...
      test(
        ...
        async ({ emails }) => {
          ...
          const _calendarMock =
            mockCalendarToCrashOnUpdateEvent("googlecalendar");
    ...
    `,
          async ({ emails }) => {
            ...
            const calendarMock = mockCalendarToHaveNoBusySlots(
              "googlecalendar",
              {
                create: {
                  uid: "MOCK_ID",
