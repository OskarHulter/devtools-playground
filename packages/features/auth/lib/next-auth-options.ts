...
import checkLicense from "@sln/features/ee/common/server/checkLicense";
import createUsersAndConnectToOrg from "@sln/features/ee/dsync/lib/users/createUsersAndConnectToOrg";
import ImpersonationProvider from "@sln/features/ee/impersonation/lib/ImpersonationProvider";
import {
  getOrgFullOrigin,
  subdomainSuffix,
} from "@sln/features/ee/organizations/lib/orgDomains";
import {
...
import { symmetricDecrypt, symmetricEncrypt } from "@sln/lib/crypto";
import { defaultCookies } from "@sln/lib/default-cookies";
import { isENVDev } from "@sln/lib/env";
...
import { IdentityProvider, MembershipRole } from "@sln/prisma/enums";
import { teamMetadataSchema, userMetadata } from "@sln/prisma/zod-utils";
...
const providers: Provider[] = [
  CredentialsProvider({
    ...
    async authorize(credentials) {
      ...
      const user =
        await UserRepository.findByEmailAndIncludeProfilesAndPassword({
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const { oauthController } = await (
          await import("@sln/features/ee/sso/lib/jackson")
        ).default();
        ...
        if (!user) {
          ...
          if (hostedCal && email) {
            ...
            const organizationId =
              await getVerifiedOrganizationByAutoAcceptEmailDomain(domain);
            if (organizationId) {
