...
import { hashPassword } from "@sln/features/auth/lib/hashPassword";
import { sendEmailVerification } from "@sln/features/auth/lib/verifyEmail";
import { createOrUpdateMemberships } from "@sln/features/auth/signup/utils/createOrUpdateMemberships";
import { prefillAvatar } from "@sln/features/auth/signup/utils/prefillAvatar";
import { WEBAPP_URL } from "@sln/lib/constants";
import { getLocaleFromRequest } from "@sln/lib/getLocaleFromRequest";
import { HttpError } from "@sln/lib/http-error";
import logger from "@sln/lib/logger";
import {
  usernameHandler,
  type RequestWithUsernameStatus,
} from "@sln/lib/server/username";
import { createWebUser as syncServicesCreateWebUser } from "@sln/lib/sync/SyncServiceManager";
import { closeComUpsertTeamUser } from "@sln/lib/sync/SyncServiceManager";
import { validateAndGetCorrectedUsernameAndEmail } from "@sln/lib/validateUsername";
...
async function handler(req: RequestWithUsernameStatus, res: NextApiResponse) {
  ...
  if (req.usernameStatus.statusCode === 418) {
    return res
      .status(req.usernameStatus.statusCode)
      .json(req.usernameStatus.json);
  }
  ...
  return res
    .status(201)
    .json({ message: "Created user", stripeCustomerId: customer.id });
}
