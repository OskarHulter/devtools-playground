...
import { checkPremiumUsername } from "@sln/ee/common/lib/checkPremiumUsername";
...
import { isUsernameReservedDueToMigration } from "@sln/lib/server/username";
...
export default async function handler(
  ...
  if (foundToken && foundToken?.teamId) {
    ...
    if (team) {
      ...
      const isCheckingUsernameInGlobalNamespace =
        !team.isOrganization && !isInviteForATeamInOrganization;

      if (isCheckingUsernameInGlobalNamespace) {
        const isUsernameAvailable = !(await isUsernameReservedDueToMigration(
          correctedUsername
        ));
    ...
    if (IS_PREMIUM_USERNAME_ENABLED) {
      ...
      if (checkUsername.premium) {
        res.status(422).json({
          message:
            "Sign up from https://oskarhulter.com/signup to claim your premium username",
        });
        return;
      }
    }
