...
import {
  ...
  DateOverrideList,
} from "@sln/features/schedules";
import Schedule from "@sln/features/schedules/components/Schedule";
...
export function AvailabilityEditSheet(props: Props) {
  ...
  const { data, isPending } =
    trpc.viewer.availability.schedule.getScheduleByUserId.useQuery({
      userId: userId,
    });
  ...
  // when revalidating.
  return (
    <AvailabilityEditSheetForm {...props} data={data} isPending={isPending} />
  );
}

type Data =
  RouterOutputs["viewer"]["availability"]["schedule"]["getScheduleByUserId"];
export function AvailabilityEditSheetForm(
  props: Props & { data: Data; isPending: boolean }
) {
  // This sheet will not be rendered without a selected user
  const userId = props.selectedUser?.id as number;
  const { t } = useLocale();
        ...
        >
          ...
          {!data.hasDefaultSchedule && !isPending && hasEditPermission && (
            <div className="my-2">
              <Alert
                severity="warning"
                title={t("view_only_edit_availability_not_onboarded")}
              />
            </div>
          )}
          {!hasEditPermission && !loadingPermissions && (
            <div className="my-2">
              <Alert
                severity="warning"
                title={t("view_only_edit_availability")}
              />
            </div>
          )}
