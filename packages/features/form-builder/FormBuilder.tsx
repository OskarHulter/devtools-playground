...
import { getAndUpdateNormalizedValues } from "@sln/features/form-builder/FormBuilderField";
...
export const FormBuilder = function FormBuilder({
  ...
  dataStore: {
    options: Record<
      string,
      { label: string; value: string; inputPlaceholder?: string }[]
    >;
  };
}) {
        ...
        >
          {fields.map((field, index) => {
            ...
            if (fieldsThatSupportLabelAsSafeHtml.includes(field.type)) {
              field = {
                ...field,
                labelAsSafeHtml: markdownToSafeHTML(field.label ?? ""),
              };
            }

            const { hidden } = getAndUpdateNormalizedValues(
              { ...field, options },
              t
            );
            ...
            if (field.type === "radioInput" && options.length === 1) {
              fieldType =
                fieldTypesConfigMap[
                  field.optionsInputs?.[options[0].value].type || field.type
                ];
              isRequired =
                field.optionsInputs?.[options[0].value].required ||
                field.required;
            }
                  ...
                  <div className="flex items-center space-x-2">
                    {!isFieldEditableSystem &&
                      ...
                      !disabled && (
                        <Switch
                          ...
                          classNames={{
                            container: "p-2 hover:bg-subtle rounded",
                          }}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (
                          fieldToEdit.type === "radioInput" &&
                          options.length === 1
                        ) {
                          fieldToEdit.type =
                            fieldToEdit.optionsInputs?.[options[0].value]
      ...
      return (
        <span
          dangerouslySetInnerHTML={{
            ...
            __html:
              markdownToSafeHTML(field.label || "") ||
            ...
            >
              {!isSimpleVariant && (
                <Label className="flex justify-between">
                  ...
                  <span className="text-muted">{`${fieldForm.getValues(
                    "name"
                  )}.${f.name}`}</span>
                </Label>
              )}
