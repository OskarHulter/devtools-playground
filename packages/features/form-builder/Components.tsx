...
import type { SelectLikeComponentProps, TextLikeComponentProps } from "@sln/app-store/routing-forms/components/react-awesome-query-builder/widgets";
import Widgets from "@sln/app-store/routing-forms/components/react-awesome-query-builder/widgets";
import { useLocale } from "@sln/lib/hooks/useLocale";
import { AddressInput, Button, CheckboxField, EmailField, Group, Icon, InfoBadge, InputField, Label, PhoneInput, RadioField, Tooltip } from "@sln/ui";
  ...
  | {
      ...
      factory: <
        TProps extends SelectLikeComponentProps<{
          ...
          optionsInputs: NonNullable<
            z.infer<typeof fieldSchema>["optionsInputs"]
          >;
...
export const Components: Record<FieldType, Component> = {
  ...
  multiselect: {
    ...
    factory: (props) => {
      const newProps = {
        ...props,
        listValues: props.options.map((o) => ({
          title: o.label,
          value: o.value,
        })),
      };
                ...
                <>
                  <Label className="flex">
                    ...
                    {!readOnly && optionsInputs[options[0].value]?.required ? (
                      <span className="text-default mb-1 ml-1 text-sm font-medium">
                        *
                      </span>
                    ) : null}
