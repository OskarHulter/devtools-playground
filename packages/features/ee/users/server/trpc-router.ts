...
import { _UserModel as User } from "@sln/prisma/zod";
import type { inferRouterOutputs } from "@sln/trpc";
import { TRPCError } from "@sln/trpc";
import { authedAdminProcedure } from "@sln/trpc/server/procedures/authedProcedure";
...
const authedAdminProcedureWithRequestedUser = authedAdminProcedure.use(
...
export const userAdminRouter = router({
  get: authedAdminProcedureWithRequestedUser
    .input(userIdSchema)
    .query(async ({ ctx }) => {
  ...
  delete: authedAdminProcedureWithRequestedUser
    .input(userIdSchema)
    .mutation(async ({ ctx }) => {
      const { prisma, requestedUser } = ctx;
      await prisma.user.delete({ where: { id: requestedUser.id } });
      return {
        message: `User with id: ${requestedUser.id} deleted successfully`,
      };
    }),
});
