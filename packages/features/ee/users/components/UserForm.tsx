...
import { localeOptions } from "@sln/lib/i18n";
import { nameOfDay } from "@sln/lib/weekday";
import {
  Avatar,
  Button,
  EmailField,
...
type FormValues = Pick<
  User,
  "avatarUrl" | "name" | "username" | "email" | "bio"
> &
  OptionValues;
  ...
  const form = useForm<FormValues>({
    defaultValues: {
      ...
      locale: {
        value: defaultLocale,
        label:
          new Intl.DisplayNames(defaultLocale, { type: "language" }).of(
            defaultLocale
          ) || "",
      },
      timeFormat: {
        ...
        label:
          timeFormatOptions.find(
            (option) => option.value === defaultValues?.timeFormat
          )?.label || "12",
      },
      ...
      weekStart: {
        ...
        label: nameOfDay(
          localeProp,
          defaultValues?.weekStart === "Sunday" ? 0 : 1
        ),
      },
      role: {
        value: defaultValues?.role || userRoleOptions[0].value,
        label:
          userRoleOptions.find((option) => option.value === defaultValues?.role)
            ?.label || userRoleOptions[0].label,
      },
      identityProvider: {
        value:
          defaultValues?.identityProvider || identityProviderOptions[0].value,
        label:
          identityProviderOptions.find(
            (option) => option.value === defaultValues?.identityProvider
          )?.label || identityProviderOptions[0].label,
      },
    },
  });
