...
import { bookingMetadataSchema } from "@sln/prisma/zod-utils";
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  Promise.allSettled(deletePromises).then((results) => {
    results.forEach((result) => {
      if (result.status === "rejected") {
        logger.error(
          `Error deleting batch id from scheduled_sends: ${result.reason}`
        );
      }
    });
  });
  ...
  const unscheduledReminders: PartialWorkflowReminder[] =
    await getAllUnscheduledReminders();
      ...
      try {
        ...
        if (reminder.workflowStep.reminderBody) {
          ...
          const organizerOrganizationId =
            organizerOrganizationProfile?.organizationId;
          ...
          const variables: VariablesType = {
            ...
            responses: responses,
            meetingUrl: bookingMetadataSchema.parse(
              reminder.booking.metadata || {}
            )?.videoCallUrl,
          ...
          const emailSubject = customTemplate(
            ...
            getTimeFormatStringFromUserTimeFormat(
              reminder.booking.user?.timeFormat
            ),
          ...
          reminder.workflowStep.template === WorkflowTemplates.REMINDER
        ) {
          ...
          reminder.workflowStep.template === WorkflowTemplates.RATING
        ) {
          const organizerOrganizationProfile = await prisma.profile.findFirst({
            where: {
              userId: reminder.booking.user?.id,
          ...
          emailContent = emailRatingTemplate({
            isEditingMode: true,
            action:
              reminder.workflowStep.action || WorkflowActions.EMAIL_ADDRESS,
            ...
            name: name || "",
            ratingUrl:
              `${bookerUrl}/booking/${reminder.booking.uid}?rating` || "",
            noShowUrl:
              `${bookerUrl}/booking/${reminder.booking.uid}?noShow=true` || "",
          });
        }
            ...
            sendSendgridMail(
              {
                ...
                attachments: reminder.workflowStep.includeCalendarEvent
                  ? [
                      {
                        content: Buffer.from(
                          getiCalEventAsString(reminder.booking) || ""
                        ).toString("base64"),
                        filename: "event.ics",
  ...
  res
    .status(200)
    .json({ message: `${unscheduledReminders.length} Emails to schedule` });
}
