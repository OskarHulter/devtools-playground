...
} from "../lib/variableTranslations";
import { zodResolver } from "@hookform/resolvers/zod";
import type { WorkflowStep } from "@prisma/client";
import Shell, { ShellMain } from "@sln/features/shell/Shell";
import { classNames } from "@sln/lib";
import { SENDER_ID } from "@sln/lib/constants";
...
import { stringOrNumber } from "@sln/prisma/zod-utils";
...
import type { MultiSelectCheckboxesOptionType as Option } from "@sln/ui";
import { Alert, Badge, Button, Form, showToast } from "@sln/ui";
...
function WorkflowPage() {
  ...
  const { data: otherTeams, isPending: isPendingTeams } =
          ...
          ...group.eventTypes
            .filter(
              (evType) =>
                ...
                !!evType.metadata?.managedEventConfig.unlockedFields
                  ?.workflows ||
                !!teamId
            )
            .map((eventType) => ({
              ...
              label: `${eventType.title} ${
                eventType.children && eventType.children.length
                  ? `(+${eventType.children.length})`
                  : ``
              }`,
            })),
        ];
  ...
  function setFormData(
    ...
    if (workflowData) {
      if (
        workflowData.userId &&
        workflowData.activeOn.find((active) => !!active.eventType.teamId)
      ) {
      ...
      const steps = workflowData.steps.map((step) => {
        ...
        if (step.reminderBody) {
          updatedStep.reminderBody = getTranslatedText(
            step.reminderBody || "",
            {
        ...
        if (step.emailSubject) {
          updatedStep.emailSubject = getTranslatedText(
            step.emailSubject || "",
            {
              locale: i18n.language,
              t,
        ...
        <Form
          ...
          handleSubmit={async (values) => {
            ...
            values.steps.forEach((step) => {
              const strippedHtml =
                step.reminderBody?.replace(/<[^>]+>/g, "") || "";

              const isBodyEmpty =
                !isSMSOrWhatsappAction(step.action) && strippedHtml.length <= 1;
              ...
              if (step.reminderBody) {
                step.reminderBody = translateVariablesToEnglish(
                  step.reminderBody,
                  {
                    locale: i18n.language,
                    t,
              ...
              if (step.emailSubject) {
                step.emailSubject = translateVariablesToEnglish(
                  step.emailSubject,
                  {
                    locale: i18n.language,
                    t,
              ...
              if (
                ...
                !verifiedNumbers?.find(
                  (verifiedNumber) => verifiedNumber.phoneNumber === step.sendTo
                )
              ) {
                isVerified = false;
              ...
              if (
                ...
                !verifiedEmails?.find(
                  (verifiedEmail) => verifiedEmail.email === step.sendTo
                )
              ) {
                isVerified = false;
                ...
                {isAllDataLoaded && user ? (
                  <>
                    <WorkflowDetailsPage
                      ...
                      teamId={
                        workflow ? workflow.teamId || undefined : undefined
                      }
                      readOnly={readOnly}
                      isOrg={isOrg}
