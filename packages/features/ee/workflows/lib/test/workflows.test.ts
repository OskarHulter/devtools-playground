...
import { BookingStatus, WorkflowMethods, TimeUnit } from "@sln/prisma/enums";
import {
...
describe("deleteRemindersOfActiveOnIds", () => {
  test("should delete all reminders from removed event types", async ({}) => {
    ...
    expect(
      workflowReminders.filter(
        (reminder) => reminder.booking?.eventTypeId === 1
      ).length
    ).toBe(0);
    expect(
      workflowReminders.filter(
        (reminder) => reminder.booking?.eventTypeId === 2
      ).length
    ).toBe(2);
  });
  ...
  test("should delete all reminders from removed event types (org workflow)", async ({}) => {
    ...
    const workflowRemindersWithOneTeamActive =
      await prismock.workflowReminder.findMany({
        where: {
          booking: {
            userId: organizer.id,
          },
    ...
    const workflowRemindersWithNoTeamActive =
      await prismock.workflowReminder.findMany({
        where: {
          booking: {
            userId: organizer.id,
...
describe("scheduleBookingReminders", () => {
  test("schedules workflow notifications with before event trigger and email to host action", async ({}) => {
    ...
    scheduledWorkflowReminders.forEach((reminder, index) => {
      expect(expectedScheduledDates[index]).toStrictEqual(
