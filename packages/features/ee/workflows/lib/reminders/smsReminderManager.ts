...
import type { TimeFormat } from "@sln/lib/timeFormat";
...
import { WorkflowTriggerEvents } from "@sln/prisma/enums";
import { bookingMetadataSchema } from "@sln/prisma/zod-utils";
import type { CalEventResponses, RecurringEvent } from "@sln/types/Calendar";
...
export const scheduleSMSReminder = async (args: ScheduleTextReminderArgs) => {
  ...
  if (action === WorkflowActions.SMS_ATTENDEE) {
    const attendeeWithReminderPhoneAsSMSReminderNumber =
      reminderPhone &&
      evt.attendees.find(
        (attendee) => attendee.email === evt.responses?.email?.value
      );
  ...
  const name =
    action === WorkflowActions.SMS_ATTENDEE ? attendeeToBeUsedInSMS.name : "";
  const attendeeName =
    action === WorkflowActions.SMS_ATTENDEE
      ? evt.organizer.name
      : attendeeToBeUsedInSMS.name;
  const timeZone =
    action === WorkflowActions.SMS_ATTENDEE
