...
import { guessEventLocationType } from "@sln/app-store/locations";
import dayjs from "@sln/dayjs";
import { preprocessNameFieldDataWithVariant } from "@sln/features/form-builder/utils";
...
function getiCalEventAsString(evt: BookingInfo, status?: ParticipationStatus) {
  ...
  if (evt.eventType.recurringEvent?.count) {
    recurrenceRule = new RRule(evt.eventType.recurringEvent)
      .toString()
      .replace("RRULE:", "");
  }
  ...
  const icsEvent = createEvent({
    ...
    duration: {
      minutes: dayjs(evt.endTime).diff(dayjs(evt.startTime), "minute"),
    },
    title: evt.title,
    ...
    attendees: [
      {
        name: preprocessNameFieldDataWithVariant(
          "fullName",
...
export const scheduleEmailReminder = async (
  ...
  const timeUnit: timeUnitLowerCase | undefined =
    timeSpan.timeUnit?.toLocaleLowerCase() as timeUnitLowerCase;

  let scheduledDate = null;

  if (triggerEvent === WorkflowTriggerEvents.BEFORE_EVENT) {
    scheduledDate =
      timeSpan.time && timeUnit
        ? dayjs(startTime).subtract(timeSpan.time, timeUnit)
        : null;
  } else if (triggerEvent === WorkflowTriggerEvents.AFTER_EVENT) {
    scheduledDate =
      timeSpan.time && timeUnit
        ? dayjs(endTime).add(timeSpan.time, timeUnit)
        : null;
  }
  ...
  log.debug(
    `Sending Email for trigger ${triggerEvent}`,
    ...
    return sendSendgridMail(
      {
        ...
        attachments: includeCalendarEvent
          ? [
              {
                content: Buffer.from(
                  getiCalEventAsString(evt, status) || ""
                ).toString("base64"),
                filename: "event.ics",
  ...
  if (
    ...
    try {
      ...
      const promises = addressees.map((email) =>
        sendEmail({ to: email }, triggerEvent)
      );
