...
import {
  ...
  isWhatsappAction,
} from "@sln/features/ee/workflows/lib/actionHelperFunctions";
import type {
  Workflow,
  WorkflowStep,
} from "@sln/features/ee/workflows/lib/types";
import { checkSMSRateLimit } from "@sln/lib/checkRateLimitAndThrowError";
import { SENDER_NAME } from "@sln/lib/constants";
import { WorkflowActions, WorkflowTriggerEvents } from "@sln/prisma/enums";
...
export interface ScheduleWorkflowRemindersArgs
  extends ProcessWorkflowStepParams {
  workflows: Workflow[];
...
const processWorkflowStep = async (
  ...
  if (isSMSOrWhatsappAction(step.action)) {
    await checkSMSRateLimit({
      identifier: `sms:${workflow.teamId ? "team:" : "user:"}${
        workflow.teamId || workflow.userId
      }`,
...
export const sendCancelledReminders = async (
  args: SendCancelledRemindersArgs
) => {
