...
const emailReminderTemplate = (
  ...
  if (isEditingMode) {
    ...
    timeZone = "{TIMEZONE}";
    otherPerson =
      action === WorkflowActions.EMAIL_ATTENDEE ? "{ORGANIZER}" : "{ATTENDEE}";
    name =
      action === WorkflowActions.EMAIL_ATTENDEE ? "{ATTENDEE}" : "{ORGANIZER}";
  ...
  const branding =
    !isBrandingDisabled && !isEditingMode
  ...
  const emailBody =
    introHtml + eventHtml + dateTimeHtml + attendeeHtml + endingHtml;
