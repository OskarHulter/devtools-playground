...
import type { CalEventResponses } from "@sln/types/Calendar";
...
const customTemplate = (
  ...
  const translatedDate = new Intl.DateTimeFormat(locale, {
    ...
    variables.eventDate
      ?.add(dayjs().tz(variables.timeZone).utcOffset(), "minute")
      .toDate()
  );
  ...
  if (text.includes("{LOCATION}")) {
    locationString =
      guessEventLocationType(locationString)?.label || locationString;
  }
  ...
  let dynamicText = text
    ...
    .replaceAll(
      "{EVENT_TIME}",
      variables.eventDate?.format(currentTimeFormat) || ""
    )
    .replaceAll(
      "{START_TIME}",
      variables.eventDate?.format(currentTimeFormat) || ""
    ...
    .replaceAll(
      "{EVENT_END_TIME}",
      variables.eventEndTime?.format(currentTimeFormat) || ""
    )
  ...
  const customInputvariables = dynamicText
    .match(/\{(.+?)}/g)
    ?.map((variable) => {
      ...
      Object.keys(variables.responses).forEach((customInput) => {
        ...
        if (
          ...
          dynamicText = dynamicText.replace(
            `{${variable}}`,
            variables.responses[
              customInput as keyof typeof variables.responses
            ].value.toString()
          );
        }
      });
  ...
  const branding = !isBrandingDisabled
    ? `<br><br>_<br><br>Scheduling by ${APP_NAME}`
    : "";
