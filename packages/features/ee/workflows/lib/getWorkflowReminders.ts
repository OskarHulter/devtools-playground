...
import { WorkflowMethods } from "@sln/prisma/enums";
...
async function getWorkflowReminders<T extends Prisma.WorkflowReminderSelect>(
  ...
  const pageSize = 90;
  let pageNumber = 0;
  const filteredWorkflowReminders: Array<
    Prisma.WorkflowReminderGetPayload<{ select: T }>
  > = [];
  ...
  while (true) {
    ...
    filteredWorkflowReminders.push(
      ...(newFilteredWorkflowReminders as Array<
        Prisma.WorkflowReminderGetPayload<{ select: T }>
      >)
    );
    pageNumber++;
...
export async function getAllRemindersToDelete(): Promise<
  RemindersToDeleteType[]
> {
...
export async function getAllRemindersToCancel(): Promise<
  RemindersToCancelType[]
> {
  const whereFilter: Prisma.WorkflowReminderWhereInput = {
    cancelled: true,
...
export async function getAllUnscheduledReminders(): Promise<
  PartialWorkflowReminder[]
> {
  const whereFilter: Prisma.WorkflowReminderWhereInput = {
    method: WorkflowMethods.EMAIL,
    scheduled: false,
  ...
  const unscheduledReminders = (await getWorkflowReminders(
    whereFilter,
    select
