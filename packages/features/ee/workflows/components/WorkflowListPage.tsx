        ...
        <div className="bg-default border-subtle overflow-hidden rounded-md border sm:mx-0">
          ...
          >
            {workflows.map((workflow, index) => {
              ...
              const dataTestId = `workflow-${workflow.name
                .toLowerCase()
                .replaceAll(" ", "-")}`;
              return (
                <li
                  ...
                  <div className="first-line:group hover:bg-muted flex w-full items-center justify-between p-4 sm:px-6">
                            ...
                            <Badge variant="gray">
                              <div>
                                ...
                                {workflow.timeUnit && workflow.time && (
                                  <span className="mr-1">
                                    {t(`${workflow.timeUnit.toLowerCase()}`, {
                                      count: workflow.time,
                                    })}
                                  </span>
                                )}
                          ...
                          <li>
                            <Badge variant="gray">
                              ...
                              {workflow.isActiveOnAll ? (
                                ...
                                workflow.activeOn.length > 0 ? (
                                //active on event types badge
                                <Tooltip
                                  content={workflow.activeOn
                                    .filter((wf) =>
                                      workflow.teamId
                                        ? wf.eventType.parentId === null
                                        : true
                                    )
                                    .map((activeOn, key) => (
                                      <p key={key}>
                                ...
                                workflow.activeOnTeams.length > 0 ? (
                                //active on teams badge
                                <Tooltip
                                  content={workflow.activeOnTeams.map(
                                    (activeOn, key) => (
                    ...
                    <div>
                      <div className="hidden md:block">
                        {workflow.team?.name && (
                          <Badge
                            className="mr-4 mt-1 p-[1px] px-2"
                            variant="gray"
                          >
                            <Avatar
