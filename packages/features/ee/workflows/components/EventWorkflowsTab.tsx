...
import useLockedFieldsManager from "@sln/features/ee/managed-event-types/hooks/useLockedFieldsManager";
...
type PartialWorkflowType = Pick<
  WorkflowType,
  "name" | "activeOn" | "isOrg" | "steps" | "id" | "readOnly"
>;
...
const WorkflowListItem = (props: ItemProps) => {
  ...
  const activateEventTypeMutation =
    trpc.viewer.workflows.activateEventType.useMutation({
      onSuccess: async () => {
      ...
      <div className="flex items-center ">
        <div className="bg-subtle mr-4 flex h-10 w-10 items-center justify-center rounded-full text-xs font-medium">
          {getActionIcon(
            workflow.steps,
            isActive
              ? "h-6 w-6 stroke-[1.5px] text-default"
              : "h-6 w-6 stroke-[1.5px] text-muted"
          )}
        </div>
        <div className=" grow">
          <div
          ...
          <div className="flex-none">
            ...
            >
              <Button type="button" color="minimal" className="mr-4">
                ...
                <Icon
                  name="external-link"
                  className="text-default -mt-[2px] h-4 w-4 stroke-2"
                />
              </Button>
            </Link>
          </div>
...
function EventWorkflowsTab(props: Props) {
  ...
  const workflowsDisableProps = shouldLockDisableProps("workflows", {
    simple: true,
  });
  ...
  useEffect(() => {
    if (data?.workflows) {
      const allActiveWorkflows = workflows.map((workflowOnEventType) => {
        const dataWf = data.workflows.find(
          (wf) => wf.id === workflowOnEventType.id
        );
        return {
          ...workflowOnEventType,
          readOnly:
            isChildrenManagedEventType && dataWf?.teamId
              ? true
              : dataWf?.readOnly ?? false,
        } as WorkflowType;
      });
  ...
  const createMutation = trpc.viewer.workflows.create.useMutation({
    onSuccess: async ({ workflow }) => {
      await router.replace(
        `/workflows/${workflow.id}?eventTypeId=${eventType.id}`
      );
    },
    onError: (err) => {
      if (err instanceof HttpError) {
                ...
                {sortedWorkflows.map((workflow) => {
                  return (
                    <WorkflowListItem
                      ...
                      isActive={
                        !!workflows.find(
                          (activeWorkflow) => activeWorkflow.id === workflow.id
                        )
                      }
                    />
                  );
              ...
              <EmptyScreen
                ...
                buttonRaw={
                  <Button
                    ...
                    color="secondary"
                    onClick={() =>
                      createMutation.mutate({ teamId: eventType.team?.id })
                    }
