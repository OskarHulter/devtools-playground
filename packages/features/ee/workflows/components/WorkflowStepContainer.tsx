...
export default function WorkflowStepContainer(props: WorkflowStepProps) {
  ...
  const verifiedEmails =
    _verifiedEmails?.map((verified) => verified.email) || [];
  ...
  if (
    ...
    const templateBody = getWhatsappTemplateForAction(
      step.action,
      step.template,
      timeFormat
    );
  ...
  const addVariableBody = (variable: string) => {
    if (step) {
      ...
      const cursorPosition =
        refReminderBody?.current?.selectionStart || currentMessageBody.length;
      const messageWithAddedVariable = `${currentMessageBody.substring(
        0,
        cursorPosition
      )}{${variable
        .toUpperCase()
        ...
        <div className="flex justify-center">
          <div className="min-w-80 bg-default border-subtle w-full rounded-md border p-7">
            <div className="flex">
              ...
              <div>
                <div className="text-emphasis text-base font-bold">
                  {t("trigger")}
                </div>
    ...
    return (
      <>
        <div className="my-3 flex justify-center">
          <Icon
            name="arrow-down"
            className="text-subtle stroke-[1.5px] text-3xl"
          />
        </div>
        <div className="flex justify-center">
                        ...
                        onChange={(val) => {
                          if (val) {
                            ...
                            if (
                              ...
                              if (
                                isSMSOrWhatsappAction(val.value) ===
                                isSMSOrWhatsappAction(oldValue)
                              ) {
                                if (
                                  isAttendeeAction(oldValue) !==
                                  isAttendeeAction(val.value)
                                ) {
                                  ...
                                  if (!isSMSOrWhatsappAction(val.value)) {
                                    ...
                                    const newEmailSubject = isAttendeeAction(
                                      val.value
                                    )
                                      ? currentEmailSubject.replace(
                                          "{ORGANIZER}",
                                          "{ATTENDEE}"
                                        )
                                      : currentEmailSubject.replace(
                              ...
                              const template = isWhatsappAction(val.value)
                                ? "REMINDER"
                                : "CUSTOM";
                              template &&
                                form.setValue(
                ...
                step.action !== WorkflowActions.SMS_NUMBER ? (
                  <>
                    ...
                    <Editor
                      ...
                      editable={
                        !props.readOnly && !isWhatsappAction(step.action)
                      }
                    />
                  </>
                ) : (
                  ...
                  <div className="mt-3 ">
                    ...
                    >
                      <div className="text-default mt-2 flex text-sm">
                        <Icon
                          name="circle-help"
                          className="mt-[3px] h-3 w-3 ltr:mr-2 rtl:ml-2"
                        />
