...
import { useHasTeamPlan } from "@sln/lib/hooks/useHasPaidPlan";
import { useLocale } from "@sln/lib/hooks/useLocale";
import type { RecordingItemSchema } from "@sln/prisma/zod-utils";
import type { RouterOutputs } from "@sln/trpc/react";
import { trpc } from "@sln/trpc/react";
import type { PartialReference } from "@sln/types/EventManager";
import {
  Button,
  Dialog,
  DialogClose,
...
const useRecordingDownload = () => {
  ...
  const { isFetching, data } =
    trpc.viewer.getDownloadLinkOfCalVideoRecordings.useQuery(
      {
        recordingId,
      },
      {
    ...
    <>
      {recordings && "data" in recordings && recordings?.data?.length > 0 ? (
        <div className="flex flex-col gap-3">
          {recordings.data.map(
            (recording: RecordingItemSchema, index: number) => {
              return (
                <div
...
export const ViewRecordingsDialog = (props: IViewRecordingsDialog) => {
  ...
  const roomName =
    booking?.references?.find(
      (reference: PartialReference) => reference.type === "daily_video"
    )?.meetingId ?? undefined;

  const subtitle = `${booking?.title} - ${dayjs(booking?.startTime).format(
    "ddd"
  )} ${dayjs(booking?.startTime).format("D")}, ${dayjs(
    booking?.startTime
  ).format("MMM")} ${getTimeSpan({
