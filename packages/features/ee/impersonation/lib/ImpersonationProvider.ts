...
import { ensureOrganizationIsReviewed } from "@sln/ee/organizations/lib/ensureOrganizationIsReviewed";
import { getSession } from "@sln/features/auth/lib/getSession";
import { ProfileRepository } from "@sln/lib/server/repository/profile";
import prisma from "@sln/prisma";
import type { Prisma } from "@sln/prisma/client";
import type { Membership } from "@sln/prisma/client";
import type {
  OrgProfile,
  PersonalProfile,
  UserAsPersonalProfile,
} from "@sln/types/UserProfile";
...
const auditAndReturnNextUser = async (
  impersonatedUser: Pick<
    User,
    "id" | "username" | "email" | "name" | "role" | "locale"
  > & {
...
type Credentials =
  | Record<"username" | "teamId" | "returnToId", string>
  | undefined;

export function parseTeamId(creds: Partial<Credentials>) {
  return creds?.teamId
    ? teamIdschema.parse({ teamId: creds.teamId }).teamId
    : undefined;
}

export function checkSelfImpersonation(
  session: Session | null,
  creds: Partial<Credentials>
) {
  if (
    session?.user.username === creds?.username ||
    session?.user.email === creds?.username
  ) {
...
async function isReturningToSelf({
  session,
  creds,
}: {
  session: Session | null;
  ...
  if (returningUser) {
    ...
    const inOrg =
      ...
      returningUser.profiles.some(
        (profile) => profile.organizationId !== undefined
      ); // New way of seeing if the user has a profile in orgs.
...
const ImpersonationProvider = CredentialsProvider({
  ...
  async authorize(creds, req) {
    ...
    if (!teamId)
      throw new Error(
        "Error-teamNotFound: You do not have permission to do this."
      );
    ...
    if (
      sessionUserFromDb?.teams.length === 0 ||
      impersonatedUser.teams.length === 0
    ) {
      throw new Error(
        "Error-UserHasNoTeams: You do not have permission to do this."
      );
    }

    // We find team by ID so we know there is only one team in the array
    if (
      sessionUserFromDb?.teams[0].role === "ADMIN" &&
      impersonatedUser.teams[0].role === "OWNER"
    ) {
      throw new Error("You do not have permission to do this.");
    }
...
async function findProfile(returningUser: {
  id: number;
