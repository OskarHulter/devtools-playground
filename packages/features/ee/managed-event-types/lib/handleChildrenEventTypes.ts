...
import { sendSlugReplacementEmail } from "@sln/emails/email-manager";
import { generateHashedLink } from "@sln/lib/generateHashedLink";
...
import { _EventTypeModel } from "@sln/prisma/zod";
import {
...
const checkExistentEventTypes = async ({
  ...
  "updatedEventType" | "children" | "prisma"
> & {
  userIds: number[];
  teamName: string | null;
}) => {
  const replaceEventType = children?.filter(
    (ch) =>
      ch.owner.eventTypeSlugs.includes(updatedEventType.slug) &&
      userIds.includes(ch.owner.id)
  );
...
export default async function handleChildrenEventTypes({
  ...
  const allManagedEventTypePropsZod = _ManagedEventTypeModel.pick(
    allManagedEventTypeProps
  );
  ...
  const previousUserIds = oldEventType.children?.flatMap(
    (ch) => ch.userId ?? []
  );
  const currentUserIds = children?.map((ch) => ch.owner.id);
  const deletedUserIds = previousUserIds?.filter(
    (id) => !currentUserIds?.includes(id)
  );
  const newUserIds = currentUserIds?.filter(
    (id) => !previousUserIds?.includes(id)
  );
  const oldUserIds = currentUserIds?.filter((id) =>
    previousUserIds?.includes(id)
  );
      ...
      newUserIds.map((userId) => {
        return prisma.eventType.create({
          data: {
            ...
            bookingLimits:
              (managedEventTypeValues.bookingLimits as unknown as Prisma.InputJsonObject) ??
              undefined,
            recurringEvent:
              (managedEventTypeValues.recurringEvent as unknown as Prisma.InputJsonValue) ??
              undefined,
            metadata:
              (managedEventTypeValues.metadata as Prisma.InputJsonValue) ??
              undefined,
            bookingFields:
              (managedEventTypeValues.bookingFields as Prisma.InputJsonValue) ??
              undefined,
            durationLimits:
              (managedEventTypeValues.durationLimits as Prisma.InputJsonValue) ??
              undefined,
            onlyShowFirstAvailableSlot:
              managedEventTypeValues.onlyShowFirstAvailableSlot ?? false,
            userId,
            users: {
              connect: [{ id: userId }],
            },
  ...
  if (oldUserIds?.length) {
    ...
    const { unlockedFields } =
      managedEventTypeValues.metadata?.managedEventConfig;
      ...
      oldUserIds.map((userId) => {
        return prisma.eventType.update({
          ...
          data: {
            ...
            hashedLink:
              "hashedLink" in unlockedFieldProps
                ? undefined
