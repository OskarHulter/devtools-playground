...
import type { _EventTypeModel } from "@sln/prisma/zod/eventtype";
import { Badge, Icon, Switch, Tooltip } from "@sln/ui";
...
export const LockedIndicator = (
  ...
  const tooltipText = t(
    `${isLocked ? "locked" : "unlocked"}_fields_${
      isManagedEventType ? "admin" : "member"
    }_description`
  );
...
const useLockedFieldsManager = ({
  ...
  eventType: Pick<
    ...
    "schedulingType" | "userId" | "metadata" | "id"
  >;
  translate: TFunction;
  ...
  const getLockedInitState = (fieldName: string): boolean => {
    ...
    const unlockedFieldList =
      getValues("metadata")?.managedEventConfig?.unlockedFields;
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    //@ts-ignore
    const fieldIsUnlocked =
      unlockedFieldList && unlockedFieldList[fieldName] === true;
  ...
  const useLockedLabel = (fieldName: string, options?: { simple: true }) => {
    ...
    return {
      disabled:
        ...
        eventType.metadata?.managedEventConfig !== undefined &&
        unlockedFields[
          fieldName as keyof Omit<Prisma.EventTypeSelect, "id">
        ] === undefined,
  ...
  const useShouldLockDisableProps = (
    fieldName: string,
    options?: { simple: true }
  ) => {
    if (!fieldStates[fieldName]) {
