...
import type { orgSettingsSchema } from "@sln/prisma/zod-utils";
import { trpc } from "@sln/trpc/react";
import { Button, Form, Meta, TextField, showToast } from "@sln/ui";
...
const OrgEditView = ({ orgId }: { orgId: number }) => {
  const [org] = trpc.viewer.organizations.adminGet.useSuspenseQuery({
    id: orgId,
  });
  ...
  return (
    <Form form={form} className="space-y-4" handleSubmit={onSubmit}>
      ...
      <p className="text-default mt-2 text-sm">
        Changing the slug would delete the previous organization domain and DNS
        and setup new domain and DNS for the organization.
      </p>
      <TextField
