...
import { md } from "@sln/lib/markdownIt";
import { markdownToSafeHTML } from "@sln/lib/markdownToSafeHTML";
import objectKeys from "@sln/lib/objectKeys";
import slugify from "@sln/lib/slugify";
import turndown from "@sln/lib/turndownService";
...
const OtherTeamProfileView = () => {
  ...
  const isBioEmpty =
    !team || !team.bio || !team.bio.replace("<p><br></p>", "").length;
    ...
    <>
      ...
      {!isPending ? (
        <>
              ...
              handleSubmit={(values) => {
                if (team) {
                  ...
                  objectKeys(variables).forEach((key) => {
                    if (
                      variables[key as keyof typeof variables] === team?.[key]
                    )
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <>
                      <Avatar
                        alt=""
                        imageSrc={getPlaceholderAvatar(value, team?.name)}
                        size="lg"
                      />
                      <div className="ms-4">
                        <ImageUploader
                ...
                render={({ field: { value, onChange } }) => (
                  <div className="mt-8">
                    <TextField
                      ...
                      addOnLeading={
                        team?.parent
                          ? `${team.parent.slug}.${subdomainSuffix()}/`
                          : `${WEBAPP_URL}/team/`
                      }
                      onChange={(e) => {
                  ...
                  <>
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: md.render(markdownToSafeHTML(team.bio)),
                      }}
                    />
                  </>
                )}
          ...
          <div className="text-default mb-3 text-base font-semibold">
            {t("danger_zone")}
          </div>
