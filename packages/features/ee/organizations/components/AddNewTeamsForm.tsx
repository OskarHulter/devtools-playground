...
import { IS_TEAM_BILLING_ENABLED_CLIENT } from "@sln/lib/constants";
...
export const AddNewTeamsForm = () => {
  ...
  const { data: org } = trpc.viewer.teams.get.useQuery({
    teamId: orgId,
    isOrg: true,
  });
  ...
  return (
    <AddNewTeamsFormChild org={orgWithRequestedSlug} teams={regularTeams} />
  );
};
...
const AddNewTeamsFormChild = ({
  ...
  const form = useForm({
    ...
    resolver: async (data) => {
      try {
        ...
        return {
          values: {},
          errors: { teams: { message: "Error validating input" } },
        };
      }
    },
  });
  ...
  const allowWizardCompletionWithoutUpgrading =
  ...
  const createTeamsMutation = trpc.viewer.organizations.createTeams.useMutation(
    {
      async onSuccess(data) {
        if (data.duplicatedSlugs.length) {
          showToast(
            t("duplicated_slugs_warning", {
              slugs: data.duplicatedSlugs.join(", "),
            }),
            "warning"
          );
    ...
    <>
      <Form form={form} handleSubmit={handleFormSubmit}>
                  ...
                  <li key={team.id}>
                    ...
                    {moveTeams[index].shouldMove ? (
                      <TextField
                        ...
                        defaultValue={
                          teams.find((t) => t.id === team.id)?.slug ?? ""
                        }
        ...
        {fields.map((field, index) => (
          <div
            className={classNames("relative", index > 0 ? "mb-2" : "")}
            key={field.id}
          >
            <TextField
              key={field.id}
        ...
        <Button
          ...
          disabled={
            createTeamsMutation.isPending || createTeamsMutation.isSuccess
          }
          onClick={handleFormSubmit}
        >
          {allowWizardCompletionWithoutUpgrading
            ? t("continue")
            : t("checkout")}
        </Button>
      </Form>
