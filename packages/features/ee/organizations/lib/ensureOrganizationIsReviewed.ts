 ...
 */
export async function ensureOrganizationIsReviewed(
  loggedInUserOrgId: number | undefined
) {
  if (loggedInUserOrgId) {
    const org =
      await OrganizationRepository.findByIdIncludeOrganizationSettings({
        id: loggedInUserOrgId,
      });

    if (!org) {
      throw new Error(
        "Error-OrgNotFound: You do not have permission to do this."
      );
    }
    ...
    if (!org.organizationSettings?.isAdminReviewed) {
      ...
      throw new Error(
        "Error-OrgNotReviewed: You do not have permission to do this."
      );
    }
  }
}
