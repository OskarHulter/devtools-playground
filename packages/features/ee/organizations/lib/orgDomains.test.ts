...
import * as constants from "@sln/lib/constants";
import { describe, expect, it } from "vitest";

function setupEnvs({
  WEBAPP_URL = "https://app.oskarhulter.com",
  WEBSITE_URL,
} = {}) {
...
describe("Org Domains Utils", () => {
  describe("getOrgDomainConfigFromHostname", () => {
    it("should return a valid org domain", () => {
      setupEnvs();
      expect(
        getOrgDomainConfigFromHostname({ hostname: "acme.oskarhulter.com" })
      ).toEqual({
    ...
    it("should return a non valid org domain", () => {
      setupEnvs();
      expect(
        getOrgDomainConfigFromHostname({ hostname: "app.oskarhulter.com" })
      ).toEqual({
    ...
    it("should return a non valid org domain for localhost", () => {
      setupEnvs();
      expect(
        getOrgDomainConfigFromHostname({ hostname: "localhost:3000" })
      ).toEqual({
        currentOrgDomain: null,
        isValidOrgDomain: false,
      });
  ...
  describe("getOrgSlug", () => {
    it("should handle a prod web app url with a prod subdomain hostname", () => {
      setupEnvs();
      expect(getOrgSlug("acme.oskarhulter.com")).toEqual("acme");
    });
    ...
    it("should handle a local web app with port url with a non-local subdomain hostname", () => {
      setupEnvs({ WEBAPP_URL: "http://app.cal.local:3000" });
      expect(getOrgSlug("acme.oskarhulter.com:3000")).toEqual(null);
    });
  });
  ...
  describe("getOrgFullOrigin", () => {
    ...
    it("should return the org origin if slug is set", () => {
      setupEnvs({
        ...
        WEBSITE_URL: "https://oskarhulter.com",
      });
