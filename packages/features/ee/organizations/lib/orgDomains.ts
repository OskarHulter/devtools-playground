...
import { IS_PRODUCTION, WEBSITE_URL } from "@sln/lib/constants";
import {
  ALLOWED_HOSTNAMES,
  RESERVED_SUBDOMAINS,
  WEBAPP_URL,
...
export function getOrgSlug(hostname: string, forcedSlug?: string) {
  ...
  if (!currentHostname) {
    log.warn("Match of WEBAPP_URL with ALLOWED_HOSTNAME failed", {
      WEBAPP_URL,
      ALLOWED_HOSTNAMES,
    });
    return null;
...
export function orgDomainConfig(
  ...
  const forcedSlug =
    forcedSlugHeader instanceof Array ? forcedSlugHeader[0] : forcedSlugHeader;
...
export function getOrgDomainConfigFromHostname({
  ...
  const isValidOrgDomain =
    ...
    !RESERVED_SUBDOMAINS.includes(currentOrgDomain);
...
export function subdomainSuffix() {
  ...
  const urlSplit = WEBAPP_URL.replace("https://", "")
    ?.replace("http://", "")
    .split(".");
  return urlSplit.length === 3
    ? urlSplit.slice(1).join(".")
    : urlSplit.join(".");
}
...
export function userOrgQuery(
  ...
  return isValidOrgDomain && currentOrgDomain
