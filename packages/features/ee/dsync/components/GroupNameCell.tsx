...
import { Badge, Icon, showToast, TextField } from "@sln/ui";
  ...
  const createMutation = trpc.viewer.dsync.teamGroupMapping.create.useMutation({
    onSuccess: (data) => {
      utils.viewer.dsync.teamGroupMapping.get.setData(undefined, (prev) => {
        if (prev) {
          const teamIndex = prev.teamGroupMapping.findIndex(
            (team) => team.id === teamId
          );
    ...
    onSuccess: (data) => {
      utils.viewer.dsync.teamGroupMapping.get.setData(undefined, (prev) => {
        if (prev) {
          ...
          const indexToRemove = prev.teamGroupMapping[
            teamIndex
          ].groupNames.indexOf(data.deletedGroupName);
  ...
  return (
    <div className="flex items-center space-x-4">
      ...
      <Badge
        variant="gray"
        size="lg"
        className={classNames(!showTextInput && "hover:bg-emphasis")}
      >
        <div
        ...
        >
          ...
          <div
            className={classNames(
              "rounded p-1",
              showTextInput && "hover:bg-emphasis ml-2"
            )}
          >
            <Icon
              name="plus"
              className="h-4 w-4 stroke-[3px]"
              onClick={() => addGroupName(newGroupName)}
            />
          </div>
        </div>
      </Badge>
