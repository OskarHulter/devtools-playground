...
} from "@sln/trpc/server/routers/viewer/teams/inviteMember/utils";

const handleGroupEvents = async (
  ...
  if (isEmail) {
    userEmails = eventData.raw.members.map(
      (member: { display: string }) => member.display
    );
  }
  ...
  for (const group of groupNames) {
    ...
    const newMembers = users.filter(
      (user) => !user.teams.find((team) => team.id === group.teamId)
    );
