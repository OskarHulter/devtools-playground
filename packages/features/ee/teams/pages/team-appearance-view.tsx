...
import BrandColorsForm from "@sln/features/ee/components/BrandColorsForm";
import { AppearanceSkeletonLoader } from "@sln/features/ee/components/CommonSkeletonLoaders";
import SectionBottomActions from "@sln/features/settings/SectionBottomActions";
...
import { Button, Form, Meta, showToast, SettingsToggle } from "@sln/ui";
...
const ProfileView = ({ team }: ProfileViewProps) => {
  ...
  const [hideBookATeamMember, setHideBookATeamMember] = useState(
    team?.hideBookATeamMember ?? false
  );
  ...
  const isAdmin =
    ...
    (team.membership.role === MembershipRole.OWNER ||
      team.membership.role === MembershipRole.ADMIN);
...
const ProfileViewWrapper = () => {
  ...
  const {
    ...
    isPending,
    error,
  } = trpc.viewer.teams.get.useQuery({ teamId: Number(params.id) });
