...
import { Button, showToast, Tooltip } from "@sln/ui";
...
export function GoogleWorkspaceInviteButton(
  ...
  const { data: credential } =
    trpc.viewer.googleWorkspace.checkForGWorkspace.useQuery();
  const { data: hasGcalInstalled } =
    trpc.viewer.appsRouter.checkGlobalKeys.useQuery({
      slug: "google-calendar",
    });
  const mutation =
    trpc.viewer.googleWorkspace.getUsersFromGWorkspace.useMutation({
      onSuccess: (data) => {
  ...
  if (
    featureFlags["google-workspace-directory"] == false ||
    !hasGcalInstalled
  ) {
    return null;
  }
