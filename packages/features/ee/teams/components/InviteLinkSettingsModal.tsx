...
export default function InviteLinkSettingsModal(
  props: InvitationLinkSettingsModalProps
) {
  const { t } = useLocale();
  ...
  const setInviteExpirationMutation =
    trpc.viewer.teams.setInviteExpiration.useMutation({
      onSuccess: () => {
        ...
        <Form form={linkSettingsFormMethods} handleSubmit={handleSubmit}>
            ...
            render={({ field: { onChange } }) => (
              <div className="-mt-2">
                ...
                <Select
                  ...
                  defaultValue={expiresInDaysOption.find(
                    (option) => option.value === props.expiresInDays
                  )}
                  className="w-full"
          ...
          <DialogFooter showDivider className="mt-10">
            <Button
              ...
              color="secondary"
              onClick={() =>
                deleteInviteMutation.mutate({ token: props.token })
              }
              className="mr-auto"
