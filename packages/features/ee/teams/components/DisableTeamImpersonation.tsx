...
import { showToast, SettingsToggle } from "@sln/ui";
...
const DisableTeamImpersonation = ({
  ...
  const query = trpc.viewer.teams.getMembershipbyUser.useQuery({
    teamId,
    memberId,
  });
  ...
  const [allowImpersonation, setAllowImpersonation] = useState(
    !query.data?.disableImpersonation ?? true
  );
      ...
      <SettingsToggle
        ...
        onCheckedChange={(_allowImpersonation) => {
          ...
          mutation.mutate({
            teamId,
            memberId,
            disableImpersonation: !_allowImpersonation,
          });
        }}
