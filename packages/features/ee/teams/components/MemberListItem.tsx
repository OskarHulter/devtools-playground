...
export default function MemberListItem(props: Props) {
  ...
  const [showChangeMemberRoleModal, setShowChangeMemberRoleModal] =
    useState(false);
  const [showTeamAvailabilityModal, setShowTeamAvailabilityModal] =
    useState(false);
  ...
  const bookingLink =
    ...
    `${bookerUrlWithoutProtocol}/${props.member.username}`;
  const isAdmin =
    props.team && ["ADMIN", "OWNER"].includes(props.team.membership?.role);
  const appList = props.member.connectedApps?.map(
    ({ logo, name, externalId }) => {
      return logo ? (
        externalId ? (
        ...
        <div className="flex w-full flex-col justify-between truncate sm:flex-row">
          <div className="flex">
            ...
            <div className="ms-3 inline-block">
              <div
                className="mb-1 flex"
                data-testid={`member-${props.member.username}`}
              >
                <span
                  data-testid="member-name"
                  className="text-default mr-2 text-sm font-bold leading-4"
                >
                  {name}
                </span>
                {!props.member.accepted && (
      ...
      {showTeamAvailabilityModal && (
        ...
        >
          <DialogContent type="creation" size="md">
            ...
            <DialogFooter>
              <Button onClick={() => setShowTeamAvailabilityModal(false)}>
                {t("done")}
              </Button>
            </DialogFooter>
          </DialogContent>
