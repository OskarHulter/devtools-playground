import { getStripeCustomerIdFromUserId } from "@sln/app-store/stripepayment/lib/customer";
...
import { BillingPeriod } from "@sln/prisma/zod-utils";
...
export const purchaseTeamOrOrgSubscription = async (input: {
  ...
  const seats = isOrg
    ? Math.max(seatsUsed, MINIMUM_NUMBER_OF_ORG_SEATS)
    : seatsUsed;
  ...
  async function createPrice({
    ...
    try {
      ...
      log.error(
        `Error creating custom price for ${
          isOrg ? "Organization" : "Team"
        } ID: ${teamId}`,
        safeStringify(e)
      );
  ...
  async function getFixedPrice() {
    ...
    log.debug(
      "Getting price ID",
      safeStringify({ fixedPriceId, isOrg, teamId, pricePerSeat })
    );
...
export const updateQuantitySubscriptionFromStripe = async (teamId: number) => {
  try {
    ...
    const orgMinimumSubscriptionQuantity =
      orgSeats || MINIMUM_NUMBER_OF_ORG_SEATS;
    ...
    if (
      team.isOrganization &&
      membershipCount < orgMinimumSubscriptionQuantity
    ) {
      console.info(
