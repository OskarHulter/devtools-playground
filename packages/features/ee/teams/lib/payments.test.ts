...
vi.mock("@sln/app-store/stripepayment/lib/customer", () => {
  return {
...
vi.mock("@sln/lib/constant", () => {
  return {
    MINIMUM_NUMBER_OF_ORG_SEATS: 30,
  };
});

vi.mock("@sln/app-store/stripepayment/lib/server", () => {
  return {
    default: {
      checkout: {
        sessions: {
...
function mockStripeCheckoutSessionRetrieve(data, expectedArgs) {
  return vi
    .mocked(stripe.checkout.sessions.retrieve)
    .mockImplementation(async (sessionId) =>
      ...
      //@ts-ignore
      {
        ...
        return new Promise((resolve) =>
          resolve(conditionMatched ? data : null)
        );
      }
    );
}
...
function mockStripeSubscriptionsRetrieve(data, expectedArgs) {
  return vi.mocked(stripe.subscriptions.retrieve).mockImplementation(
    ...
    async (subscriptionId) => {
      const conditionMatched = expectedArgs
        ? expectedArgs[0] === subscriptionId
        : true;
      return new Promise((resolve) =>
        resolve(conditionMatched ? data : undefined)
      );
    }
  );
}
