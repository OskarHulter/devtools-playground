import { deriveOrgNameFromEmail } from "@sln/ee/organizations/components/CreateANewOrganizationForm";
import { deriveSlugFromEmail } from "@sln/ee/organizations/components/CreateANewOrganizationForm";
import { useLocale } from "@sln/lib/hooks/useLocale";
import slugify from "@sln/lib/slugify";
import { telemetryEventTypes, useTelemetry } from "@sln/lib/telemetry";
import { UserPermissionRole } from "@sln/prisma/enums";
import { trpc } from "@sln/trpc/react";
import type { Ensure } from "@sln/types/utils";
import { Alert, Form, TextField, Button } from "@sln/ui";
...
const CreateANewPlatformFormChild = ({
  session,
}: {
  ...
  const createOrganizationMutation =
    trpc.viewer.organizations.create.useMutation({
      ...
      onError: (err) => {
        if (err.message === "organization_url_taken") {
          ...
          err.message === "domain_taken_team" ||
          err.message === "domain_taken_project"
        ) {
          newOrganizationFormMethods.setError("slug", {
            type: "custom",
              ...
              <>
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    if (
                      newOrganizationFormMethods.formState.touchedFields[
                        "slug"
                      ] === undefined
                    ) {
                      newOrganizationFormMethods.setValue(
