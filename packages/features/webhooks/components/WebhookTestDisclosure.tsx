...
import { ZTestTriggerInputSchema } from "@sln/trpc/server/routers/viewer/webhook/testTrigger.schema";
import { Badge, Button, showToast } from "@sln/ui";
import { useWatch } from "react-hook-form";
import { ZodError } from "zod";

export default function WebhookTestDisclosure() {
  const [subscriberUrl, webhookSecret]: [string, string] = useWatch({
    name: ["subscriberUrl", "secret"],
  });
  ...
  return (
    <>
      <div className="border-subtle flex justify-between rounded-t-lg border p-6">
        <div>
          <p className="text-emphasis text-sm font-semibold leading-5">
            {t("webhook_test")}
          </p>
