...
const WEBHOOK_TRIGGER_EVENTS_GROUPED_BY_APP_V2: Record<
  string,
  WebhookTriggerEventOptions
> = {
  core: [
    {
      value: WebhookTriggerEvents.BOOKING_CANCELLED,
    ...
    {
      value: WebhookTriggerEvents.BOOKING_REQUESTED,
      label: "booking_requested",
    },
    {
      value: WebhookTriggerEvents.BOOKING_PAYMENT_INITIATED,
      label: "booking_payment_initiated",
    },
    {
      value: WebhookTriggerEvents.BOOKING_RESCHEDULED,
      label: "booking_rescheduled",
    },
    { value: WebhookTriggerEvents.BOOKING_PAID, label: "booking_paid" },
    {
      value: WebhookTriggerEvents.BOOKING_NO_SHOW_UPDATED,
      label: "booking_no_show_updated",
    },
...
const WebhookForm = (props: {
  ...
  const translatedTriggerOptions = triggerOptions.map((option) => ({
    ...option,
  ...
  const getEventTriggers = () => {
    ...
    return (
      selectOnlyInstantMeetingOption
        ? translatedTriggerOptions.filter(
            (option) => option.value === WebhookTriggerEvents.INSTANT_MEETING
          )
        : translatedTriggerOptions.filter(
            (option) => option.value !== WebhookTriggerEvents.INSTANT_MEETING
          )
    ...
    >
      <div className="border-subtle border-x p-6">
                ...
                onChange={(e) => {
                  ...
                  if (hasTemplateIntegration({ url: e.target.value })) {
                    ...
                    formMethods.setValue(
                      "payloadTemplate",
                      customTemplate({ url: e.target.value }),
                      {
                        shouldDirty: true,
                      }
                    );
        ...
        <Controller
          ...
          render={({ field: { onChange, value } }) => {
            const selectValue = translatedTriggerOptions.filter((option) =>
      ...
      <SectionBottomActions align="end">
        <Button
          ...
          {...(!props.onCancel
            ? { href: `${WEBAPP_URL}/settings/developer/webhooks` }
            : {})}
        >
          {t("cancel")}
        </Button>
