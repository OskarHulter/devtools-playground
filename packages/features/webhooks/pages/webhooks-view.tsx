...
import type { WebhooksByViewer } from "@sln/trpc/server/routers/viewer/webhook/getByViewer.handler";
import {
  Avatar,
...
const WebhooksView = () => {
  ...
  const { data, isPending } = trpc.viewer.webhook.getByViewer.useQuery(
    undefined,
    {
  ...
  return (
    <>
      <Meta
        ...
        borderInShellHeader={
          (data && data.profiles.length === 1) || !data?.webhookGroups?.length
        }
      />
      <div>
        <WebhooksList
          webhooksByViewer={data}
          isAdmin={isAdmin}
          createFunction={createFunction}
        />
      </div>
