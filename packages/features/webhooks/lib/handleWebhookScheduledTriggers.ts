  ...
  for (const job of jobsToRun) {
    ...
    if (!webhook && job.jobName) {
      ...
      try {
        ...
        logger.error(
          `Error finding webhook for subscriberId: ${subscriberId}, appId: ${appId}`
        );
      }
    }
    ...
    const headers: Record<string, string> = {
      "Content-Type":
        !job.payload || jsonParse(job.payload)
          ? "application/json"
    ...
    fetchPromises.push(
      fetch(job.subscriberUrl, {
        ...
        console.error(
          `Webhook trigger for subscriber url ${job.subscriberUrl} failed with error: ${error}`
        );
      })
    );
