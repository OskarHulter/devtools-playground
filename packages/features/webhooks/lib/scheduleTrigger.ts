...
import { getHumanReadableLocationValue } from "@sln/core/location";
...
import type { ApiKey } from "@sln/prisma/client";
...
export async function addSubscription({
  ...
  try {
    const userId = appApiKey
      ? appApiKey.userId
      : account && !account.isTeam
      ? account.id
      : null;
    const teamId = appApiKey
      ? appApiKey.teamId
      : account && account.isTeam
      ? account.id
      : null;
    ...
    log.error(
      `Error creating subscription for ${
...
export async function listBookings(
  ...
  try {
    ...
    log.error(
      `Error retrieving list of bookings for ${
        teamId ? `team ${teamId}` : `user ${userId}`
      }.`,
      safeStringify(err)
    );
  ...
  try {
    if (appId && (userId || teamId)) {
      ...
      if (booking) {
        ...
        const where: Prisma.WebhookScheduledTriggersWhereInput = {
          webhookId: webhookId,
        };
...
export async function updateTriggerForExistingBookings(
  ...
  const addedEventTriggers = updatedEventTriggers.filter(
    (trigger) =>
      !existingEventTriggers.includes(trigger) &&
      SCHEDULING_TRIGGER.includes(trigger)
  );
  const removedEventTriggers = existingEventTriggers.filter(
    (trigger) =>
      !updatedEventTriggers.includes(trigger) &&
      SCHEDULING_TRIGGER.includes(trigger)
  );

  if (addedEventTriggers.length === 0 && removedEventTriggers.length === 0)
    return;
