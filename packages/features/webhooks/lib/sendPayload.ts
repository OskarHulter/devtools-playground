...
import type { TGetTranscriptAccessLink } from "@sln/app-store/dailyvideo/zod";
import { getHumanReadableLocationValue } from "@sln/app-store/locations";
...
function addUTCOffset(
  ...
  if (data?.attendees?.length) {
    (data.attendees as (Person & UTCOffset)[]).forEach((attendee) => {
      attendee.utcOffset = getUTCOffsetByTimezone(
        attendee.timeZone,
        data.startTime
      );
    });
...
function getZapierPayload(
  data: WithUTCOffsetType<
    CalendarEvent & EventTypeInfo & { status?: string; createdAt: string }
  >
): string {
  const attendees = (data.attendees as (Person & UTCOffset)[]).map(
    (attendee) => {
      return {
        name: attendee.name,
...
export const createWebhookSignature = (params: {
  secret?: string | null;
  body: string;
}) =>
  params.secret
...
const _sendPayload = async (
  ...
  const response = await fetch(subscriberUrl, {
    ...
    headers: {
      ...
      "X-Cal-Signature-256": createWebhookSignature({
        secret: secretKey,
        body,
      }),
    },
