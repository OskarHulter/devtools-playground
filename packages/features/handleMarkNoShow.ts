import { WebhookService } from "@sln/features/webhooks/lib/WebhookService";
...
import type { TNoShowInputSchema } from "@sln/trpc/server/routers/publicViewer/noShow.schema";
...
const getResultPayload = async (
  ...
  if (attendees.length === 1) {
    ...
    return {
      messageKey: attendee.noShow
        ? "x_marked_as_no_show"
        : "x_unmarked_as_no_show",
      attendees: [attendee],
    };
  }
  return {
  ...
  try {
    ...
    if (attendees && attendeeEmails.length > 0) {
      ...
      const _attendees = results
        ...
        .map(
          (x) =>
            (x as PromiseFulfilledResult<{ noShow: boolean; email: string }>)
              .value
        )
      ...
      const message = t(payload.messageKey, {
        x: payload.attendees[0]?.email || "User",
      });
