import { getDefinedBufferTimes } from "../../examples/features/features/eventtypes/lib/getDefinedBufferTimes";
import type { Booking, EventType } from "@prisma/client";
import { getBusyCalendarTimes } from "@sln/core/CalendarManager";
import dayjs from "@sln/dayjs";
import { subtract } from "@sln/lib/date-ranges";
import { intervalLimitKeyToUnit } from "@sln/lib/intervalLimit";
import logger from "@sln/lib/logger";
import { getPiiFreeBooking } from "@sln/lib/piiFreeData";
import { performance } from "@sln/lib/server/perfObserver";
import prisma from "@sln/prisma";
import type { Prisma, SelectedCalendar } from "@sln/prisma/client";
import { BookingStatus } from "@sln/prisma/enums";
import { stringToDayjs } from "@sln/prisma/zod-utils";
import type { EventBusyDetails, IntervalLimit } from "@sln/types/Calendar";
...
export async function getBusyTimes(params: {
   ...
   * https://github.com/calcom/oskarhulter.com/issues/6374
   *
  ...
  const startTimeAdjustedWithMaxBuffer = dayjs(startTimeDate)
    .subtract(maxBuffer, "minute")
    .toDate();
  const endTimeAdjustedWithMaxBuffer = dayjs(endTimeDate)
    .add(maxBuffer, "minute")
    .toDate();
  ...
  const busyTimes = bookings.reduce(
    (
      ...
      if (rest._count?.seatsReferences) {
        const bookedAt = `${dayjs(startTime).utc().format()}<>${dayjs(endTime)
          .utc()
          .format()}`;
...
export async function getBusyTimesForLimitChecks(params: {
  ...
  busyTimes = bookings.map(
    ({ id, startTime, endTime, eventType, title, userId }) => ({
