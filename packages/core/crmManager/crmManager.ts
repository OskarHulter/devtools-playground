import getCrm from "@sln/app-store/_utils/getCrm";
...
import type { CRM, ContactCreateInput } from "@sln/types/CrmService";
...
export default class CrmManager {
  ...
  public async createEvent(event: CalendarEvent) {
    ...
    let contacts =
      (await this.getContacts(event.attendees.map((a) => a.email))) || [];
    ...
    const contactsToCreate = event.attendees.filter(
      (attendee) =>
        !contacts.some((contact) => contact.email === attendee.email)
    );
  ...
  public async createContacts(contactsToCreate: ContactCreateInput[]) {
    ...
    const createdContacts =
      (await crmService?.createContacts(contactsToCreate)) || [];
