...
const log = logger.getSubLogger({
  prefix: ["builders", "CalendarEvent", "director"],
});
export class CalendarEventDirector {
  ...
  public async buildForRescheduleEmail(): Promise<void> {
    if (
      ...
      this.existingBooking.eventTypeId &&
      this.existingBooking.uid
    ) {
      await this.builder.buildEventObjectFromInnerClass(
        this.existingBooking.eventTypeId
      );
      ...
      this.builder.buildRescheduleLink(
        this.existingBooking,
      ...
      log.debug(
        ...
        safeStringify({
          existingBooking: this.existingBooking,
