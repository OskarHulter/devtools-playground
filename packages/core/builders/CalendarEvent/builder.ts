...
const log = logger.getSubLogger({
  prefix: ["builders", "CalendarEvent", "builder"],
});
...
export class CalendarEventBuilder implements ICalendarEventBuilder {
  ...
  public buildAttendeesList() {
    ...
    this.attendeesList = [
      ...(this.calendarEvent
        .attendees as unknown as PersonAttendeeCommonFields[]),
      ...this.teamMembers,
    ];
  }
  ...
  private async getEventFromEventId(eventTypeId: number) {
    ...
    log.debug(
      "getEventFromEventId.resultEventType",
      safeStringify(resultEventType)
    );
  ...
  public buildUIDCalendarEvent() {
    ...
    const seed = `${mainOrganizer.username}:${dayjs(
  ...
  public setDestinationCalendar(
    destinationCalendar: CalendarEventClass["destinationCalendar"]
  ) {
    this.calendarEvent.destinationCalendar = destinationCalendar;
  }

  public setHideCalendarNotes(
    hideCalendarNotes: CalendarEventClass["hideCalendarNotes"]
  ) {
  ...
  public setCancellationReason(
    cancellationReason: CalendarEventClass["cancellationReason"]
  ) {
  ...
  public buildRescheduleLink(
    ...
    eventType?: CalendarEventBuilder["eventType"]
  ) {
    try {
      if (!booking) {
        throw new Error(
          "Parameter booking is required to build reschedule link"
        );
      }
      const isTeam = !!eventType && !!eventType.teamId;
      const isDynamic =
        booking?.dynamicEventSlugRef && booking?.dynamicGroupSlugRef;

      let slug = "";
      if (isTeam && eventType?.team?.slug) {
        slug = `team/${eventType.team?.slug}/${eventType.slug}`;
      } else if (isDynamic) {
        const dynamicSlug = isDynamic
          ? `${booking.dynamicGroupSlugRef}/${booking.dynamicEventSlugRef}`
          : "";
        slug = dynamicSlug;
