...
import type { WorkingHours } from "@sln/types/schedule";
...
export const getAggregateWorkingHours = (
  usersWorkingHoursAndBusySlots: (Omit<
    Awaited<
      ReturnType<
        Awaited<typeof import("./getUserAvailability")>["getUserAvailability"]
      >
    >,
  ...
  const looseHostWorkingHours = usersWorkingHoursAndBusySlots
    .filter(
      ({ user }) =>
        schedulingType !== SchedulingType.COLLECTIVE && user?.isFixed !== true
    )
    ...
    s.workingHours.forEach((workingHour) => {
      ...
      updatedWorkingHours.push(
        ...sameDayWorkingHours.map((compare) => {
          const intersect = workingHour.days.filter((day) =>
            compare.days.includes(day)
          );
