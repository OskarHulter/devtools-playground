...
function parseName(
  name: z.infer<typeof nameObjectSchema> | string | undefined
) {
...
export function getEventName(
  ...
  if (
    eventNameObj.eventName.includes("{Location}") ||
    eventNameObj.eventName.includes("{LOCATION}")
  ) {
  ...
  let dynamicEventName = eventName
    ...
    .replaceAll(
      "{HOST/ATTENDEE}",
      forAttendeeView ? eventNameObj.host : attendeeName
    );
  ...
  const customInputvariables = dynamicEventName
    .match(/\{(.+?)}/g)
    ?.map((variable) => {
