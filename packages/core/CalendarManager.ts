...
import type { CalendarEvent, EventBusyDate, IntegrationCalendar, NewCalendarEventType } from "@sln/types/Calendar";
import type { CredentialPayload } from "@sln/types/Credential";
import type { EventResult } from "@sln/types/EventManager";
    ...
    calendarCredentials.map(async (item) => {
      try {
        ...
        const calendars = sortBy(
          cals.map((cal: IntegrationCalendar) => {
            ...
            return {
              ...
              isSelected: selectedCalendars.some(
        ...
        const primary =
          ...
          calendars.find((cal) => cal !== undefined);
        if (!primary) {
          return {
            integration,
            credentialId,
        ...
        // HACK https://github.com/calcom/oskarhulter.com/pull/7644/files#r1131508414
...
const cleanIntegrationKeys = (
  appIntegration: ReturnType<
    typeof getCalendarCredentials
  >[number]["integration"] & {
    credentials?: Array<CredentialPayload>;
...
export const getCachedResults = async (
  ...
  const calendarCredentials = withCredentials.filter((credential) =>
    credential.type.endsWith("_calendar")
  );
  const calendars = await Promise.all(
    calendarCredentials.map((credential) => getCalendar(credential))
  );
  ...
  const results = calendars.map(async (c, i) => {
     ...
     */
    const passedSelectedCalendars = selectedCalendars.filter(
      (sc) => sc.integration === type
    );
    if (!passedSelectedCalendars.length) return [];
    /** We extract external Ids so we don't cache too much */
    const selectedCalendarIds = passedSelectedCalendars.map(
      (sc) => sc.externalId
    );
  ...
  performance.measure(
    `getBusyCalendarTimes took $1 for creds ${calendarCredentials.map(
      (cred) => cred.id
    )}`,
  ...
  const creationResult = calendar
    ? await calendar
        ...
        .catch(async (error: { code: number; calError: string }) => {
          ...
          //https://github.com/calcom/oskarhulter.com/issues/3949
...
export const updateEvent = async (
    ...
    calendar && bookingRefUid
      ? await calendar
          ...
          .catch(async (e: { calError: string }) => {
            // @TODO: This code will be off till we can investigate an error with it
            // @see https://github.com/calcom/oskarhulter.com/issues/3949
            // await sendBrokenIntegrationEmail(calEvent, "calendar");
            log.error(
              "updateEvent failed",
  ...
  if (Array.isArray(updatedResult)) {
    calWarnings = updatedResult.flatMap(
      (res) => res.additionalInfo?.calWarnings ?? []
    );
  } else {
