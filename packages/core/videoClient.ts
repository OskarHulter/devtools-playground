import appStore from "@sln/app-store";
import { getDailyAppKeys } from "@sln/app-store/dailyvideo/lib/getDailyAppKeys";
import { DailyLocationType } from "@sln/app-store/locations";
import { sendBrokenIntegrationEmail } from "@sln/emails";
import { getUid } from "@sln/lib/CalEventParser";
import logger from "@sln/lib/logger";
import { getPiiFreeCalendarEvent, getPiiFreeCredential } from "@sln/lib/piiFreeData";
import { safeStringify } from "@sln/lib/safeStringify";
import { prisma } from "@sln/prisma";
import type { GetRecordingsResponseSchema } from "@sln/prisma/zod-utils";
import type { CalendarEvent, EventBusyDate } from "@sln/types/Calendar";
...
import type { VideoApiAdapter, VideoApiAdapterFactory, VideoCallData } from "@sln/types/VideoApiAdapter";
...
const getVideoAdapters = async (
  ...
  for (const cred of withCredentials) {
    ...
    log.silly(
      ...
      safeStringify({ appName, cred: getPiiFreeCredential(cred) })
    );
    ...
    if ("lib" in app && "VideoApiAdapter" in app.lib) {
      const makeVideoApiAdapter = app.lib
        .VideoApiAdapter as VideoApiAdapterFactory;
...
const getBusyVideoTimes = async (withCredentials: CredentialPayload[]) =>
  Promise.all(
    (await getVideoAdapters(withCredentials)).map((c) => c?.getAvailability())
  ).then((results) =>
    results.reduce(
      (acc, availability) => acc.concat(availability),
      [] as (EventBusyDate | undefined)[]
    )
  );
...
const updateMeeting = async (
  ...
  const updatedMeeting = canCallUpdateMeeting
    ? await firstVideoAdapter
        ?.updateMeeting(bookingRef, calEvent)
        .catch(async (e) => {
          await sendBrokenIntegrationEmail(calEvent, "video");
...
// @TODO: This is a temporary solution to create a meeting with oskarhulter.com video as fallback url
...
const checkIfRoomNameMatchesInRecording = async (
  roomName: string,
  recordingId: string
) => {
  let dailyAppKeys: Awaited<ReturnType<typeof getDailyAppKeys>>;
  ...
  return videoAdapter?.checkIfRoomNameMatchesInRecording?.(
    roomName,
    recordingId
  );
};
