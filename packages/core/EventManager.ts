...
import { FAKE_DAILY_CREDENTIAL } from "@sln/app-store/dailyvideo/lib/VideoApiAdapter";
import { appKeysSchema as calVideoKeysSchema } from "@sln/app-store/dailyvideo/zod";
import { getEventLocationTypeFromApp, MeetLocationType } from "@sln/app-store/locations";
import getApps from "@sln/app-store/utils";
import { getUid } from "@sln/lib/CalEventParser";
import logger from "@sln/lib/logger";
import { getPiiFreeDestinationCalendar, getPiiFreeUser, getPiiFreeCredential, getPiiFreeCalendarEvent } from "@sln/lib/piiFreeData";
...
import { createdEventSchema } from "@sln/prisma/zod-utils";
import type { AdditionalInformation, CalendarEvent, NewCalendarEventType } from "@sln/types/Calendar";
import type { CredentialPayload } from "@sln/types/Credential";
import type { Event } from "@sln/types/Event";
import type { CreateUpdateResult, EventResult, PartialBooking, PartialReference } from "@sln/types/EventManager";
...
export default class EventManager {
  ...
  constructor(user: EventManagerUser) {
    log.silly(
      ...
      safeStringify({ user: getPiiFreeUser(user) })
    );
    ...
    this.calendarCredentials = appCredentials
      .filter(
        ...
        (cred) =>
          ...
          !cred.type.includes("other_calendar")
      )
      //see https://github.com/calcom/oskarhulter.com/issues/11671#issue-1923600672
    ...
    this.crmCredentials = appCredentials.filter(
      (cred) =>
        cred.type.endsWith("_crm") || cred.type.endsWith("_other_calendar")
    );
  }
  ...
  public async create(event: CalendarEvent): Promise<CreateUpdateResult> {
    ...
    if (
      ...
      log.warn(
        "Falling back to Cal Video integration as Google Calendar not installed"
      );
      evt["location"] = "integrations:daily";
    }
  ...
  private async deleteVideoEventForBookingReference({
    ...
    log.debug(
      ...
      safeStringify({ bookingVideoReference: reference })
    );
  ...
  public async reschedule(
    ...
    if (!rescheduleUid) {
      throw new Error(
        "You called eventManager.update without an `rescheduleUid`. This should never happen."
      );
    }
    ...
    if (evt.requiresConfirmation) {
      ...
      if (changedOrganizer) {
        ...
        log.debug(
          "RescheduleOrganizerChanged: Creating Event and Meeting for for new booking"
        );

        const createdEvent = await this.create(originalEvt);
        results.push(...createdEvent.results);
        bookingReferenceChangedOrganizer.push(
          ...createdEvent.referencesToCreate
        );
      } else {
        ...
        if (bookingCalendarReference) {
          ...
          results.push(
            ...(await this.updateAllCalendarEvents(evt, booking, newBookingId))
          );
        }
    ...
    return {
      results,
      referencesToCreate: changedOrganizer
  ...
  private async deleteEventsAndMeetings({
    ...
    for (const reference of bookingReferences) {
      if (
        reference.type.includes("_calendar") &&
        !reference.type.includes("other_calendar")
      ) {
    ...
    log.debug(
      ...
      safeStringify({ calendarReferences, videoReferences })
    );
    ...
    if (!allPromises.length) {
      log.warn(
        "No calendar or video references found for booking - Couldn't delete events or meetings"
      );
    }
  }
  ...
  private async createAllCalendarEvents(event: CalendarEvent) {
    ...
    const fallbackToFirstCalendarInTheList = async () => {
      ...
      const [credential] = this.calendarCredentials.filter(
        (cred) => !cred.type.endsWith("other_calendar")
      );
      if (credential) {
    ...
    if (event.destinationCalendar && event.destinationCalendar.length > 0) {
      ...
      const destinationCalendars: DestinationCalendar[] =
        event.destinationCalendar.reduce((destinationCals, cal) => {
      ...
      for (const destination of destinationCalendars) {
        ...
        if (destination.credentialId) {
          let credential = this.calendarCredentials.find(
            (c) => c.id === destination.credentialId
          );
          if (!credential) {
          ...
          const destinationCalendarCredentials =
            this.calendarCredentials.filter(
          ...
          if (!firstCalendarCredential) {
            ...
            log.warn(
              ...
              safeStringify({
                ...
                firstConnectedCalendar: getPiiFreeCredential(
                  firstCalendarCredential
                ),
              })
            );
            createdEvents.push(
              await createEvent(firstCalendarCredential, event)
            );
            eventCreated = true;
          }
        }
  ...
  private async updateAllCalendarEvents(
    ...
    log.silly(
      ...
      JSON.stringify({ event, booking, newBookingId })
    );
    try {
      ...
      calendarReference = newBooking?.references.length
        ? newBooking.references.filter((reference) =>
            reference.type.includes("_calendar")
          )
        : booking.references.filter((reference) =>
            reference.type.includes("_calendar")
          );
      ...
      for (const reference of calendarReference) {
        ...
        if (reference.credentialId) {
          ...
          result.push(
            updateEvent(credential, event, bookingRefUid, calenderExternalId)
          );
        } else {
      ...
      if (newBookingId) {
        const oldCalendarEvent = booking.references.find((reference) =>
          reference.type.includes("_calendar")
        );
        ...
        if (oldCalendarEvent?.credentialId) {
          ...
          await calendar?.deleteEvent(
            oldCalendarEvent.uid,
            event,
            oldCalendarEvent.externalCalendarId
          );
        }
      }
   ...
   * @private
   */
  private async updateVideoEvent(
    event: CalendarEvent,
    booking: PartialBooking
  ) {
    const credential = this.getVideoCredential(event);
    ...
    if (credential) {
      const bookingRef = booking
        ? booking.references.filter((ref) => ref.type === credential.type)[0]
        : null;
  ...
  private async updateAllCRMEvents(
    event: CalendarEvent,
    booking: PartialBooking
  ) {
    const updatedEvents = [];
    ...
    for (const reference of booking.references) {
      const credential = this.crmCredentials.find(
        (cred) => cred.id === reference.credentialId
      );
      let success = true;
      if (credential) {
        const crm = new CrmManager(credential);
        const updatedEvent = await crm
          .updateEvent(reference.uid, event)
          .catch((error) => {
            success = false;
