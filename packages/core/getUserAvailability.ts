...
import { getWorkingHours } from "@sln/lib/availability";
import type { DateOverride, WorkingHours } from "@sln/lib/date-ranges";
import { buildDateRanges, subtract } from "@sln/lib/date-ranges";
import { ErrorCode } from "@sln/lib/errorCodes";
import { HttpError } from "@sln/lib/http-error";
import {
  descendingLimitKeys,
  intervalLimitKeyToUnit,
} from "@sln/lib/intervalLimit";
import logger from "@sln/lib/logger";
import { safeStringify } from "@sln/lib/safeStringify";
import { checkBookingLimit } from "@sln/lib/server";
import { performance } from "@sln/lib/server/perfObserver";
import { getTotalBookingDuration } from "@sln/lib/server/queries";
...
import type { TimeRange } from "@sln/types/schedule";
import { z } from "zod";
...
const _getUserAvailability =
  async function getUsersWorkingHoursLifeTheUniverseAndEverythingElse(
    ...
    if (!user)
      throw new HttpError({
        statusCode: 404,
        message: "No user found in getUserAvailability",
      });
    log.debug(
    ...
    const useHostSchedulesForTeamEvent =
      eventType?.metadata?.config?.useHostSchedulesForTeamEvent;
    const schedule =
      !useHostSchedulesForTeamEvent && eventType?.schedule
        ? eventType.schedule
        : userSchedule;
    ...
    if (
      ...
      throw new HttpError({
        statusCode: 400,
        message: ErrorCode.AvailabilityNotFoundInSchedule,
      });
    }
