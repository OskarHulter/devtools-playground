...
import { useGetBookings } from "@sln/atoms";
import dayjs from "@sln/dayjs";
import { Inter } from "next/font/google";
// eslint-disable-next-line @sln/eslint/deprecated-imports-next-router
...
export default function Bookings(props: {
  ...
  const { isLoading: isLoadingUpcomingBookings, data: upcomingBookings } =
    useGetBookings({
      limit: 50,
      cursor: 0,
  ...
  const { isLoading: isLoadingPastBookings, data: pastBookings } =
    useGetBookings({
      limit: 50,
      cursor: 0,
      filters: { status: "past" },
  ...
  return (
    ...
    >
      ...
      <h1 className="my-4 text-2xl font-semibold">
        {props.calUsername} Bookings
      </h1>
      {isLoading && <p>Loading...</p>}
      {!isLoading &&
        pastBookings?.bookings &&
        upcomingBookings?.bookings &&
        (Boolean(upcomingBookings?.bookings.length) ||
          Boolean(pastBookings?.bookings.length)) &&
        [...pastBookings?.bookings, ...upcomingBookings?.bookings].map(
          (booking) => {
            const date = dayjs(booking.startTime).toDate();
            const startTime = dayjs(booking?.startTime).format(
              12 === 12 ? "h:mma" : "HH:mm"
            );
            const endTime = dayjs(booking?.endTime).format(
              12 === 12 ? "h:mma" : "HH:mm"
            );
