...
} from "@sln/features/schedules/components/Schedule";
import WebShell from "@sln/features/shell/Shell";
import { availabilityAsString } from "@sln/lib/availability";
...
import type { TimeRange, WorkingHours } from "@sln/types/schedule";
import {
  Button,
...
const useExcludedDates = () => {
  const watchValues = useWatch<AvailabilityFormValues>({
    name: "dateOverrides",
  }) as {
    ranges: TimeRange[];
  }[];
  return useMemo(() => {
    return watchValues?.map((field) =>
    ...
    >
      <div className="mt-4 w-full md:mt-0">
        <Form
          ...
          className={cn(
            customClassNames?.formClassName,
            "flex flex-col sm:mx-0 xl:flex-row xl:space-x-6"
          )}
        >
