import type { ScheduleLabelsType } from "@sln/features/schedules/components/Schedule";
import type {
  ApiErrorResponse,
  ApiResponse,
...
export const AvailabilitySettingsPlatformWrapper = ({
  ...
  const { data: me } = useMe();
  const atomSchedule = transformApiScheduleForAtom(
    me?.data,
    schedule,
    schedules?.length || 0
  );
  ...
  const { mutate: deleteSchedule, isPending: isDeletionInProgress } =
    useDeleteSchedule({
      onSuccess: (res) => {
        onDeleteSuccess?.(res);
        toast({
  ...
  const { mutate: updateSchedule, isPending: isSavingInProgress } =
    useUpdateSchedule({
      onSuccess: (res) => {
        onUpdateSuccess?.(res);
        toast({
  ...
  if (!atomSchedule)
    return <div className="px-10 py-4 text-xl">No user schedule present</div>;
