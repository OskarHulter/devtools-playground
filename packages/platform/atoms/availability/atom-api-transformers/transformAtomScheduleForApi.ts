...
export function transformAtomScheduleForApi(
    ...
    dateOverrides.flatMap(
      (dateOverridesRanges) =>
        dateOverridesRanges?.ranges?.map((range) =>
          transfromAtomOverrideForApi(range)
        ) ?? []
    ) ?? [];
...
function transfromAtomOverrideForApi(override: AtomDateOverride) {
  const date = `${override.start.getUTCFullYear()}-${(
    override.start.getUTCMonth() + 1
  )
    .toString()
    .padStart(2, "0")}-${override.start
    .getUTCDate()
    .toString()
    .padStart(2, "0")}`;
  ...
  return {
    date,
    startTime: padHoursMinutesWithZeros(
      `${override.start.getUTCHours()}:${override.start.getUTCMinutes()}`
    ),
    endTime: padHoursMinutesWithZeros(
      `${override.end.getUTCHours()}:${override.end.getUTCMinutes()}`
    ),
  };
}
