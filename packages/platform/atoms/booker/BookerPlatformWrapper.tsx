...
import { useOverlayCalendarStore } from "@sln/features/bookings/Booker/components/OverlayCalendar/store";
...
import { useLocalSet } from "@sln/features/bookings/Booker/components/hooks/useLocalSet";
import {
  useBookerStore,
  useInitializeBookerStore,
} from "@sln/features/bookings/Booker/store";
import { useTimePreferences } from "@sln/features/bookings/lib";
import { useTimesForSchedule } from "@sln/features/schedules/lib/use-schedule/useTimesForSchedule";
import { getUsernameList } from "@sln/lib/defaultEvents";
...
> & {
  ...
  onCreateRecurringBookingSuccess?: (
    data: ApiSuccessResponse<BookingResponse[]>
  ) => void;
  onCreateRecurringBookingError?: (data: ApiErrorResponse | Error) => void;
  onCreateInstantBookingSuccess?: (
    data: ApiSuccessResponse<BookingResponse>
  ) => void;
...
export const BookerPlatformWrapper = (
  props: BookerPlatformWrapperAtomProps
) => {
  ...
  const setSelectedDuration = useBookerStore(
  ...
  const event = useMemo(() => {
    return {
      ...
      data:
        data && data.length > 0
          ? transformApiEventTypeForAtom(data[0], props.entity)
          : undefined,
    };
