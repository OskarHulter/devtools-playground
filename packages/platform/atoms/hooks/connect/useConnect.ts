...
import type { CALENDARS } from "@sln/platform-constants";
import { SUCCESS_STATUS, ERROR_STATUS } from "@sln/platform-constants";
import type { ApiResponse, ApiErrorResponse } from "@sln/platform-types";

import http from "../../lib/http";

export const getQueryKey = (calendar: (typeof CALENDARS)[number]) => [
  `get-${calendar}-redirect-uri`,
];
    ...
    queryFn: () => {
      return http
        ...
        .then(({ data: responseBody }) => {
          ...
          if (responseBody.status === ERROR_STATUS)
...
export const useSaveCalendarCredentials = (
  ...
  >({
    mutationFn: (data) => {
      ...
      return http
        .post(`/calendars/${calendar}/credentials`, body)
        .then((res) => {
          return res.data;
        });
    },
