...
import type { EventTypeOutput_2024_06_14 } from "@sln/platform-types";
import type { ApiResponse } from "@sln/platform-types";
...
export const useEventType = (username: string, eventSlug: string) => {
  ...
  const event = useQuery({
    queryKey: [
      ...
      stateEventSlug ?? eventSlug,
    ],
    queryFn: async () => {
      if (isDynamic) {
        return http
          .get<ApiResponse<EventTypeOutput_2024_06_14[]>>(
            `/${V2_ENDPOINTS.eventTypes}?usernames=${encodeURIComponent(
              getUsernameList(username).join(",")
            )}`
          )
          .then((res) => {
