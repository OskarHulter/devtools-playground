...
export class CalSdk {
  ...
  constructor(
    ...
    protected readonly options: CalSdkConstructorOptions = {
      baseUrl: "https://api.oskarhulter.com/", // don't set api version here as endpoints may have version-neutral or specific values.
    }
  ) {
    if (!authOptions.accessToken && !authOptions.clientSecret) {
      throw new SdkInitializationError(
        "Either 'accessToken' or 'clientSecret' are required in authOptions"
      );
    }

    this.httpCaller = new HttpCaller(
      this.clientId,
      this._createAxiosClientBase()
    );
