...
describe("Permissions Function: listPermission", () => {
  ...
  it("it should return the list of permissions a user has", () => {
    ...
    const shouldNotContainPermissions = [
      SCHEDULE_WRITE,
      EVENT_TYPE_WRITE,
      BOOKING_READ,
      BOOKING_WRITE,
    ];
    shouldContainPermissions.forEach((permission) =>
      expect(result).toContain(permission)
    );
    shouldNotContainPermissions.forEach((permission) =>
  ...
  it("it should return the list of permissions a user has", () => {
    ...
    const result = listPermissions(userPermissions);
    const shouldContainPermissions = [
      EVENT_TYPE_READ,
      SCHEDULE_READ,
      BOOKING_WRITE,
      BOOKING_READ,
