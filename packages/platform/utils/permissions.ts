import {
  ...
  PROFILE_READ,
  PROFILE_WRITE,
} from "@sln/platform-constants";
import type { PLATFORM_PERMISSION } from "@sln/platform-types";
...
export const hasEventTypeReadPermission = (
  userPermissions: number
): boolean => {
  return hasPermission(userPermissions, EVENT_TYPE_READ);
};

export const hasEventTypeWritePermission = (
  userPermissions: number
): boolean => {
...
export const hasScheduleWritePermission = (
  userPermissions: number
): boolean => {
