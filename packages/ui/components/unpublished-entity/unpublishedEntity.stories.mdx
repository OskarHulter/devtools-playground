...
import { Title, CustomArgsTable, VariantsTable, VariantRow } from "@sln/storybook/components";
...
- An `EmptyScreen` component from `@sln/ui` that displays a visual representation of an "empty screen."
- An `Avatar` component from `@sln/ui` that shows an avatar image associated with the organization or team.
