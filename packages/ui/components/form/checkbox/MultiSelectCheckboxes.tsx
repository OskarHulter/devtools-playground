...
export default function MultiSelectCheckboxes({
  ...
  const allSelected =
    selected.length === options.length ? allOptions : selected;
  ...
  return (
    <Select
      ...
      onChange={(
        s: MultiValueType<Option> | SingleValue<Option>,
        event: any
      ) => {
        const allSelected = [];
        ...
        if (s !== null && Array.isArray(s) && s.length > 0) {
          if (s.find((option) => option.value === "all")) {
            ...
            if (s.length === options.length) {
              if (s.find((option) => option.value === "all")) {
                allSelected.push(
                  ...s.filter((option) => option.value !== "all")
                );
              } else {
                if (event.action === "select-option") {
                  allSelected.push(
                    ...[...s, { label: "Select all", value: "all" }]
                  );
                }
              }
            } else {
