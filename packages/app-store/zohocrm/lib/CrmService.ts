...
import type { CalendarEvent, NewCalendarEventType } from "@sln/types/Calendar";
import type { CredentialPayload } from "@sln/types/Credential";
import type { CRM, Contact, ContactCreateInput } from "@sln/types/CrmService";
...
export default class ZohoCrmCrmService implements CRM {
  ...
  async getContacts(emails: string | string[]) {
    ...
    const searchCriteria = `(${emailsArray
      .map((email) => `(Email:equals:${encodeURI(email)})`)
      .join("or")})`;
  ...
  private zohoCrmAuth = async (credential: CredentialPayload) => {
    ...
    if (!this.client_id)
      throw new HttpError({
        statusCode: 400,
        message: "Zoho CRM client_id missing.",
      });
    if (!this.client_secret)
      throw new HttpError({
        statusCode: 400,
        message: "Zoho CRM client_secret missing.",
      });
  ...
  async handleEventCreation(event: CalendarEvent, contacts: Contact[]) {
    ...
    if (
      meetingEvent.data &&
      meetingEvent.data.length &&
      meetingEvent.data[0].status === "success"
    ) {
    ...
    return Promise.reject(
      "Something went wrong when creating a meeting in ZohoCRM"
    );
  }
