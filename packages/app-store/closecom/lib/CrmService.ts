import type { CloseComFieldOptions } from "@sln/lib/CloseCom";
import CloseCom from "@sln/lib/CloseCom";
import { getCustomActivityTypeInstanceData } from "@sln/lib/CloseComeUtils";
...
export default class CloseComCRMService implements CRM {
  ...
  constructor(credential: CredentialPayload) {
    ...
    if (parsedCredentialKey.success) {
      decrypted = symmetricDecrypt(
        parsedCredentialKey.data.encrypted,
  ...
  closeComUpdateCustomActivity = async (uid: string, event: CalendarEvent) => {
    const customActivityTypeInstanceData =
      await getCustomActivityTypeInstanceData(
        event,
        calComCustomActivityFields,
        this.closeCom
      );
    // Create Custom Activity type instance
    const customActivityTypeInstance =
      await this.closeCom.activity.custom.create(
        customActivityTypeInstanceData
      );
    return this.closeCom.activity.custom.update(
      uid,
      customActivityTypeInstance
    );
  };
