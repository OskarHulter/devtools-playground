...
import type { Calendar, CalendarEvent, EventBusyDate, IntegrationCalendar, NewCalendarEventType } from "@sln/types/Calendar";
...
export default class BasecampCalendarService implements Calendar {
  ...
  private async getBasecampDescription(event: CalendarEvent): Promise<string> {
    const timeZone = await this.getUserTimezoneFromDB(
      event.organizer?.id as number
    );
    ...
    const baseString = `<div>Event title: ${event.title}<br/>Date and time: ${date}, ${startTime} - ${endTime} ${timeZone}<br/>View on Cal.com: <a target="_blank" rel="noreferrer" class="autolinked" data-behavior="truncate" href="https://app.oskarhulter.com/booking/${event.uid}">https://app.oskarhulter.com/booking/${event.uid}</a> `;
    const guestString = `<br/>Guests: ${event.attendees.reduce(
      (acc, attendee) => {
