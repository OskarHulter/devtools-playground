...
export const projectMutationHandler = async ({
  ctx,
  input,
}: ProjectMutationHandlerOptions) => {
  ...
  const scheduleId = scheduleJson.dock.find(
    (dock: IDock) => dock.name === "schedule"
  ).id;
  await prisma.credential.update({
    where: { id: credential.id },
    data: {
      key: { ...credentialKey, projectId: Number(projectId), scheduleId },
    },
  });
