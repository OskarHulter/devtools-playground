import type { IntegrationOAuthCallbackState } from "@sln/app-store/types";
  ...
  >({
    ...
    mutationFn: async (variables) => {
      ...
      const defaultInstall =
        variables && variables.defaultInstall
          ? variables.defaultInstall
          : undefined;
      ...
      if (externalUrl) {
        ...
        return {
          setupPending:
            json?.url?.endsWith("/setup") ||
            json?.url?.includes("/apps/installation/event-types"),
        };
      } else if (returnTo) {
