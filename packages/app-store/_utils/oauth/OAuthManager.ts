...
import type {
  OAuth2TokenResponseInDbWhenExistsSchema,
...
import { CREDENTIAL_SYNC_ENDPOINT } from "@sln/lib/constants";
import logger from "@sln/lib/logger";
import { safeStringify } from "@sln/lib/safeStringify";
import type { z } from "zod";

const log = logger.getSubLogger({
  prefix: ["app-store/_utils/oauth/OAuthManager"],
});
...
type isTokenObjectUnusable = (
  response: Response
) => Promise<{ reason: string } | null>;
type isAccessTokenUnusable = (
  response: Response
) => Promise<{ reason: string } | null>;
type IsTokenExpired = (
  token: z.infer<typeof OAuth2UniversalSchema>
...
export class OAuthManager {
  ...
  private async getAndValidateOAuth2Response({
    ...
    const myLog = log.getSubLogger({
      prefix: ["getAndValidateOAuth2Response"],
    });
    const clonedResponse = response.clone();
...
function ensureValidResourceOwner(
  resourceOwner:
    | { id: number | null; type: "team" }
    | { id: number | null; type: "user" }
) {
...
function handleFetchError(e: unknown) {
  ...
  if (e instanceof Error) {
    return new Response(JSON.stringify({ myFetchError: e.message }), {
      status: 500,
    });
  }
  return new Response(JSON.stringify({ myFetchError: "UNKNOWN_ERROR" }), {
    status: 500,
  });
}
