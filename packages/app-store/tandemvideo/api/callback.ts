...
export default async function handler(
  ...
  if (!client_id)
    return res.status(400).json({ message: "Tandem client_id missing." });
  if (!client_secret)
    return res.status(400).json({ message: "Tandem client_secret missing." });
  if (!base_url)
    return res.status(400).json({ message: "Tandem base_url missing." });
  ...
  const credentialIdsToDelete = existingCredentialTandemVideo.map(
    (item) => item.id
  );
  ...
  if (result.ok) {
    ...
    await createOAuthAppCredential(
      { appId: "tandem", type: "tandem_video" },
      responseBody,
      req
    );
    ...
    res.redirect(
      getSafeRedirectUrl(state?.returnTo) ??
        getInstalledAppPath({ variant: "conferencing", slug: "tandem" })
    );
  }
}
