...
import type {
  VideoApiAdapter,
  VideoCallData,
} from "@sln/types/VideoApiAdapter";
...
const tandemAuth = async (credential: CredentialPayload) => {
  ...
  if (!client_id)
    throw new HttpError({
      statusCode: 400,
      message: "Tandem client_id missing.",
    });
  if (!client_secret)
    throw new HttpError({
      statusCode: 400,
      message: "Tandem client_secret missing.",
    });
  if (!base_url)
    throw new HttpError({
      statusCode: 400,
      message: "Tandem base_url missing.",
    });

  const credentialKey = credential.key as unknown as TandemToken;
  const isTokenValid = (token: TandemToken) =>
    token && token.access_token && token.expiry_date < Date.now();
...
const TandemVideoApiAdapter = (
  ...
  return {
    ...
    createMeeting: async (event: CalendarEvent): Promise<VideoCallData> => {
      ...
      const responseBody = await handleErrorsJson<ITandemCreateMeetingResponse>(
        result
      );
    ...
    updateMeeting: async (
      ...
      const result = await fetch(
        `${base_url}/api/v1/meetings/${bookingRef.meetingId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${accessToken}`,
