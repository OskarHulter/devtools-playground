...
import type { ContactCreateInput, CRM, Contact } from "@sln/types/CrmService";
...
export default class PipedriveCrmService implements CRM {
  ...
  constructor(credential: CredentialPayload) {
    ...
    this.tenantId = String(
      credential.teamId ? credential.teamId : credential.userId
    ); // Question: Is this a reasonable assumption to be made? Get confirmation on the exact field to be used here.
    ...
    const result = contactsToCreate.map(async (attendee) => {
      ...
      try {
        const response = await fetch(
          `${this.revertApiUrl}crm/contacts`,
          requestOptions
        );
  ...
  async getContacts(email: string | string[]): Promise<Contact[]> {
    ...
    const result = emailArray.map(async (attendeeEmail) => {
      ...
      try {
        const response = await fetch(
          `${this.revertApiUrl}crm/contacts/search`,
          requestOptions
        );
  ...
  private createPipedriveEvent = async (
    event: CalendarEvent,
    contacts: Contact[]
  ) => {
    const eventPayload = {
  ...
  async handleEventCreation(event: CalendarEvent, contacts: Contact[]) {
    const meetingEvent = await (
      await this.createPipedriveEvent(event, contacts)
    ).json();
    ...
    return Promise.reject(
      "Something went wrong when creating a meeting in PipedriveCRM"
    );
  }
  ...
  async updateEvent(
    ...
    return Promise.reject(
      "Something went wrong when updating a meeting in PipedriveCRM"
    );
  }
