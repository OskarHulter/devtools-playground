...
async function getHandler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const response = await fetch(
      `https://${FEISHU_HOST}/open-apis/authen/v1/access_token`,
      {
        method: "POST",
        headers: {
    ...
    if (primaryCalendarResponse.status === 200) {
      ...
      if (
        primaryCalendar.data.calendars.calendar.calendar_id &&
        req.session?.user?.id
      ) {
        await prisma.selectedCalendar.create({
          data: {
            ...
            externalId: primaryCalendar.data.calendars.calendar
