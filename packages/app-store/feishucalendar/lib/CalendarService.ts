...
export default class FeishuCalendarService implements Calendar {
  ...
  private refreshAccessToken = async (credential: CredentialPayload) => {
    ...
    try {
      ...
      const newFeishuAuthCredentials: FeishuAuthCredentials = {
        ...
        refresh_expires_date: Math.round(
          +new Date() / 1000 + data.data.refresh_expires_in
        ),
  ...
  async createEvent(
    ...
    try {
      const eventResponse = await this.fetcher(
        `/calendar/v4/calendars/${calendarId}/events/create_event`,
        {
          method: "POST",
          body: JSON.stringify(this.translateEvent(event)),
        }
      );
      eventRespData = await handleFeishuError<CreateEventResp>(
        eventResponse,
        this.log
      );
  ...
  async deleteEvent(
    ...
    try {
      const response = await this.fetcher(
        `/calendar/v4/calendars/${calendarId}/events/${uid}`,
        {
          method: "DELETE",
        }
      );
  ...
  listCalendars = async (): Promise<IntegrationCalendar[]> => {
    try {
      ...
      const dataPrimary = await handleFeishuError<GetPrimaryCalendarsResp>(
        respPrimary,
        this.log
      );
      ...
      this.log.error(
        "There was an error contacting feishu calendar service: ",
        err
      );
      throw err;
    }
