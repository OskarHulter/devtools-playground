...
import type { Person } from "@sln/types/Calendar";
...
const Reschedule = async (bookingUid: string, cancellationReason: string) => {
  ...
  if (
    bookingToReschedule &&
    bookingToReschedule.eventTypeId &&
    bookingToReschedule.user
  ) {
    ...
    const userOwnerTranslation = await getTranslation(
      userOwner.locale ?? "en",
      "common"
    );
    const [userOwnerAsPeopleType] = usersToPeopleType(
      [userOwner],
      userOwnerTranslation
    );
    ...
    const bookingRefsFiltered: BookingReference[] =
      bookingToReschedule.references.filter(
    ...
    const promises = bookingRefsFiltered.map(async (bookingRef) => {
      ...
      if (bookingRef.type.endsWith("_calendar")) {
        ...
        return deleteMeeting(
          credentialsMap.get(bookingRef.type),
          bookingRef.uid
        );
      }
    });
