...
import { appStoreMetadata } from "@sln/app-store/bookerAppsMetaData";
import logger from "@sln/lib/logger";
import { BookingStatus } from "@sln/prisma/enums";
import type { Ensure, Optional } from "@sln/types/utils";
...
for (const [appName, meta] of Object.entries(appStoreMetadata)) {
  ...
  if (location) {
    ...
    const newLocation = {
      ...location,
      messageForOrganizer:
        location.messageForOrganizer || `Set ${location.label} link`,
      iconUrl: meta.logo,
...
export const getStaticLinkBasedLocation = (locationType: string) =>
  locationsFromApps.find(
    (l) => l.linkType === "static" && l.type === locationType
  );
...
export const getEventLocationTypeFromValue = (
...
export const guessEventLocationType = (
  locationTypeOrValue: string | undefined | null
) =>
  getEventLocationType(locationTypeOrValue) ||
  getEventLocationTypeFromValue(locationTypeOrValue);
...
export const privacyFilteredLocations = (
  locations: LocationObject[]
): PrivacyFilteredLocationObject[] => {
  const locationsAfterPrivacyFilter = locations.map((location) => {
    const eventLocationType = getEventLocationType(location.type);
    if (!eventLocationType) {
      logger.debug(
        `Couldn't find location type. App might be uninstalled: ${location.type} `
      );
    }
...
export const getMessageForOrganizer = (location: string, t: TFunction) => {
  ...
  if (
    videoLocation &&
    videoLocation.linkType !== "static" &&
    videoLocation.type !== "integrations:zoom"
  ) {
...
export const getHumanReadableLocationValue = (
  ...
  if (eventLocationType) {
    ...
    return isDefault
      ? translationFunction(eventLocationType.label)
...
export const getEventLocationValue = (
  eventLocations: LocationObject[],
  bookingLocation: LocationObject
) => {
  ...
  if (!defaultValueVariable) {
    console.error(
      `${defaultValueVariable} not set for ${bookingLocation.type}`
    );
    return "";
  }
  const eventLocation = getEventLocationWithType(
    eventLocations,
  ...
  return (
    bookingLocation[defaultValueVariable] ||
    eventLocation[defaultValueVariable] ||
...
export const getOrganizerInputLocationTypes = () => {
  const result:
    | DefaultEventLocationType["type"]
    | EventLocationTypeFromApp["type"][] = [];

  const locations = locationsTypes.filter(
    (location) => !!location.organizerInputType
  );
