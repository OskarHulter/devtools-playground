...
import type { CRM, Contact, CrmEvent } from "@sln/types/CrmService";
...
export default class SalesforceCRMService implements CRM {
  ...
  private getClient = async (credential: CredentialPayload) => {
    ...
    if (typeof appKeys.consumer_key === "string")
      consumer_key = appKeys.consumer_key;
    if (typeof appKeys.consumer_secret === "string")
      consumer_secret = appKeys.consumer_secret;
    if (!consumer_key)
      throw new HttpError({
        statusCode: 400,
        message: "Salesforce consumer key is missing.",
      });
    if (!consumer_secret)
      throw new HttpError({
        statusCode: 400,
        message: "Salesforce consumer secret missing.",
      });
  ...
  async getContacts(email: string | string[], includeOwner?: boolean) {
    ...
    if (includeOwner) {
      ...
      const contactsWithOwners = records.map((record) => {
        const ownerEmail = ownersQuery.find(
          (user) => user.records[0].Id === record.OwnerId
        )?.records[0].Email;
        return {
          id: record.Id,
          email: record.Email,
  ...
  async createContacts(
    contactsToCreate: { email: string; name: string }[],
    ...
    if (organizerEmail) {
      ...
      if (userQuery) {
        organizerId = (userQuery.records[0] as { Email: string; Id: string })
          .Id;
      }
    }
    ...
    return createdContacts.filter(
      (contact): contact is Contact => contact !== undefined
    );
  }
}
