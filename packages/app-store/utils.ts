...
import type { EventLocationType } from "@sln/app-store/locations";
import logger from "@sln/lib/logger";
import { getPiiFreeCredential } from "@sln/lib/piiFreeData";
import { safeStringify } from "@sln/lib/safeStringify";
import type { App, AppMeta } from "@sln/types/App";
...
function getApps(
  ...
  const apps = ALL_APPS.reduce((reducedArray, appMeta) => {
    const appCredentials = credentials.filter(
      (credential) => credential.appId === appMeta.slug
    );

    if (filterOnCredentials && !appCredentials.length && !appMeta.isGlobal)
      return reducedArray;
    ...
    const credential: (typeof appCredentials)[number] | null =
      appCredentials[0] || null;
...
export function isConferencing(appCategories: string[]) {
  return appCategories.some(
    (category) => category === "conferencing" || category === "video"
  );
}
