...
import type { Contact, ContactCreateInput, CRM } from "@sln/types/CrmService";
...
export default class BiginCrmService implements CRM {
  ...
  private async refreshAccessToken(
    ...
    if (!tokenInfo.data.error) {
      ...
      tokenInfo.data.expiryDate = Math.round(
        Date.now() + tokenInfo.data.expires_in
      );
  ...
  async handleEventCreation(event: CalendarEvent, contacts: Contact[]) {
    ...
    return Promise.reject(
      "Something went wrong when creating a meeting in Zoho Bigin"
    );
  }
  ...
  async updateEvent(
    ...
    return axios
      .put(
        token.api_domain + this.eventsSlug,
        JSON.stringify({ data: [biginEvent] }),
        {
          headers: {
            "content-type": "application/json",
            authorization: `Zoho-oauthtoken ${token.access_token}`,
