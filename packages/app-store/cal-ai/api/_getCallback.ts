...
import { apiKeysRouter } from "@sln/trpc/server/routers/viewer/apiKeys/_router";
...
export async function getHandler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!checkoutId) {
    return {
      url: `/apps/installed?error=${JSON.stringify({
        message: "No Stripe Checkout Session ID",
      })}`,
    };
  }

  const { url } = await withStripeCallback(
    checkoutId,
    slug,
