...
import { checkPremiumUsername } from "@sln/features/ee/common/lib/checkPremiumUsername";
...
export default async function handler(
  ...
  if (req.method === "GET") {
    ...
    if (!stripeCustomer || stripeCustomer.deleted) {
      return res
        .status(400)
        .json({ message: "Stripe customer not found or deleted" });
    }
    ...
    return res
      .status(404)
      .json({ message: "Couldn't redirect to stripe checkout session" });
  }
}
