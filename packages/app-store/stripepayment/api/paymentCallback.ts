import { getCustomerAndCheckoutSession } from "@sln/app-store/stripepayment/lib/getCustomerAndCheckoutSession";
...
async function getHandler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { stripeCustomer, checkoutSession } =
    await getCustomerAndCheckoutSession(checkoutSessionId);
  ...
  if (
    ...
    stripeCustomer.metadata.username
  ) {
    try {
      ...
      throw new HttpError({
        ...
        method: req.method,
        message:
          "We have received your payment. Your premium username could still not be reserved. Please contact support@oskarhulter.com and mention your premium username",
      });
    }
  }
