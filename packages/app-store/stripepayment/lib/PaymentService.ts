...
import { sendAwaitingPaymentEmail } from "@sln/emails";
...
import type { IAbstractPaymentService } from "@sln/types/PaymentService";
...
export class PaymentService implements IAbstractPaymentService {
  ...
  async chargeCard(
    ...
    try {
      ...
      // Parse keys with zod
      const { payment_fee_fixed, payment_fee_percentage } =
        stripeAppKeysSchema.parse(stripeAppKeys?.keys);

      const paymentFee = Math.round(
        payment.amount * payment_fee_percentage + payment_fee_fixed
      );
      ...
      if (!customer) {
        throw new Error(
          `Stripe customer does not exist for setupIntent ${setupIntent.id}`
        );
      }

      if (!paymentMethod) {
        throw new Error(
          `Stripe paymentMethod does not exist for setupIntent ${setupIntent.id}`
        );
      }
      ...
      log.error(
        "Stripe: Could not charge card for payment",
        _bookingId,
        safeStringify(error)
      );
  ...
  async refund(paymentId: Payment["id"]): Promise<Payment> {
    try {
      ...
      const refund = await this.stripe.refunds.create(
        ...
        {
          stripeAccount: (payment.data as unknown as StripePaymentData)[
            "stripeAccount"
          ],
        }
      );
  ...
  async deletePayment(paymentId: Payment["id"]): Promise<boolean> {
    try {
      ...
      const stripeAccount = (payment.data as unknown as StripePaymentData)
        .stripeAccount;
      ...
      log.error(
        "Stripe: Unable to delete Payment in stripe of paymentId",
        paymentId,
        safeStringify(e)
      );
      return false;
