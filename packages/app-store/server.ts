...
import { defaultVideoAppCategories } from "@sln/app-store/utils";
...
export async function getLocationGroupedOptions(
  ...
  const integrations = await getEnabledAppsFromCredentials(credentials, {
    ...
    if (app.locationOption) {
      ...
      let groupByCategory =
        app.categories.length >= 2
          ? app.categories.find(
              (groupByCategory) =>
                !defaultVideoAppCategories.includes(groupByCategory)
            )
        ...
        const option = {
          ...
          ...(app.credential
            ? {
                ...
                teamName: app.credential.team?.name ?? null,
              }
            : {}),
        };
