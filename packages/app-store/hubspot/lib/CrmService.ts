...
import { getLocation } from "@sln/lib/CalEventParser";
...
export default class HubspotCalendarService implements CRM {
  ...
  private auth: Promise<{
    getToken: () => Promise<HubspotToken | void | never[]>;
  }>;
  ...
  private getHubspotMeetingBody = (event: CalendarEvent): string => {
    ...
    }<br><br><b>${event.organizer.language.translate(
  ...
  private hubspotCreateMeeting = async (event: CalendarEvent) => {
    ...
    return hubspotClient.crm.objects.meetings.basicApi.create(
      simplePublicObjectInput
    );
  };

  private hubspotAssociate = async (
    meeting: SimplePublicObject,
  ...
  private hubspotUpdateMeeting = async (uid: string, event: CalendarEvent) => {
    ...
    return hubspotClient.crm.objects.meetings.basicApi.update(
      uid,
      simplePublicObjectInput
    );
  };
  ...
  private hubspotAuth = async (credential: CredentialPayload) => {
    ...
    if (typeof appKeys.client_id === "string")
      this.client_id = appKeys.client_id;
    if (typeof appKeys.client_secret === "string")
      this.client_secret = appKeys.client_secret;
    if (!this.client_id)
      throw new HttpError({
        statusCode: 400,
        message: "Hubspot client_id missing.",
      });
    if (!this.client_secret)
      throw new HttpError({
        statusCode: 400,
        message: "Hubspot client_secret missing.",
      });
    ...
    const refreshAccessToken = async (refreshToken: string) => {
      try {
        ...
        hubspotRefreshToken.expiryDate = Math.round(
          Date.now() + hubspotRefreshToken.expiresIn * 1000
        );
  ...
  async handleMeetingCreation(event: CalendarEvent, contacts: Contact[]) {
    ...
    if (meetingEvent) {
      ...
      return Promise.reject(
        "Something went wrong when associating the meeting and attendees in HubSpot"
      );
    }
    this.log.debug("meeting:creation:notOk", { meetingEvent, event, contacts });
    return Promise.reject(
      "Something went wrong when creating a meeting in HubSpot"
    );
  }
    ...
    const createdContacts = await Promise.all(
      simplePublicObjectInputs.map((contact) =>
        hubspotClient.crm.contacts.basicApi.create(contact).catch((error) => {
          ...
          if (
            error.body.message.includes("Contact already exists. Existing ID:")
          ) {
            const split = error.body.message.split(
