...
import { WEBAPP_URL_FOR_OAUTH } from "@sln/lib/constants";
...
export default async function handler(
  ...
  if (!client_id)
    return res.status(400).json({ message: "HubSpot client id missing." });
  if (!client_secret)
    return res.status(400).json({ message: "HubSpot client secret missing." });

  const hubspotToken: HubspotToken =
  ...
  hubspotToken.expiryDate = Math.round(
    Date.now() + hubspotToken.expiresIn * 1000
  );
  ...
  res.redirect(
    getSafeRedirectUrl(state?.returnTo) ??
      getInstalledAppPath({ variant: "other", slug: "hubspot" })
  );
}
