...
import { renewSelectedCalendarCredentialId } from "@sln/lib/connectedCalendar";
import { WEBAPP_URL, WEBAPP_URL_FOR_OAUTH } from "@sln/lib/constants";
import { handleErrorsJson } from "@sln/lib/errors";
...
export default async function handler(
  ...
  if (!client_id)
    return res.status(400).json({ message: "Office 365 client_id missing." });
  if (!client_secret)
    return res
      .status(400)
      .json({ message: "Office 365 client_secret missing." });
  ...
  if (!response.ok) {
    return res.redirect(
      `/apps/installed?error=${JSON.stringify(responseBody)}`
    );
  }
  ...
  while (!finishedParsingCalendars) {
    ...
    let calBody = await handleErrorsJson<{
    ...
    const findDefaultCalendar = calBody.value.find(
