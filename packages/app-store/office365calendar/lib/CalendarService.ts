...
import { getLocation, getRichDescription } from "@sln/lib/CalEventParser";
import { handleErrorsJson, handleErrorsRaw } from "@sln/lib/errors";
import logger from "@sln/lib/logger";
import type { BufferedBusyTime } from "@sln/types/BufferedBusyTime";
import type {
  Calendar,
  CalendarEvent,
  EventBusyDate,
...
export default class Office365CalendarService implements Calendar {
  ...
  constructor(credential: CredentialPayload) {
    ...
    this.auth = new OAuthManager({
      ...
      invalidateTokenObject: () =>
        oAuthManagerHelper.invalidateCredential(credential.id),
      expireAccessToken: () =>
        oAuthManagerHelper.markTokenAsExpired(credential),
      updateTokenObject: (tokenObject) =>
        oAuthManagerHelper.updateTokenObject({
          tokenObject,
  ...
  async createEvent(
    ...
    try {
      ...
      const responseJson = await handleErrorsJson<
        NewCalendarEventType & { iCalUId: string }
      >(response);
  ...
  async getAvailability(
    ...
    try {
      ...
      const ids = await (selectedCalendarIds.length === 0
        ? this.listCalendars().then(
            (cals) => cals.map((e_2) => e_2.externalId).filter(Boolean) || []
          )
      ...
      const responseBody = await this.handleErrorJsonOffice365Calendar(
        response
      );
      let responseBatchApi: IBatchResponse = { responses: [] };
      if (typeof responseBody === "string") {
        responseBatchApi =
          this.handleTextJsonResponseWithHtmlInBody(responseBody);
      }
      ...
      const retryAfter =
        ...
        this.findRetryAfterResponse(responseBatchApi.responses);
      ...
      alreadySuccessResponse = await this.fetchResponsesWithNextLink(
        responseBatchApi.responses
      );

      return alreadySuccessResponse
        ? this.processBusyTimes(alreadySuccessResponse)
        : [];
    } catch (err) {
      console.log(err);
        ...
        // Add the calEvent organizer
        {
          emailAddress: {
            address: event.destinationCalendar
              ? event.destinationCalendar.find(
                  (cal) => cal.userId === event.organizer.id
                )?.externalId ?? event.organizer.email
  ...
  private fetchResponsesWithNextLink = async (
    ...
    settledResponses?.forEach((response) => {
      if (
        response.status === 200 &&
        response.body["@odata.nextLink"] === undefined
      ) {
        alreadySuccess.push(response);
      } else {
        const nextLinkUrl = response.body["@odata.nextLink"]
          ? String(response.body["@odata.nextLink"]).replace(
              this.apiGraphUrl,
              ""
            )
          : "";
    ...
    let newResponseBody = await handleErrorsJson<IBatchResponse | string>(
      newResponse
    );

    if (typeof newResponseBody === "string") {
      newResponseBody =
        this.handleTextJsonResponseWithHtmlInBody(newResponseBody);
    }

    // Going recursive to fetch next link
    const newSettledResponses = await this.fetchResponsesWithNextLink(
  ...
  private fetchRequestWithRetryAfter = async (
    ...
    settledPromises.forEach((item) => {
      if (item.status === 200) {
        ...
        retryAfterTimeout =
          newTimeout > retryAfterTimeout ? newTimeout : retryAfterTimeout;
    ...
    const retryAfter =
      !!newResponseBody?.responses &&
      this.findRetryAfterResponse(newResponseBody.responses);
    ...
    return {
      responses: [
        ...alreadySuccessRequest,
        ...(newResponseBody?.responses || []),
      ],
    };
  };

  private apiGraphBatchCall = async (
    requests: IRequest[]
  ): Promise<Response> => {
  ...
  private handleTextJsonResponseWithHtmlInBody = (
    ...
    try {
      ...
      const resultString = `${response
        .repeat(1)
        .substring(0, htmlBeginning)} ""${response
        .repeat(1)
  ...
  private findRetryAfterResponse = (response: ISettledResponse[]) => {
    const foundRetry = response.find(
      (request: ISettledResponse) => request.status === 429
    );
    return !!foundRetry;
  };
  ...
  private processBusyTimes = (responses: ISettledResponse[]) => {
    return responses.reduce(
      (
        acc: BufferedBusyTime[],
        subResponse: { body: { value?: BodyValue[]; error?: Error[] } }
      ) => {
        if (!subResponse.body?.value) return acc;
        return acc.concat(
          subResponse.body.value.reduce(
            (acc: BufferedBusyTime[], evt: BodyValue) => {
  ...
  private handleErrorJsonOffice365Calendar = <Type>(
    response: Response
  ): Promise<Type | string> => {
