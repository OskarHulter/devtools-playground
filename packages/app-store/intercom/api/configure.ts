import type { NewCanvas, ListComponent, ListItem, SpacerComponent, TextComponent, InputComponent } from "../lib";
...
export default async function handler(
  ...
  if (component_id || input_values?.submit_booking_url) {
    ...
    if (isValid === true)
      return res.status(200).json({ results: { submit_booking_url: url } });
  }
  ...
  const input: InputComponent = {
    ...
    placeholder: "https://oskarhulter.com/valentinchmara/30min",
    save_state: "unsaved",
    action: {
      type: "submit",
    },
  ...
  const canvasData: NewCanvas = {
    canvas: {
      content: {
        components:
          isValid === true
            ? [components, spacer, text, input]
            : [components, spacer, text, input, isValid],
      },
    },
  };
