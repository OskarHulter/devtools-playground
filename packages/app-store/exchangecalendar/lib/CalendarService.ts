...
export default class ExchangeCalendarService implements Calendar {
  ...
  async updateEvent(
    ...
    const appointment: Appointment = await Appointment.Bind(
  ...
  async getAvailability(
    ...
    const promises: Promise<EventBusyDate[]>[] = calendars
      .filter((lcal) =>
        selectedCalendars.some((rcal) => lcal.externalId == rcal.externalId)
      )
      .map(async (calendar) => {
  ...
  async listCalendars(): Promise<IntegrationCalendar[]> {
    ...
    const deletedItemsFolder: Folder = await Folder.Bind(
      service,
      WellKnownFolderName.DeletedItems
    );
    const searchFilterCollection = new SearchFilter.SearchFilterCollection(
      LogicalOperator.And
    );
    ...
    return service
      ...
      .then((res: FindFoldersResults) => {
        return res.Folders.filter((folder: Folder) => {
          return (
            folder.ParentFolderId.UniqueId != deletedItemsFolder.Id.UniqueId
          );
        }).map((folder: Folder) => {
          return {
