...
import type { UpdateAppCredentialsOptions } from "@sln/trpc/server/routers/viewer/apps/updateAppCredentials.handler";
import { z } from "zod";
...
const handlePaypalValidations = async ({
  input,
  ...
  if (webhooksToDelete) {
    const promises = webhooksToDelete.map((webhook) =>
      paypalClient.deleteWebhook(webhook)
    );
