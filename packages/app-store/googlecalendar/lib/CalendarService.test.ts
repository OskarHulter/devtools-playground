...
const testSelectedCalendar = {
  ...
  externalId: "example@oskarhulter.com",
};
...
test("Calendar Cache is being read and updated", async () => {
  ...
  // Test cache hit
  const data = await calendarService.getAvailability(dateFrom1, dateTo1, [
  ...
  // Test Cache Miss
  await calendarService2.getAvailability(dateFrom2, dateTo2, [
    testSelectedCalendar,
  ]);
