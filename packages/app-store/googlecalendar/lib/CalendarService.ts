...
import { MeetLocationType } from "@sln/app-store/locations";
...
import type CalendarService from "@sln/lib/CalendarService";
import {
...
export default class GoogleCalendarService implements Calendar {
  ...
  private async getMyGoogleAuthSingleton() {
    ...
    this.myGoogleAuth = new MyGoogleAuth(
      client_id,
      client_secret,
      redirect_uris[0]
    );
    ...
    const auth = new OAuthManager({
      ...
      fetchNewTokenObject: async ({
        ...
        const response = new Response(
          JSON.stringify(fetchTokens.res?.data ?? null),
          {
    ...
    const attendees: calendar_v3.Schema$EventAttendee[] = [
      {
        ...
        email:
          selectedHostDestinationCalendar?.externalId ?? event.organizer.email,
      },
      ...eventAttendees,
    ];
  ...
  async createEvent(
    ...
    if (
      formattedCalEvent.conferenceData &&
      formattedCalEvent.location === MeetLocationType
    ) {
    ...
    const selectedCalendar =
      formattedCalEvent.destinationCalendar?.find(
    ...
    try {
      ...
      if (formattedCalEvent.existingRecurringEvent) {
        recurringEventId =
          formattedCalEvent.existingRecurringEvent.recurringEventId;
        ...
        if (recurringEventInstances.data.items) {
          ...
          for (let i = 0; i < recurringEventInstances.data.items.length; i++) {
            ...
            const instanceStartTime = dayjs(instance.start?.dateTime)
              .tz(
                instance.start?.timeZone == null
                  ? undefined
                  : instance.start?.timeZone
              )
              .format();
  ...
  async deleteEvent(
    ...
    try {
      ...
      /**
       *  410 is when an event is already deleted on the Google cal before on oskarhulter.com
  ...
  async getCacheOrFetchAvailability(args: {
    ...
    if (!calendarCacheEnabled) {
      ...
      if (cached) {
        freeBusyResult =
          cached.value as unknown as calendar_v3.Schema$FreeBusyResponse;
      } else {
        ({ json: freeBusyResult } = await this.oAuthManagerInstance.request(
          async () =>
            new AxiosLikeResponseToFetchResponse(
  ...
  async getAvailability(
    ...
    async function getCalIds() {
      ...
      return cals.data.items.reduce(
        (c, cal) => (cal.id ? [...c, cal.id] : c),
        [] as string[]
      );
    }
