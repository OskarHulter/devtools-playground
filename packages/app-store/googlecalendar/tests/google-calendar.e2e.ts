...
import {
  createBookingAndFetchGCalEvent,
  deleteBookingAndEvent,
  assertValueExists,
} from "./testUtils";
...
import { selectSecondAvailableTimeSlotNextMonth } from "@sln/web/playwright/lib/testUtils";
...
test.describe("Google Calendar", async () => {
  ...
  test.describe.skip("Test using the primary calendar", async () => {
    ...
    test("On new booking, event should be created on GCal", async ({
      page,
    }) => {
      const { gCalEvent, gCalReference, booking, authedCalendar } =
        await createBookingAndFetchGCalEvent(
          page as Page,
          qaGCalCredential,
          qaUsername
        );
    ...
    test("On reschedule, event should be updated on GCal", async ({ page }) => {
      ...
      const { gCalReference, booking, authedCalendar } =
        await createBookingAndFetchGCalEvent(
          page,
          qaGCalCredential,
          qaUsername
      ...
      const rescheduledBookingUid =
        rescheduledBookingUrl.match(/booking\/([^\/?]+)/);
      ...
      assertValueExists(
        rescheduledGCalEvent.start?.timeZone,
        "rescheduledGCalEvent"
      );
      assertValueExists(
        rescheduledGCalEvent.end?.timeZone,
        "rescheduledGCalEvent"
      );

      // Ensure that the new start and end times are matching
      const rescheduledStartTimeMatches = dayjs(
        rescheduledBooking.startTime
      ).isSame(
        dayjs(rescheduledGCalEvent.start?.dateTime).tz(
          rescheduledGCalEvent.start?.timeZone
        )
      );
      const rescheduledEndTimeMatches = dayjs(
        rescheduledBooking.endTime
      ).isSame(
        dayjs(rescheduledGCalEvent.end?.dateTime).tz(
          rescheduledGCalEvent.end.timeZone
        )
      );
      expect(rescheduledStartTimeMatches && rescheduledEndTimeMatches).toBe(
        true
      );
    ...
    test("When canceling the booking, the GCal event should also be deleted", async ({
      page,
    }) => {
      const { gCalReference, booking, authedCalendar } =
        await createBookingAndFetchGCalEvent(
