...
import { Button, Icon, showToast, Tooltip } from "@sln/ui";
...
export default function ZapierSetup(props: IZapierSetupProps) {
  ...
  const oldApiKey = trpc.viewer.apiKeys.findKeyOfType.useQuery({
    appId: ZAPIER,
  });
  const teamsList = trpc.viewer.teams.listOwnedTeams.useQuery(undefined, {
    refetchOnWindowFocus: false,
  });
  const teams = teamsList.data?.map((team) => ({
    id: team.id,
    name: team.name,
  }));
  ...
  const zapierCredentials: { userCredentialIds: number[] } | undefined =
        ...
        <div className="bg-default m-auto max-w-[43em] overflow-auto rounded pb-10 md:p-10">
          <div className="md:flex md:flex-row">
            <div className="invisible md:visible">
              <img
                className="h-11"
                src="/api/app-store/zapier/icon.svg"
                alt="Zapier Logo"
              />
            </div>
            <div className="ml-2 ltr:mr-2 rtl:ml-2 md:ml-5">
              ...
              <ol className="mb-5 ml-5 mt-5 list-decimal ltr:mr-5 rtl:ml-5">
                ...
                <Trans i18nKey="zapier_setup_instructions">
                  ...
                  <li>
                    Select Cal.com as your Trigger app. Also choose a Trigger
                    event.
                  </li>
                  <li>
              ...
              <Link
                href="/apps/installed/automation?hl=zapier"
                passHref={true}
                legacyBehavior
              >
...
const CopyApiKey = ({ apiKey }: { apiKey: string }) => {
  ...
  return (
    <div>
      ...
      <div className="text-subtle mb-5 mt-2 text-sm">
