...
// Probably don't need
// import { CALENDAR_INTEGRATIONS_TYPES } from "@sln/lib/integrations/calendar/constants/generals";
...
export default class ExchangeCalendarService implements Calendar {
    ...
    try {
      ...
      for (let i = 0; i < event.attendees.length; i++) {
        appointment.RequiredAttendees.Add(
          new Attendee(event.attendees[i].email)
        );
      }
  ...
  async getAvailability(
    ...
    try {
      const externalCalendars: IntegrationCalendar[] =
      ...
      for (let i = 0; i < selectedCalendars.length; i++) {
        ...
        for (let k = 0; k < externalCalendars.length; k++) {
          if (
            selectedCalendars[i].externalId == externalCalendars[k].externalId
          ) {
      ...
      for (let i = 0; i < calendarsToGetAppointmentsFrom.length; i++) {
        const calendarFolderId = new FolderId(
          calendarsToGetAppointmentsFrom[i].externalId
        );
        const localReturn = await this.getExchangeService()
          ...
          .then(function (params) {
            ...
            for (let k = 0; k < params.Items.length; k++) {
              if (
                params.Items[k].LegacyFreeBusyStatus !=
  ...
  private getExchangeService(): ExchangeService {
    ...
    exch1.Credentials = new WebCredentials(
