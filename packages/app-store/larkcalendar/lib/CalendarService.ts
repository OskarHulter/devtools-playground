...
export default class LarkCalendarService implements Calendar {
  ...
  async createEvent(
    ...
    try {
      ...
      eventRespData = await handleLarkError<CreateEventResp>(
        eventResponse,
        this.log
      );
  ...
  listCalendars = async (): Promise<IntegrationCalendar[]> => {
    try {
      ...
      const dataPrimary = await handleLarkError<GetPrimaryCalendarsResp>(
        respPrimary,
        this.log
      );
      ...
      this.log.error(
        "There was an error contacting lark calendar service: ",
        err
      );
      throw err;
    }
