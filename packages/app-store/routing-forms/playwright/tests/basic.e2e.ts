...
import type { Fixtures } from "@sln/web/playwright/lib/fixtures";
import { test } from "@sln/web/playwright/lib/fixtures";
import { gotoRoutingLink } from "@sln/web/playwright/lib/testUtils";
...
test.describe("Routing Forms", () => {
  test.describe("Zero State Routing Forms", () => {
    test("should be able to add a new form and view it", async ({ page }) => {
      ...
      expect(
        await page.locator('[data-testid="routing-forms-list"] > div').count()
      ).toBe(1);
    ...
    test("should be able to edit the form", async ({ page }) => {
      ...
      const createdFields: Record<
        number,
        { label: string; typeIndex: number }
      > = {};

      const { fieldTypesList: types, fields } =
        await addAllTypesOfFieldsAndSaveForm(formId, page, {
          description,
          label,
        });

      expect(await page.inputValue(`[data-testid="description"]`)).toBe(
        description
      );
      expect(await page.locator('[data-testid="field"]').count()).toBe(
        types.length
      );
    ...
    test.describe("F1<-F2 Relationship", () => {
      test("Create relationship by adding F1 as route.Editing F1 should update F2", async ({
        page,
      }) => {
        ...
        const { types } = await addOneFieldAndDescriptionAndSaveForm(
          form2Id,
          page,
          {
        ...
        await expectCurrentFormToHaveFields(
          page,
          { 1: { label: "F1 Field1", typeIndex: 1 } },
          types
        );
    ...
    test("should be able to submit a prefilled form with all types of fields", async ({
      page,
    }) => {
      const formId = await addForm(page);
      await page.click('[href*="/route-builder/"]');
      ...
      await page.fill(
        '[data-testid="form-field-Test Field Long Text"]',
        "manual-fill"
      );

      expect(
        await page.locator(`[data-testid="form-field-firstField"]`).inputValue()
      ).toBe("456");
      expect(
        await page
          .locator(`[data-testid="form-field-Test Field Number"]`)
          .inputValue()
      ).toBe("456");
      ...
      expect(
        await page
          .locator(`[data-testid="form-field-Test Field Phone"]`)
          .inputValue()
      ).toBe("456");
      expect(
        await page
          .locator(`[data-testid="form-field-Test Field Email"]`)
          .inputValue()
      ).toBe("456@example.com");
      ...
      expect(
        url.searchParams.getAll("Test Field Multiple Selection")
      ).toMatchObject(["456", "789"]);
  ...
  test.describe("Seeded Routing Form ", () => {
    ...
    const createUserAndLogin = async function ({
      users,
      page,
    }: {
    ...
    test("Routing Link - Reporting and CSV Download ", async ({
      page,
      users,
    }) => {
      ...
      expect(download.suggestedFilename()).toEqual(
        `${routingForm.name}-${routingForm.id}.csv`
      );
      ...
      expect(new Date(firstResponseCells.at(-1) as string).getDay()).toEqual(
        new Date().getDay()
      );
      ...
      expect(new Date(secondResponseCells.at(-1) as string).getDay()).toEqual(
        new Date().getDay()
      );

      expect(thirdResponseCells.slice(0, -1).join(",")).toEqual("custom-page,");
      expect(new Date(thirdResponseCells.at(-1) as string).getDay()).toEqual(
        new Date().getDay()
      );
    });
    ...
    test("Router URL should work", async ({ page, users }) => {
      ...
      await page.waitForURL((url) => {
        return (
          url.pathname.endsWith("/pro/30min") &&
          url.searchParams.get("Test field") === "event-routing"
        );
      });
      ...
      await page.waitForURL((url) => {
        return (
          ...
          url.searchParams.get("Test field") === "external-redirect"
        );
      });
      ...
      await page.goto(
        `/router?form=${routingForm.id}&Test field=doesntmatter&multi=Option-2`
      );
    ...
    test("Routing Link should validate fields", async ({ page, users }) => {
      ...
      expect(
        await page.locator('button[type="submit"][disabled]').count()
      ).toBe(0);
    });

    test("Test preview should return correct route", async ({
      page,
      users,
    }) => {
      const user = await createUserAndLogin({ users, page });
      ...
      let routingType = await page
        .locator('[data-testid="test-routing-result-type"]')
        .innerText();
      let route = await page
        .locator('[data-testid="test-routing-result"]')
        .innerText();
      ...
      //external redirect
      await page.fill(
        '[data-testid="form-field-Test field"]',
        "external-redirect"
      );
      await page.click('[data-testid="test-routing"]');
      routingType = await page
...
async function expectCurrentFormToHaveFields(
  ...
  for (const [index, field] of Object.entries(fields)) {
    expect(await page.inputValue(`[data-testid="fields.${index}.label"]`)).toBe(
      field.label
    );
    expect(
      await page
