...
import { RoutingFormSettings } from "@sln/prisma/zod-utils";
...
export async function getSerializableForm<TForm extends App_RoutingForms_Form>({
  ...
  const prisma = (await import("@sln/prisma")).default;
  ...
  const { routes, routers } = await getEnrichedRoutesAndRouters(
    parsedRoutes,
    form.userId
  );
   ...
   */
  async function getEnrichedRoutesAndRouters(
    parsedRoutes: z.infer<typeof zodRoutes>,
    userId: number
  ) {
    ...
    for (const [, route] of Object.entries(parsedRoutes)) {
      if (isRouter(route)) {
        ...
        if (!router) {
          throw new Error(
            `Form - ${route.id}, being used as router, not found`
          );
        }
