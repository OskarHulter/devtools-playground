...
import { orgDomainConfig } from "@sln/features/ee/organizations/lib/orgDomains";
import logger from "@sln/lib/logger";
import { TRPCError } from "@sln/trpc/server";
import type {
  AppGetServerSidePropsContext,
  AppPrisma,
} from "@sln/types/AppGetServerSideProps";
...
export const getServerSideProps = async function getServerSideProps(
  ...
  const { UserRepository } = await import("@sln/lib/server/repository/user");
  ...
  if (
    !(await isAuthorizedToViewTheForm({
  ...
  const { createContext } = await import("@sln/trpc/server/createContext");
  const ctx = await createContext(context);

  const { default: trpcRouter } = await import(
    "@sln/app-store/routing-forms/trpc/_router"
  );
