...
import { areTheySiblingEntitites } from "@sln/lib/entityPermissionUtils";
...
const Route = ({
  ...
  const { data: eventTypesByGroup, isLoading } =
                ...
                route.action?.type === "customPageMessage" ? (
                  ...
                  <div className="block w-full">
                    <Select
                      ...
                      options={
                        eventOptions.length !== 0
                          ? [{ label: t("custom"), value: "custom" }].concat(
                              eventOptions
                            )
                          : []
                      }
                      onChange={(option) => {
                        ...
                        if (option.value !== "custom") {
                          ...
                          setRoute(route.id, {
                            action: { ...route.action, value: "custom" },
                          });
                          setCustomEventTypeSlug("");
                        }
                      }}
                ...
                <Query
                  ...
                  onChange={(immutableTree, config) => {
                    onChange(
                      route,
                      immutableTree,
                      config as QueryBuilderUpdatedConfig
                    );
                  }}
...
const Routes = ({
  ...
  const isConnectedForm = (id: string) =>
    form.connectedForms.map((f) => f.id).includes(id);
