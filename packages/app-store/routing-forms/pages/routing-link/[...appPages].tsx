...
import { navigateInTopWindow } from "@sln/lib/navigateInTopWindow";
import { trpc } from "@sln/trpc/react";
import type { inferSSRProps } from "@sln/types/inferSSRProps";
import { Button, showToast, useCalcomTheme } from "@sln/ui";
...
function RoutingForm({ form, profile, ...restProps }: Props) {
  ...
  const responseMutation =
    trpc.viewer.appRoutingForms.public.response.useMutation({
      onSuccess: async () => {
        const decidedActionWithFormResponse =
          decidedActionWithFormResponseRef.current;
        ...
        const allURLSearchParams = getUrlSearchParamsToForward(
          decidedActionWithFormResponse.response,
          fields
        );
...
function getUrlSearchParamsToForward(
  response: Response,
  fields: NonNullable<Props["form"]["fields"]>
) {
  ...
  Object.entries(response).forEach(([key, fieldResponse]) => {
    ...
    const valueAsStringOrStringArray =
      typeof fieldResponse.value === "number"
        ? String(fieldResponse.value)
        : fieldResponse.value;
    paramsFromResponse[
      getFieldIdentifier(foundField) as keyof typeof paramsFromResponse
