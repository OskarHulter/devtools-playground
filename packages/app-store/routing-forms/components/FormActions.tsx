...
} from "@sln/features/embed/RoutingFormEmbed";
...
function NewFormDialog({ appUrl }: { appUrl: string }) {
  ...
  const { action, target } = routerQuery as z.infer<
    typeof newFormModalQuerySchema
  >;
...
>(
  ...
  > = {
    ...
    duplicate: {
      onClick: () =>
        openModal({ action: "duplicate", target: routingForm?.id }),
    },
    embed: {
