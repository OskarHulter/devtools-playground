...
import AddMembersWithSwitch from "@sln/features/eventtypes/components/AddMembersWithSwitch";
import { ShellMain } from "@sln/features/shell/Shell";
import useApp from "@sln/lib/hooks/useApp";
...
const Actions = ({
  ...
  return (
    <div className="flex items-center">
      ...
      <ButtonGroup
        combined
        containerProps={{ className: "hidden md:inline-flex items-center" }}
      >
...
function SingleForm({
  ...
  const [decidedAction, setDecidedAction] = useState<Route["action"] | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (
      Object.keys(hookForm.getValues()).length === 0 ||
      hookForm.getValues().id !== form.id
    ) {
      hookForm.reset(form);
    }
  ...
  const sendUpdatesTo = hookForm.watch(
                      ...
                      <div className="flex flex-col">
                        ...
                        <AddMembersWithSwitch
                          ...
                          onChange={(value) => {
                            ...
                            hookForm.setValue(
                              "settings.emailOwnerOnSubmission",
                              false,
                              {
                                shouldDirty: true,
                              }
                          ...
                          setAssignAllTeamMembers={(value) => {
                            hookForm.setValue("settings.sendToAll", !!value, {
                              shouldDirty: true,
                            });
                          }}
                        ...
                        {form.routers.map((router) => {
                          return (
                            <div key={router.id} className="mr-2">
                              <Link
                                href={`${appUrl}/route-builder/${router.id}`}
                              >
                    ...
                    <div className="mt-2">
                      {RoutingPages.map((page) => {
                        ...
                        return (
                          <span
                            key={page.value}
                            data-testid="test-routing-result-type"
                          >
                            {page.label}
                          </span>
                        );
...
export default function SingleFormWrapper({
  ...
  const { data: form, isPending } =
    trpc.viewer.appRoutingForms.formQuery.useQuery(
      { id: _form.id },
      {
        initialData: _form,
        trpc: {},
