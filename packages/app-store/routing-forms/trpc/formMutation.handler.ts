...
export const formMutationHandler = async ({
  ctx,
  input,
}: FormMutationHandlerOptions) => {
  ...
  if (duplicateFrom) {
    ({ teamId, routes, fields } = await getRoutesAndFieldsForDuplication({
      duplicateFrom,
      userId: user.id,
    }));
  } else {
   ...
   */
  async function getUpdatedRouterLinkedFields(
    fields: InputFields,
    routes: InputRoutes
  ) {
  ...
  async function updateFieldsInConnectedForms(
    ...
    for (const [, connectedForm] of Object.entries(
      serializedForm.connectedForms
    )) {
      ...
      const fieldsThatAreNotInConnectedForm = (
        inputFields?.filter(
          (f) => !findFieldWithId(f.id, connectedFormFields || [])
        ) || []
      ).map((f) => ({
        ...f,
      ...
      const updatedConnectedFormFields = connectedFormFields
        ...
        ?.map((field) => {
          if (
            isRouterLinkedField(field) &&
            field.routerId === serializedForm.id
          ) {
            return {
              ...field,
  ...
  function markMissingFieldsDeleted(
    ...
    const deletedFields =
      serializedForm.fields?.filter(
        (f) => !fields.find((field) => field.id === f.id)
      ) || [];
