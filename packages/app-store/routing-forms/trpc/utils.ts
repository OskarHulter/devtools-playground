...
import { WebhookTriggerEvents } from "@sln/prisma/client";
  ...
  if (form.settings?.emailOwnerOnSubmission) {
    ...
    logger.debug(
      `Preparing to send Form Response email for Form:${
        form.id
      } to users: ${form.userWithEmails.join(",")}`
    );
  ...
  try {
    if (typeof window === "undefined") {
      const { default: ResponseEmail } = await import(
        "../emails/templates/response-email"
      );
      const email = new ResponseEmail({
        form: form,
        toAddresses,
        orderedResponses,
      });
...
function getWebhookTargetEntity(form: {
  teamId?: number | null;
  user: { id: number };
}) {
  ...
  return {
    userId: isTeamForm ? null : form.user.id,
    teamId: isTeamForm ? form.teamId : null,
  };
}
